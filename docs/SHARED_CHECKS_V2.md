...
Primary פעיל עבור בעלים עם סמן cutover מאומת; בזמן הכנת cutover עמידה פועל מצב `preparing` סגור לעריכות רגילות כדי לאתחל ולסנכרן את Shared לפני הסמן. מצב Shadow אופציונלי משמש להשוואת replay בתקופת המעבר ואינו מקור סמכות. מסלול first-cloud של cutover לחשבון קיים מתואם עם Main וניתן לחידוש אחרי קריסה. התקנה מקומית חדשה והעלאה ראשונה מ־`local` עדיין אינן V2-only, והחלפת חשבון A→B טרם חוברה ל־production. אין למחוק V1 עד להשלמת המסלולים הללו ולאימות השימוש בשני המחשבים.
