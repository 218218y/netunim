...
Cheque identity enrichment (Bridge v47)
...
Bridge v47 returned-cheque identity: returned-cheque and returned-cheque-credit rows remain non-deposit transactions (`cheque=false`), but their bank-provided reference plus contra bank/branch/account fields are preserved in typed checkDetails so the UI and reconciliation layer can identify the cheque. PFM transaction numbers never replace the original return reference. The detail parser also accepts additional semantic wrapper names (for example fieldDesc/displayLabel/formattedValue) and explicit labelled cheque-number text, but it still refuses arbitrary unlabeled numbers.
