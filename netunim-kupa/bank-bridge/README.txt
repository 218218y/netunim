...
Supported credentials keep the same contract: Visa Cal and Max use username/password; Isracard and American Express use id/card6Digits/password. Bridge v41 keeps the published israeli-bank-scrapers 6.10.0 dependency for the other maintained paths while Amex uses a local adaptation of upstream PR #1159 until that change is released. Multiple profiles for the same provider are supported only for different login identities. The same Visa Cal/Max username or the same Isracard/Amex ID cannot be stored twice for the same provider, because one issuer login already returns all cards visible to that identity. Profiles for different identities are scraped sequentially to avoid cookie/session collisions.
...
Bridge v41 preserves Visa Cal's issuer credit limit, the maintained Isracard frame path, and the Amex DigitalV3 limit data from GetCardList. For MAX, Isracard and Amex, balance represents utilized credit with a negative sign, so the bridge derives exact availableCredit as cardFrame + balance. Visa Cal remains deliberately excluded because its balance is the next debit rather than total limit use. A stored manualFrame is never deleted: it remains a fallback when the issuer does not provide a usable frame. Missing numeric values remain null rather than being coerced to zero.
