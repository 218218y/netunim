...
A full credit refresh requests 130 historical days and up to 12 future months with combineInstallments=false and without raw transaction payloads. The 130-day lookback guarantees enough source data for the current month plus three complete prior calendar months. Bridge v30 additionally makes the ordinary/daily refresh intentionally narrow (current month + next month) while preserving older/future Last Known Good month slices from the last full refresh. The bridge returns a normalized safe subset: card/account number, optional issuer balance/balance date, optional credit limit/available credit, billing/installment dates, optional purchase/transaction date, amounts, description, installments and status. No login secrets or raw HTML are returned. Pending issuer rows remain visible for review but are intentionally excluded from the Kupa cash-flow forecast until the issuer posts them with a trustworthy billing date.
...
Bridge v30 — daily/full credit workload split
--------------------------------------------
Bridge v30 keeps Credit Connector contract v2 but makes POST /v2/credit/sync mode-aware. syncMode=daily is the default and reads only the current month plus the next month (along with provider-specific pending/frame data). syncMode=full keeps the 130-day history and +12-month issuer horizon. Kupa and Orders use daily for the main Refresh button and the once-per-day automatic refresh; the advanced panel exposes an explicit "רענון מלא · 12 חודשים" action.

This is a workload policy change, not a retry workaround. Visa Cal still uses one login and sequential per-card Frames, Pending and monthly reads; MAX/Isracard and the Camoufox Isracard-family path receive the same daily/full date scope. No extra retry is added for 403/429. Existing month slices that are outside a daily result remain in the credit feed as Last Known Good because the web merge updates month keys individually rather than replacing the whole account history.
