...
--------------------------------------------------------------
Bridge v42 fixes a status-boundary bug in the generic credit adapter: a provider row with status=pending is now always treated as pending, even when the provider intentionally fills processedDate with the purchase timestamp. MAX and Isracard-group DigitalV3 both use this shape. Pending approvals are kept outside finalized monthly slices and Kupa cash-flow, but are shown immediately in a dedicated "עסקאות ממתינות" review table.
