...
export function classifyCreditHttpResponse({status=0,text:body='',stage='',retryAfter='',now=Date.now()}={}){
  ...
  if(httpStatus===403)return safeError('חברת האשראי דחתה את בקשת הנתונים של הסשן הנוכחי.','CREDIT_PROVIDER_HTTP_ERROR',extra);
...
function parseVisaCalFrame(data,card){
  if(!data?.result||typeof data.result!=='object')throw safeError('כאל החזירה מבנה מסגרות שאינו תואם לחוזה המחבר.','CREDIT_PROVIDER_SCHEMA_ERROR',{stage:'Frames'});
  const bankGroup=data.result.bankIssuedCards,calGroup=data.result.calIssuedCards,bankFrame=bankGroup?.cardLevelFrames?.find?.(item=>item?.cardUniqueId===card.cardUniqueId),calFrame=calGroup?.cardLevelFrames?.find?.(item=>item?.cardUniqueId===card.cardUniqueId),frame=bankFrame||calFrame,group=bankFrame?bankGroup:calGroup;
  const amount=frame?.nextTotalDebit??group?.nextTotalDebitForAccount,limit=group?.frameLimitForCardAmount;
  return {balance:Number.isFinite(Number(amount))?-Number(amount):null,balanceDate:safeDate(frame?.nextDebitDate??group?.nextTotalDebitDateForAccount),cardType:bankFrame?'bankIssued':calFrame?'companyIssued':'',cardFrame:Number.isFinite(Number(limit))?Number(limit):null};
}
...
function coverageError(profile,error,{month='',tier='',accountNumber='',at=new Date().toISOString()}={}){return {profileId:profile.profileId,provider:profile.provider,label:profile.label,code:String(error?.code||'CREDIT_PROVIDER_DATA_ERROR'),stage:String(error?.stage||'Transactions').slice(0,80),httpStatus:Number(error?.httpStatus)||0,message:error?.message||'קריאת נתוני האשראי נכשלה',at,retryAfterAt:error?.retryAfterAt||null,month,tier,accountSuffix:safeSuffix(accountNumber)}}
  ...
  async scrape(){
    ...
    try{
      ...
      for(const card of cards){
        const at=this.now().toISOString(),accountNumber=text(card?.last4Digits,80),account={accountNumber,balance:null,balanceDate:null,cardType:'',cardFrame:null,availableCredit:null,pendingTransactions:[],pendingStatus:'missing',months:[]};
        try{Object.assign(account,parseVisaCalFrame(await this.request(CAL_ENDPOINTS.frames,{cardsForFrameData:[{cardUniqueId:card.cardUniqueId}]},'Frames'),card))}catch(error){errors.push(coverageError(profile,error,{accountNumber,at}))}
        if(this.requestDelayMs>0)await sleep(this.requestDelayMs);
        try{account.pendingTransactions=parseVisaCalPending(await this.request(CAL_ENDPOINTS.pending,{cardUniqueIDArray:[card.cardUniqueId]},'Pending'));account.pendingStatus='success';account.pendingFetchedAt=at}catch(error){account.pendingStatus=errorFetchStatus(error);account.pendingErrorCode=error.code;account.pendingErrorAt=at;errors.push(coverageError(profile,error,{accountNumber,at}))}
        for(const entry of plan){
          ...
          catch(error){const failedAt=this.now().toISOString();account.months.push(monthlyCoverageFailure(entry,error,failedAt));errors.push(coverageError(profile,error,{month:entry.month,tier:entry.tier,accountNumber,at:failedAt}))}
        }
      ...
      if(forecastFailures)errors.unshift({profileId:profile.profileId,provider:profile.provider,label:profile.label,code:'CREDIT_PARTIAL_FORECAST',stage:'Forecast',httpStatus:0,message:`לכאל חסרים ${forecastFailures} מקטעי תחזית; נתונים קודמים נשמרים כ־Last Known Good.`,at:this.now().toISOString()});
      if(coreFailures)errors.unshift({profileId:profile.profileId,provider:profile.provider,label:profile.label,code:'CREDIT_CORE_COVERAGE_INCOMPLETE',stage:'CoreCoverage',httpStatus:0,message:`לכאל חסרים ${coreFailures} מקטעי ליבה; זמן ההצלחה המלאה לא התקדם ונתוני Last Known Good נשמרו.`,at:this.now().toISOString()});
