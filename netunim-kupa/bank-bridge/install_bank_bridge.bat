...
for %%F in (server.mjs lib.mjs bank-diagnostics.mjs credit-adapters.mjs credit-diagnostics.mjs credit-data-diagnostics.mjs credit-identity.mjs amex-digitalv3.mjs isracard-digitalv3.mjs isracard-camoufox.mjs provision-camoufox.mjs package.json package-lock.json start_bank_bridge.bat) do (
...
node -e "fetch('http://127.0.0.1:8765/health',{cache:'no-store'}).then(r=>r.json()).then(j=>{if(j.service!=='netunim-kupa-bank-bridge'||!(Number(j.version)>=55)||Number(j.creditContractVersion)!==2)process.exit(2)}).catch(()=>process.exit(1))"
if errorlevel 1 (
  ...
  echo ERROR: Bank Bridge v55 did not start correctly. The previous runtime was restored when available.
...
echo American Express uses installed Chrome/Edge with the current DigitalV3 connector; Camoufox under %CAMOUFOX_INSTALL_DIR% remains a bounded fallback only.
