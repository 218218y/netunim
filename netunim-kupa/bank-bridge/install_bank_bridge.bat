...
node -e "fetch('http://127.0.0.1:8765/health',{cache:'no-store'}).then(r=>r.json()).then(j=>{if(j.service!=='netunim-kupa-bank-bridge'||!(Number(j.version)>=45)||Number(j.creditContractVersion)!==2)process.exit(2)}).catch(()=>process.exit(1))"
if errorlevel 1 (
