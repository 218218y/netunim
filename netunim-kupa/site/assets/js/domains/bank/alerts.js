...
export function createDomainsBankAlerts({model,bankProjectedThisMonth,bankHomeProjectedThisMonth,modal,closeModal}){
  ...
  function maybeShowStartupCashflowAlert(){
    ...
    modal('אזהרת עו״ש תזרימי',`<div class="soft-note">התחזית מבוססת על יתרת העו״ש האחרונה פחות סליקות אשראי והוצאות של אותו חשבון, ובתוספת הצ׳קים מאותו חשבון שעדיין בקופה ונכנסים עד תאריך התחזית.</div>${rows}`,'הבנתי',()=>closeModal(true));
    return true;
  }
