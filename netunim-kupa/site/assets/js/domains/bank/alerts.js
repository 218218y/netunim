...
export function createDomainsBankAlerts({model,bankProjectedThisMonth,bankHomeProjectedThisMonth,modal,closeModal}){
  ...
  function maybeShowStartupCashflowAlert(){
    ...
    const active=['עסקי','ביתי'].map(account=>cashflowNotificationData(kupaAccountCashflowData(model.state,account),model.state.cashflowSettings)).filter(row=>row.active);
    if(!active.length){if(business!==null&&home!==null)resolved=true;return false}
    shown=true;resolved=true;
    const rows=active.map(row=>`<div class="notice danger" style="margin-top:9px"><b>חשבון ${row.account}</b> · עו״ש תזרימי צפוי: <b>${money(row.projected)}</b><br>${alertReason(row)} · מועד צפוי: ${row.breachDate.split('-').reverse().join('/')}.</div>`).join('');
    modal('אזהרת עו״ש תזרימי',`<div class="soft-note">התחזית מבוססת על יתרת העו״ש האחרונה פחות סליקות אשראי והוצאות של אותו חשבון, ובתוספת הצ׳קים מאותו חשבון שעדיין בקופה ונכנסים עד מועד החריגה. הבדיקה מכסה עד סוף החודש הנוכחי או עד תאריך התחזית הרגילה, המאוחר מביניהם.</div>${rows}`,'הבנתי',()=>closeModal(true));
    return true;
  }
