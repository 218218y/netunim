...
function bankTransactionsTableMarkup(feed,role){
  ...
  const body=rows.map((row,index)=>{
    ...
    return `<tr class="${row.status==='pending'?'pending ':''}${activeMissing?'bank-transaction-missing active':''}" data-bank-search-id="${esc(bankTransactionIdentity(row,role,index))}"><td class="bank-transaction-date"><b>${esc(bankDateLabel(when))}</b>${valueDate?`<small>ערך ${esc(valueDate)}</small>`:''}</td><td class="bank-transaction-description"><div><b>${esc(row.description||'תנועת בנק')}</b>${row.status==='pending'?'<span class="bank-transaction-pending">ממתינה</span>':''}${activeMissing?'<span class="bank-transaction-missing-badge">דורש בדיקה</span>':''}</div>${row.memo?`<small>${esc(row.memo)}</small>`:''}${activeMissing?bankMissingDetailMarkup(row):''}${bankChequeDetailsMarkup(row)}</td><td class="bank-transaction-money debit">${amount<0?money(Math.abs(amount)):''}</td><td class="bank-transaction-money credit">${amount>0?money(amount):''}</td><td class="bank-transaction-money balance-after">${row.balanceAfter!==null&&row.balanceAfter!==undefined&&row.balanceAfter!==''&&Number.isFinite(Number(row.balanceAfter))?money(Number(row.balanceAfter)):'—'}</td></tr>`;
  }).join('');
