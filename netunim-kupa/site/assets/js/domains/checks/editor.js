...
function markDeposited(id){const c=model.state.checks.find(x=>x.id===id);if(!c||c.status==='הופקד - במעקב')return;c.status='הופקד - במעקב';c.depositDate=todayISO();c.depositedAt=new Date().toISOString();c.depositSeq=null;c.clearedDate=null;saveChecksState('הצק סומן כהופקד')}
