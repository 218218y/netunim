...
function renderCheckGroups(groups){const keys=Object.keys(groups).sort();if(!keys.length)return '<section class="section"><div class="empty">אין צקים בתצוגה הזאת.</div></section>';return keys.map(k=>{const arr=groups[k],sum=arr.reduce((a,x)=>a+x.amount,0);return `<div class="month-group"><div class="month-title"><b>${monthLabel(k)}</b><span class="month-check-total">${esc(arr.length)} צקים · ${money(sum)}</span></div><div style="overflow:auto"><table><thead><tr>${bulkHeader('checks')}<th>שם</th><th>סכום</th><th>פירעון</th><th>סטטוס</th><th>התראה</th><th>מס׳ צק</th><th>הערה</th><th></th></tr></thead><tbody>${arr.map(checkRow).join('')}</tbody></table></div></div>`}).join('')}

function checkRow(c){return `<tr data-bulk-collection="checks" data-bulk-id="${esc(c.id)}" class="${esc(ui.bulkSelected.has(c.id)?'bulk-selected-row':'')}">${bulkCell('checks',c.id)}<td><b>${esc(c.name)}</b></td><td class="amount">${money(c.amount)}</td><td>${dateFmt(c.dueDate)}</td><td>${badgeStatus(c.status)}</td><td>${dueBadge(c)}</td><td>${esc(c.checkNumber)||'—'}</td><td class="muted">${esc(c.note)||'—'}</td><td><div class="row-actions">${c.status==='בקופה'?`<button class="iconbtn" data-action="mark-deposited" data-click-arg0="${esc(c.id)}">הופקד</button>`:''}${c.status==='הופקד - במעקב'?`<button class="iconbtn" data-action="mark-cleared" data-click-arg0="${esc(c.id)}">נפרע</button>`:''}<button class="iconbtn" data-action="open-check-modal-2" data-click-arg0="${esc(c.id)}">עריכה</button></div></td></tr>`}
