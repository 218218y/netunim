...
import {creditCardMappingKey,creditSyncHasData,mergeCreditSyncResult,normalizeCreditSync,CREDIT_PROVIDER_LABELS} from './sync-feed.js';
...
function providerFields(provider){return provider==='isracard'?['id','card6Digits','password']:['username','password']}
...
export function createDomainsCreditController({model,saveState,toast,render,bridge,modal,armModalDraftGuard,closeModal,confirmDialog}){
  ...
  async function refreshCreditBridgeStatus({quiet=true}={}){
    try{
      ...
      if(Number(status.bridgeVersion||0)<11)local.error='Bank Bridge ישן. יש להריץ שוב install_bank_bridge.bat במחשב זה.';
  ...
  function openCreditConnectionModal(profileId=''){
    ...
    const body=`<form id="creditConnectionForm" class="form-grid credit-connect-grid" autocomplete="off">
      <div class="form-group"><label>חברה</label><select id="ccProvider"><option value="visaCal" ${provider==='visaCal'?'selected':''}>כאל</option><option value="max" ${provider==='max'?'selected':''}>MAX</option><option value="isracard" ${provider==='isracard'?'selected':''}>ישראכרט</option></select></div>
      ...
      <div class="form-group cc-field cc-card6"><label>6 ספרות אחרונות של כרטיס</label><input id="ccCard6" inputmode="numeric" maxlength="6" placeholder="${canPreserveCredentials?'השאר ריק כדי לא לשנות':''}"></div>
      ...
      <div class="form-group full"><div class="notice">כרטיס Mastercard מחברים לפי החברה המנפיקה שלו — כאל, MAX או ישראכרט — ולא כחיבור נפרד.</div></div>
    </form>`;
    ...
    const update=()=>{const fields=new Set(providerFields(select.value));document.querySelector('.cc-username').hidden=!fields.has('username');document.querySelector('.cc-id').hidden=!fields.has('id');document.querySelector('.cc-card6').hidden=!fields.has('card6Digits')};
  ...
  async function refreshCreditSync({interactive=false,auto=false}={}){
    ...
    try{
      ...
      if(Number(status.bridgeVersion||0)<11)throw new Error('יש לשדרג את Bank Bridge לפני סנכרון אשראי');
  ...
  async function setCreditCardMapping(profileId,accountNumber,field,value){
    const sync=normalizeCreditSync(model.state.creditSync),key=creditCardMappingKey(profileId,accountNumber),current=sync.cardMappings[key]||{account:'עסקי',cardName:''};
