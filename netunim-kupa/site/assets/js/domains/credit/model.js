...
export function creditDetailItemIdentity(item={}){
  ...
  if(item.source==='credit_unknown')return `unknown:${String(item.creditId||item.id||'')}:${String(item.date||'unassigned')}:${Number(item.part||1)}`;
...
export function creditForecastInstallmentsData(state,asOf=todayISO()){return kupaReconciledCreditRowsData(state,'all',asOf).filter(row=>row.date&&row.date>=asOf).filter(row=>row.includedInIlsTotal?Math.abs(row.amount)>0.004:true)}
