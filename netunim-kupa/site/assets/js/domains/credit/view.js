...
import {creditViewAllowsMonth,creditDetailDayMatch,creditUpcomingSelectorMarkup,creditHistoryMenuMarkup,creditDetailMonthTabsMarkup,creditDetailChargeHeadingMarkup} from '../../shared/credit-detail-controls.js';
...
const creditControlBase={formatMoney:money,formatMonth:monthLabel,escapeHtml:esc,monthAction:'credit-detail-month',monthDayAction:'credit-detail-month-day'};
function creditUpcomingControl(active,selectedDay,total){return creditUpcomingSelectorMarkup({...creditControlBase,active,selectedDay,total,upcomingAction:'credit-detail-upcoming',upcomingDayAction:'credit-detail-upcoming-day'})}
...
function creditMonthTabsControl(months,selectedKey='',selectedDay='all'){return creditDetailMonthTabsMarkup({...creditControlBase,months,selectedKey,selectedDay,currentMonth:monthKey(todayISO())})}
...
function creditDetailSectionMarkup(){
  ...
  return `<section id="credit-active-transactions" class="section credit-detail-section" style="margin-top:16px"><div class="section-head credit-detail-section-head"><div><div class="credit-detail-title-row"><h3 title="מוצגים עד ${esc(CREDIT_DETAIL_HISTORY_MONTHS)} חודשים קודמים וכל החיובים העתידיים שהתקבלו מהחברות">עסקאות ותשלומים</h3>${creditHistoryControl(historyMonths,selectedMonthKey,upcoming?'all':chargeDay)}${creditUpcomingControl(upcoming,chargeDay,upcomingTotal)}</div>${searching?`<small class="credit-search-count">${esc(detailItems.length)} מתוך ${esc(focusedCount)} תוצאות</small>`:''}</div>${creditMonthTabsControl(detailMonths,selectedMonthKey,upcoming?'all':chargeDay)}</div>${detailFocus?.cardKey?`<div class="credit-detail-focus"><span><b>${esc(detailFocusLabel)}</b><small>${esc(focusMonthLabel)} · מיקוד בכרטיס מתוך התחזית</small></span><button type="button" class="iconbtn" data-action="clear-credit-detail-focus" data-click-arg0="${esc(detailFocus.monthKey)}">כל הכרטיסים ×</button></div>`:''}<div class="credit-detail-table-wrap"><table class="credit-detail-table"><thead><tr>${bulkHeader('credits')}<th class="credit-detail-col-card">כרטיס</th><th class="credit-detail-col-description">תיאור</th><th class="credit-detail-col-transaction-date">תאריך עסקה</th><th class="credit-detail-col-total">סכום מקורי</th><th class="credit-detail-col-installment">תשלום</th><th class="credit-detail-col-charge">${creditChargeHeading(detailItems)}</th><th class="credit-detail-col-status">מצב</th><th class="credit-detail-col-actions"></th></tr></thead><tbody>${rowsMarkup}</tbody></table></div></section>`;
}
