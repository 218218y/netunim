...
function creditDetailState(){
  const mode=ui.creditDetailMode==='month'?'month':'upcoming',chargeDay=ui.creditDetailChargeDay==='10'||ui.creditDetailChargeDay==='15'?ui.creditDetailChargeDay:'all',detailData=creditMonthlyDetailData(model.state),upcomingData=creditUpcomingDetailData(model.state);
  ...
  if(mode==='upcoming'){
    ...
    return {mode,chargeDay,detailMonths,detailFocus:null,detailItems,focusedCount:focusedItems.length,detailFocusLabel:'',upcomingTotal:allUpcomingItems.reduce((sum,item)=>sum+item.amount,0)};
  }
  const currentMonth=monthKey(todayISO()),detailToday=todayISO();
  const nearestDetailMonth=detailMonths.find(month=>month.key!=='unassigned'&&month.items.some(item=>(item.date||item.detailDisplayBillingDate||'')>=detailToday))?.key||detailMonths.find(month=>month.key===currentMonth)?.key||detailMonths.at(-1)?.key||currentMonth;
  ...
  return {mode,chargeDay,detailMonths,detailFocus,detailItems,focusedCount:focusedItems.length,detailFocusLabel,upcomingTotal:0};
}
...
function creditDetailSectionMarkup(){
  const {mode,chargeDay,detailMonths,detailFocus,detailItems,focusedCount,detailFocusLabel,upcomingTotal}=creditDetailState();
  ...
  const emptyText=searching&&focusedCount?'אין עסקאות או תשלומים המתאימים לחיפוש.':upcoming?'אין חיוב קרוב להצגה במסנן הנוכחי.':detailMonths.length?'אין עסקאות להצגה בחודש שנבחר.':`אין עסקאות ידועות בשלושת החודשים הקודמים או בחודשים העתידיים במסנן הנוכחי.`;
