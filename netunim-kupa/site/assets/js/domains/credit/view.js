  ...
  document.getElementById('content').innerHTML=`
    ...
    <section class="section credit-sync-section">
      ...
      <div id="creditSyncPanel" class="credit-sync-settings-body" ${ui.creditSyncOpen?'':'hidden'}>
          <div class="credit-sync-settings-top"><div><b>אפשרויות סינכרון אשראי</b><small>רענון רגיל ועדכון אוטומטי קוראים רק את החודש הנוכחי והחודש הבא. רענון מלא מעדכן 130 ימי היסטוריה ו־12 חודשים קדימה; חלון אבחון אינו עוקף cooldown.</small></div><div class="credit-sync-panel-actions"><button class="btn" type="button" data-action="refresh-credit-sync-full" ${syncUi.busy?'disabled':''}>רענון מלא · 12 חודשים</button><button class="btn" type="button" data-action="refresh-credit-sync-interactive" ${syncUi.busy?'disabled':''}>רענון יומי עם חלון אבחון</button><button class="btn" type="button" data-action="copy-safe-credit-diagnostics" ${syncUi.busy?'disabled':''}>העתק אבחון טכני בטוח</button></div></div>
