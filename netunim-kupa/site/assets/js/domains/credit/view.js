...
function creditDetailState(){
  const mode=ui.creditDetailMode==='upcoming'?'upcoming':'month',detailData=creditMonthlyDetailData(model.state),upcomingData=creditUpcomingDetailData(model.state);
  ...
  if(mode==='upcoming'){
    const focusedItems=upcomingData.items.filter(item=>filterMatch(ui,item)),detailItems=focusedItems.filter(item=>creditSearchMatch(ui.creditSearchValue,item));
    return {mode,detailMonths,detailFocus:null,detailItems,focusedCount:focusedItems.length,detailFocusLabel:''};
  }
  ...
  const focusedItems=(detailMonth?.items||[]).filter(item=>!detailFocus?.cardKey||rowCardKey(item)===detailFocus.cardKey);
  ...
  return {mode,detailMonths,detailFocus,detailItems,focusedCount:focusedItems.length,detailFocusLabel};
}
...
function creditDetailSectionMarkup(){
  const {mode,detailMonths,detailFocus,detailItems,focusedCount,detailFocusLabel}=creditDetailState();
...
function creditDetailMonthTabs(months,selectedKey){
  ...
  return `<div class="credit-detail-month-tabs" role="tablist" aria-label="בחירת חודש לעסקאות ותשלומים">${months.map(month=>{const uncertain=month.key==='unassigned',timing=uncertain?'future':month.key<current?'past':month.key===current?'current':'future',label=uncertain?'מחזור לא ודאי':monthLabel(month.key);return `<button type="button" role="tab" aria-selected="${month.key===selectedKey?'true':'false'}" class="credit-detail-month-tab ${month.key===selectedKey?'active':''} ${timing}" data-action="credit-detail-month" data-click-arg0="${esc(month.key)}"><span>${esc(label)}</span><b>${uncertain?esc(month.items.length)+' עסקאות':`${money(month.total)}${month.partial?' · חלקי':''}`}</b></button>`}).join('')}</div>`;
}
