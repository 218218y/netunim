...
import {creditMonthlyDetailData,creditUpcomingDetailData,creditDetailItemIdentity,creditForecastInstallmentsData,CREDIT_DETAIL_HISTORY_MONTHS} from './model.js';
...
function creditDetailChargeDay(row){const date=String(row?.date||row?.detailDisplayBillingDate||'').slice(0,10),day=Number(date.slice(8,10));if(!Number.isInteger(day))return '';if(day>=7&&day<=12)return '10';if(day>=13&&day<=18)return '15';return ''}
function creditDetailDayMatch(row,day){return day!=='10'&&day!=='15'||creditDetailChargeDay(row)===day}
function creditViewAllowsMonth(monthValue,view,currentMonth=monthKey(todayISO())){const key=String(monthValue||'');if(!/^\d{4}-\d{2}$/.test(key)||key<currentMonth)return false;const selected=String(view||'rolling12');if(selected==='all')return true;if(/^\d{4}$/.test(selected))return key.startsWith(`${selected}-`);const end=monthKey(addMonthsISO(`${currentMonth}-01`,11));return key<=end}
...
function includedCardModels(summary,reconciledRows,asOf=todayISO()){
  return summary.sync.profiles.flatMap(profile=>profile.accounts.map(account=>{
    const mapping=summary.sync.cardMappings[creditCardMappingKey(profile.profileId,account.accountNumber)]||{},creditAccountKey=synchronizedCardKey(profile.profileId,account.accountNumber),billingDate=kupaCardDisplayBillingDateFromRowsData(reconciledRows,creditAccountKey,account,asOf).date;
    return {
...
function cardFilterMarkup(ui,cards){
  ...
  return `<div class="credit-filter-chip-row"><span class="credit-filter-label">כרטיסים</span><div class="credit-filter-chips"><button type="button" class="credit-filter-chip ${ui.creditCardFilter==='all'?'active':''}" data-action="credit-card-filter" data-click-arg0="all"><span>כל הכרטיסים</span><small>${esc(scoped.length)}</small></button>${scoped.map(card=>`<button type="button" class="credit-filter-chip card ${ui.creditCardFilter===card.creditAccountKey?'active':''}" data-action="credit-card-filter" data-click-arg0="${esc(card.creditAccountKey)}"><span>${esc(card.name)}</span><small>${esc(card.account)}${card.ownerLabel?` · ${esc(card.ownerLabel)}`:''}${card.billingDate?` · תאריך החיוב ${esc(dateFmt(card.billingDate))}`:''}</small></button>`).join('')}</div></div>`;
}
...
function creditDetailState(){
  const mode=ui.creditDetailMode==='month'?'month':'upcoming',chargeDay=ui.creditDetailChargeDay==='10'||ui.creditDetailChargeDay==='15'?ui.creditDetailChargeDay:'all',detailData=creditMonthlyDetailData(model.state),upcomingData=creditUpcomingDetailData(model.state),currentMonth=monthKey(todayISO());
  ...
  const historyMonths=allDetailMonths.filter(month=>month.key!=='unassigned'&&month.key<currentMonth),detailMonths=allDetailMonths.filter(month=>month.key==='unassigned'||creditViewAllowsMonth(month.key,ui.creditView,currentMonth)),selectableMonths=[...historyMonths,...detailMonths];
  if(mode==='upcoming'){
  ...
  if(!selectableMonths.length){detailFocus=null;ui.creditDetailFocus=null}
  else{
    if(!detailFocus||!selectableMonths.some(month=>month.key===detailFocus.monthKey))detailFocus={monthKey:nearestDetailMonth,cardKey:''};
    const selected=selectableMonths.find(month=>month.key===detailFocus.monthKey);
...
function creditDetailSectionMarkup(){
  ...
  const emptyText=searching&&focusedCount?'אין עסקאות או תשלומים המתאימים לחיפוש.':upcoming?'אין חיוב קרוב להצגה במסנן הנוכחי.':detailMonths.length||historyMonths.length?'אין עסקאות להצגה בחודש שנבחר.':`אין עסקאות ידועות בשלושת החודשים הקודמים או בחודשים העתידיים במסנן הנוכחי.`;
  const focusMonthLabel=detailFocus?.monthKey==='unassigned'?'מחזור לא ודאי':monthLabel(detailFocus?.monthKey||''),rowsMarkup=detailItems.length?(upcoming?upcomingDetailRowsMarkup(detailItems,colspan,creditMonthlyDetailRow):`${uncertainItems.length?`<tr class="credit-detail-cycle-divider uncertain"><td colspan="${colspan}"><b>מחזור לא ודאי</b><small>עסקאות ממתינות שמועד החיוב הסופי שלהן טרם נקבע; מוצגות כאן ליד החיוב הסמוך של אותו כרטיס.</small></td></tr>${uncertainItems.map(creditMonthlyDetailRow).join('')}`:''}${certainItems.length?`${uncertainItems.length?`<tr class="credit-detail-cycle-divider certain"><td colspan="${colspan}"><b>חיוב ${esc(focusMonthLabel)}</b></td></tr>`:''}${certainItems.map(creditMonthlyDetailRow).join('')}`:''}`):`<tr><td colspan="${colspan}"><div class="empty compact">${emptyText}</div></td></tr>`;
  const selectedMonthKey=upcoming?'':detailFocus?.monthKey||'';
  return `<section id="credit-active-transactions" class="section credit-detail-section" style="margin-top:16px"><div class="section-head credit-detail-section-head"><div><div class="credit-detail-title-row"><h3 title="מוצגים עד ${esc(CREDIT_DETAIL_HISTORY_MONTHS)} חודשים קודמים וכל החיובים העתידיים שהתקבלו מהחברות">עסקאות ותשלומים</h3>${creditHistorySelectorsMarkup(historyMonths,selectedMonthKey,upcoming?'all':chargeDay)}${creditUpcomingSelectorMarkup(upcoming,chargeDay,upcomingTotal)}</div>${searching?`<small class="credit-search-count">${esc(detailItems.length)} מתוך ${esc(focusedCount)} תוצאות</small>`:''}</div>${creditDetailMonthTabs(detailMonths,selectedMonthKey,upcoming?'all':chargeDay)}</div>${detailFocus?.cardKey?`<div class="credit-detail-focus"><span><b>${esc(detailFocusLabel)}</b><small>${esc(focusMonthLabel)} · מיקוד בכרטיס מתוך התחזית</small></span><button type="button" class="iconbtn" data-action="clear-credit-detail-focus" data-click-arg0="${esc(detailFocus.monthKey)}">כל הכרטיסים ×</button></div>`:''}<div class="credit-detail-table-wrap"><table class="credit-detail-table"><thead><tr>${bulkHeader('credits')}<th class="credit-detail-col-card">כרטיס</th><th class="credit-detail-col-description">תיאור</th><th class="credit-detail-col-transaction-date">תאריך עסקה</th><th class="credit-detail-col-total">סכום מקורי</th><th class="credit-detail-col-installment">תשלום</th><th class="credit-detail-col-charge">חיוב בחודש</th><th class="credit-detail-col-status">מצב</th><th class="credit-detail-col-actions"></th></tr></thead><tbody>${rowsMarkup}</tbody></table></div></section>`;
}
...
function toggleCreditSyncOptions(){
  ui.creditSyncOpen=!ui.creditSyncOpen;
  const panel=document.getElementById('creditSyncPanel'),toggle=document.getElementById('creditSyncToggle');
...
function renderCredit(){
  ...
  const filteredFuture=allFuture.filter(row=>filterMatch(ui,row)),unassigned=allReconciled.filter(row=>!row.date&&filterMatch(ui,row)),currentMonth=monthKey(todayISO()),currentYear=Number(currentMonth.slice(0,4));
  const futureYears=[...new Set(filteredFuture.map(x=>x.date.slice(0,4)))].map(Number).filter(y=>Number.isFinite(y)&&y>=currentYear);
  ...
  const future=filteredFuture.filter(row=>creditViewAllowsMonth(monthKey(row.date),ui.creditView,currentMonth)),countedPending=future.filter(row=>row.status==='pending'&&row.includedInIlsTotal),uncountedPending=[...future,...unassigned].filter(row=>row.status==='pending'&&!row.includedInIlsTotal),futureIncomplete=future.filter(row=>!row.includedInIlsTotal||row.coverageIncomplete).length+unassigned.length,futureMissingAmount=[...future,...unassigned].filter(row=>row.amountStatus!=='known_ils').length,futureCoverageGap=[...future,...unassigned].filter(row=>row.coverageIncomplete).length;
  const businessFuture=future.filter(x=>x.account==='עסקי'),businessUnassigned=unassigned.filter(x=>x.account==='עסקי'),businessIncomplete=businessFuture.filter(row=>!row.includedInIlsTotal||row.coverageIncomplete).length+businessUnassigned.length;
  ...
  const months=monthKeys.map(k=>{const inst=future.filter(x=>monthKey(x.date)===k),missingAmountCount=inst.filter(row=>row.amountStatus!=='known_ils').length,coverageGapCount=inst.filter(row=>row.coverageIncomplete).length,incompleteCount=inst.filter(row=>!row.includedInIlsTotal||row.coverageIncomplete).length;return {k,inst,total:inst.reduce((a,x)=>a+x.amount,0),incompleteCount,missingAmountCount,coverageGapCount,partial:incompleteCount>0}}).filter(month=>Math.round(month.total*100)!==0||month.partial);
  ...
  document.getElementById('content').innerHTML=`
    ...
    <section class="section credit-sync-section">
      ...
      <div id="creditSyncPanel" class="credit-sync-settings-body" ${ui.creditSyncOpen?'':'hidden'}>
          <div class="credit-sync-settings-top"><div><b>אפשרויות סינכרון אשראי</b><small>הכפתור הראשי רענן מבצע רענון מלא של 130 ימי היסטוריה ו־12 חודשים קדימה. בעדכון האוטומטי אפשר לבחור מהיר (חודש נוכחי + הבא) או מלא (12 חודשים); חלון אבחון אינו עוקף cooldown.</small></div><div class="credit-sync-panel-actions"><button class="btn" type="button" data-action="refresh-credit-sync-daily" ${syncUi.busy?'disabled':''}>רענון יומי מהיר</button><button class="btn" type="button" data-action="refresh-credit-sync-interactive" ${syncUi.busy?'disabled':''}>רענון יומי עם חלון אבחון</button><button class="btn" type="button" data-action="copy-safe-credit-diagnostics" ${syncUi.busy?'disabled':''}>העתק אבחון טכני בטוח</button><button class="btn" type="button" data-action="export-credit-data-diagnostics" ${syncUi.busy?'disabled':''}>ייצוא אבחון אשראי</button></div></div>
          ...
          <div class="credit-sync-settings-grid">
            ...
            <div class="credit-sync-panel"><div class="credit-sync-panel-head"><b>שיוך כרטיסים</b><span class="credit-profile-actions"><label class="credit-auto-toggle"><input type="checkbox" data-change="set-credit-auto-refresh" ${syncUi.autoEnabled?'checked':''}> עדכון אוטומטי פעם ביום</label><select aria-label="סוג עדכון אשראי אוטומטי" data-change="set-credit-auto-mode"><option value="daily" ${syncUi.autoMode==='daily'?'selected':''}>מהיר · חודש נוכחי + הבא</option><option value="full" ${syncUi.autoMode==='full'?'selected':''}>מלא · 12 חודשים</option></select></span></div>
    ...
    <div class="toolbar credit-filter-toolbar">
      <div class="credit-filter-primary">
        ...
        <div class="credit-filter-stats"><span class="stat-pill">סה״כ: ${money(future.reduce((a,x)=>a+x.amount,0))}${futureIncomplete?` · חלקי${futureMissingAmount?` · ${esc(futureMissingAmount)} ללא סכום ₪`:''}${futureCoverageGap?` · ${esc(futureCoverageGap)} בכיסוי LKG/חסר`:''}${unassigned.length?` · ${esc(unassigned.length)} ללא מועד חיוב`:''}`:''}</span><span class="stat-pill business">מתוכם עסקי: ${money(businessFuture.reduce((a,x)=>a+x.amount,0))}${businessIncomplete?` · חלקי${businessUnassigned.length?` · ${esc(businessUnassigned.length)} ללא מועד`:''}`:''}</span>${creditAvailableTotalMarkup(summary)}${bulkControls('credits')}<button class="btn primary" data-action="open-credit-modal">+ תוספת ידנית</button></div>
      </div>
      <div class="credit-filter-chip-stack">${providerFilterMarkup(ui,filterCards)}${cardFilterMarkup(ui,filterCards)}</div>
    </div>

    <section class="section credit-forecast-section forecast-disclosure-section"><div class="forecast-disclosure-head"><button type="button" class="forecast-disclosure-toggle ${ui.creditForecastOpen?'open':''}" data-action="toggle-credit-forecast" aria-expanded="${ui.creditForecastOpen===true}" aria-controls="creditForecastBody"><span>תצוגת אשראי עתידית לפי כרטיס</span><span class="forecast-disclosure-chevron" aria-hidden="true">⌄</span></button></div><div id="creditForecastBody" class="forecast-disclosure-body" ${ui.creditForecastOpen?'':'hidden'}><div class="credit-forecast-meta"><b>${esc(forecastTitle)}</b>${countedPending.length?`<small class="credit-forecast-pending-note">כולל זמנית ${esc(countedPending.length)} אישורים ממתינים בעלי סכום ₪ ומועד חיוב ידוע; עם אישור סופי הם מוחלפים אוטומטית בנתוני החברה.</small>`:''}${uncountedPending.length?`<small class="credit-forecast-pending-note">${esc(uncountedPending.length)} אישורים ממתינים נוספים אינם נכללים בסכום עד שיתקבלו סכום ₪ ומועד חיוב אמינים.</small>`:''}</div><div class="section-body"><div class="credit-forecast-list">${forecastRows}</div></div></div></section>
...
function upcomingChargeMarkup(account,provider,asOf,reconciled){
  const fallback=creditUpcomingCharge(account,provider,asOf),upcoming=reconciled??(fallback?.source==='issuer_balance'?fallback:null);
  if(!upcoming)return '<div class="credit-live-balance unavailable"><span>חיוב קרוב</span><b>לא זמין</b></div>';
  const pending=Number(upcoming.pendingAmount)||0,partial=upcoming.status==='incomplete'||upcoming.complete===false||upcoming.amountKnown===false,amount=upcoming.amountKnown===false&&Math.abs(Number(upcoming.amount)||0)<=0.004?'טרם ידוע':money(upcoming.amount),partialReason=Number(upcoming.unassignedCount)>0?'ללא מועד חיוב אמין':Number(upcoming.coverageGapCount)>0?'כיסוי חברה LKG/חסר':upcoming.amountKnown===false?'חסר סכום ₪ מלא':'',meta=[upcoming.date?dateFmt(upcoming.date):'',upcoming.source==='transactions'?'ממנוע המחזורים':'',Math.abs(pending)>0.004?`כולל ${money(pending)} ממתינות`:'',partialReason].filter(Boolean).join(' · ');
  return `<div class="credit-live-balance ${partial?'unavailable':''}"><span>חיוב קרוב${partial?' · חלקי':''}${meta?`<small>${esc(meta)}</small>`:''}</span><b>${amount}</b></div>`;
}
function renderSyncedAccounts(summary){
  ...
  const cards=cardModels.map(({p,a,mapping,account,name,creditAccountKey})=>{const pending=a.txns.filter(tx=>tx.status==='pending').length,frameStatus=creditFrameStatus(a,mapping,summary.today),upcoming=kupaReconciledCardUpcomingChargeData(model.state,creditAccountKey,summary.today),stale=a.months.filter(month=>month.status==='stale').length,missing=a.months.filter(month=>month.status==='missing').length,coverage=stale||missing?` · ${stale} חודשי LKG · ${missing} חסרים`:' · הכיסוי החודשי רענן';return `<article class="credit-live-card included"><div><b>${esc(name)}</b><small>${esc(p.label)}${p.ownerLabel?` · ${esc(p.ownerLabel)}`:''} · ${esc(account)} · <span class="credit-card-state on">כלול בדוחות</span></small></div>${upcomingChargeMarkup(a,p.provider,summary.today,upcoming)}${creditFrameMarkup(frameStatus)}<small>${a.txns.length} תנועות${pending?` · ${pending} ממתינות`:''}${coverage}</small></article>`}).join('');
...
function creditChargeDayButtonsMarkup(action,contextKey,selectedDay){return `<span class="credit-charge-day-stack" role="group" aria-label="סינון לפי מועד חיוב"><button type="button" class="credit-charge-day ${selectedDay==='10'?'active':''}" data-action="${action}" ${contextKey?`data-click-arg0="${esc(contextKey)}" data-click-arg1="10"`:'data-click-arg0="10"'} aria-pressed="${selectedDay==='10'}">10</button><button type="button" class="credit-charge-day ${selectedDay==='15'?'active':''}" data-action="${action}" ${contextKey?`data-click-arg0="${esc(contextKey)}" data-click-arg1="15"`:'data-click-arg0="15"'} aria-pressed="${selectedDay==='15'}">15</button></span>`}
function creditUpcomingSelectorMarkup(active,selectedDay,total){return `<span class="credit-cycle-selector credit-cycle-selector-header ${active?'active':''}"><button type="button" class="credit-cycle-main credit-upcoming-toggle ${active?'active':''}" data-action="credit-detail-upcoming" aria-pressed="${active?'true':'false'}"><span>החיוב הקרוב</span><b>${money(total)}</b></button>${creditChargeDayButtonsMarkup('credit-detail-upcoming-day','',active?selectedDay:'all')}</span>`}
function creditHistorySelectorsMarkup(months,selectedKey='',selectedDay='all'){if(!months.length)return '';return `<span class="credit-detail-history-selectors" role="group" aria-label="חיובים מחודשים קודמים">${months.map(month=>{const active=month.key===selectedKey;return `<span class="credit-cycle-selector credit-cycle-selector-header credit-cycle-selector-history ${active?'active':''} past"><button type="button" class="credit-cycle-main credit-detail-month-tab" data-action="credit-detail-month" data-click-arg0="${esc(month.key)}"><span>${esc(monthLabel(month.key))}</span><b>${money(month.total)}${month.partial?' · חלקי':''}</b></button>${creditChargeDayButtonsMarkup('credit-detail-month-day',month.key,active?selectedDay:'all')}</span>`}).join('')}</span>`}
function creditDetailMonthTabs(months,selectedKey,selectedDay='all'){
  if(!months.length)return '<div class="credit-detail-month-empty">אין חיובים להצגה</div>';
  const current=monthKey(todayISO());
  return `<div class="credit-detail-month-tabs" role="tablist" aria-label="בחירת חודש לעסקאות ותשלומים">${months.map(month=>{const uncertain=month.key==='unassigned',timing=uncertain?'future':month.key<current?'past':month.key===current?'current':'future',label=uncertain?'מחזור לא ודאי':monthLabel(month.key),active=month.key===selectedKey;return `<span class="credit-cycle-selector ${active?'active':''} ${timing}"><button type="button" role="tab" aria-selected="${active?'true':'false'}" class="credit-cycle-main credit-detail-month-tab" data-action="credit-detail-month" data-click-arg0="${esc(month.key)}"><span>${esc(label)}</span><b>${uncertain?esc(month.items.length)+' עסקאות':`${money(month.total)}${month.partial?' · חלקי':''}`}</b></button>${creditChargeDayButtonsMarkup('credit-detail-month-day',month.key,active?selectedDay:'all')}</span>`}).join('')}</div>`;
}
function creditMonthlyDetailRow(item){if(item.source==='manual')return manualMonthlyCreditRow(item);if(item.source==='credit_pending')return pendingMonthlyCreditRow(item);if(item.source==='credit_foreign')return foreignMonthlyCreditRow(item);if(item.source==='credit_unknown')return unknownMonthlyCreditRow(item);return syncedMonthlyCreditRow(item)}
...
function transactionDateCell(value,time=''){if(!value)return '<span class="muted">לא נמסר</span>';const clock=/^(?:[01]\d|2[0-3]):[0-5]\d$/.test(String(time||''))?String(time):'';return `${dateFmt(String(value).slice(0,10))}${clock?` <small class="credit-transaction-time">${esc(clock)}</small>`:''}`}
...
function billingDateEstimateMarkup(item){if(['authoritative','manual'].includes(String(item.billingDateConfidence||'')))return '';const labels={known_billing_cycle:'מועד חיוב שהושלם ממחזור ידוע',issuer_next_charge_day:'משוער לפי יום החיוב הבא מהחברה',inferred_billing_day:'משוער לפי יום החיוב ההיסטורי',bank_settlement_next_known_cycle:'הועבר למחזור הידוע הבא לאחר פירעון בבנק',unassigned_after_bank_settlement:'המחזור הקודם נפרע בבנק · אין מועד אמין למחזור הבא',unassigned:'מחזור לא ודאי · אין מועד חיוב מספיק'};return `<small class="credit-pending-estimate">${esc(labels[item.chargeDateSource]||'מועד חיוב משוער')}</small>`}
...
function creditDescriptionCell(item){
  const fx=item.foreignCurrency===true?'<span class="credit-transaction-fx">מט״ח</span>':'',missing=item.amountStatus==='unknown_amount'?`<span class="credit-transaction-pending">${item.coverageIncomplete?'כיסוי חסר':'סכום חסר'}</span>`:'',coverage=item.coverageIncomplete&&item.amountStatus!=='unknown_amount'?'<span class="credit-transaction-pending">LKG/כיסוי חלקי</span>':'';
...
function syncedMonthlyCreditRow(item){
  ...
  return `<tr class="credit-synced-detail-row" data-credit-search-id="${esc(creditDetailItemIdentity(item))}">${syncedBulkPlaceholder()}${creditCardDetailCell(item,CREDIT_PROVIDER_LABELS[item.provider]||'מסונכרן')}${creditDescriptionCell(item)}<td class="credit-detail-transaction-date">${transactionDateCell(item.transactionDate,item.transactionTime)}</td><td class="amount credit-detail-total">${money(series.totalAmount)}</td><td class="credit-detail-installment">${installmentCell(item)}</td><td class="credit-detail-charge"><b>${esc(chargeDate)}</b>${chargeAmount}${billingDateEstimateMarkup(item)}</td><td class="credit-detail-status"><span class="badge ${esc(status.cls)}">${esc(status.label)}</span>${partial?`<div class="muted credit-detail-partial">${!item.date?'אין מועד חיוב אמין':item.coverageIncomplete?'כיסוי חברה חלקי':'אופק חלקי'}</div>`:''}</td><td class="credit-detail-actions"></td></tr>`;
}
...
function pendingMonthlyCreditRow(item){
  const estimates={issuer_next_charge:'משוער לפי מועד החיוב הבא',known_future_cycle:'משוער לפי מחזור עתידי ידוע',inferred_billing_day:'משוער לפי יום החיוב ההיסטורי',bank_settlement_next_known_cycle:'הועבר למחזור הידוע הבא לאחר פירעון בבנק',unassigned_after_bank_settlement:'המחזור הקודם נפרע בבנק · אין מועד אמין למחזור הבא',unassigned:'מחזור לא ודאי · אין מועד חיוב מספיק'},estimate=estimates[item.chargeDateSource]||'שיוך זמני עד אישור סופי',stale=item.pendingFresh===false,chargeDate=item.date?dateFmt(item.date):'לא נקבע';
  return `<tr class="credit-synced-detail-row credit-pending-detail-row${stale?' stale':''}" data-credit-search-id="${esc(creditDetailItemIdentity(item))}">${syncedBulkPlaceholder()}${creditCardDetailCell(item,CREDIT_PROVIDER_LABELS[item.provider]||'מסונכרן')}${creditDescriptionCell(item)}<td class="credit-detail-transaction-date">${transactionDateCell(item.transactionDate,item.transactionTime)}</td><td class="amount credit-detail-total">${pendingAmountMarkup(item)}</td><td class="credit-detail-installment"><span class="muted">אישור</span></td><td class="credit-detail-charge"><b>${esc(chargeDate)}</b>${item.includedInIlsTotal?`<div class="amount credit-month-charge-amount">${money(item.amount)}</div>`:'<div class="muted credit-month-charge-amount">לא נכלל בסה״כ ₪</div>'}<small class="credit-pending-estimate">${esc(estimate)}</small></td><td class="credit-detail-status"><span class="credit-transaction-pending">ממתינה</span>${stale?'<div class="muted credit-detail-partial">נתון קודם</div>':''}</td><td class="credit-detail-actions"></td></tr>`;
}
function foreignMonthlyCreditRow(item){
  const status=item.date?chargeStatus(item.date):{label:'מחזור לא ודאי',cls:'orange'},formatted=new Intl.NumberFormat('he-IL',{maximumFractionDigits:2}).format(Number(item.displayAmount)||0),display=`${formatted} ${esc(item.displayCurrency||item.originalCurrency||'')}`,chargeDate=item.date?dateFmt(item.date):'לא נקבע';
  return `<tr class="credit-synced-detail-row credit-foreign-detail-row" data-credit-search-id="${esc(creditDetailItemIdentity(item))}">${syncedBulkPlaceholder()}${creditCardDetailCell(item,CREDIT_PROVIDER_LABELS[item.provider]||'מסונכרן')}${creditDescriptionCell(item)}<td class="credit-detail-transaction-date">${transactionDateCell(item.transactionDate,item.transactionTime)}</td><td class="amount credit-detail-total">${display}</td><td class="credit-detail-installment">${installmentCell(item)}</td><td class="credit-detail-charge"><b>${esc(chargeDate)}</b><div class="muted credit-month-charge-amount">לא נכלל בסה״כ ₪</div>${billingDateEstimateMarkup(item)}</td><td class="credit-detail-status"><span class="badge ${esc(status.cls)}">${esc(status.label)}</span></td><td class="credit-detail-actions"></td></tr>`;
}
function unknownMonthlyCreditRow(item){
  const status=item.date?chargeStatus(item.date):{label:'מחזור לא ודאי',cls:'orange'},chargeDate=item.date?dateFmt(item.date):'לא נקבע';
  return `<tr class="credit-synced-detail-row credit-pending-detail-row" data-credit-search-id="${esc(creditDetailItemIdentity(item))}">${syncedBulkPlaceholder()}${creditCardDetailCell(item,CREDIT_PROVIDER_LABELS[item.provider]||'מסונכרן')}${creditDescriptionCell(item)}<td class="credit-detail-transaction-date">${transactionDateCell(item.transactionDate,item.transactionTime)}</td><td class="amount credit-detail-total">${item.originalAmount===null?'—':item.foreignCurrency?`${new Intl.NumberFormat('he-IL',{maximumFractionDigits:2}).format(item.originalAmount)} ${esc(item.originalCurrency)}`:money(item.originalAmount)}</td><td class="credit-detail-installment">${installmentCell(item)}</td><td class="credit-detail-charge"><b>${esc(chargeDate)}</b><div class="muted credit-month-charge-amount">${item.coverageIncomplete?'נתוני החברה לחודש אינם מלאים':'הסכום השקלי טרם נמסר'}</div>${billingDateEstimateMarkup(item)}</td><td class="credit-detail-status"><span class="badge ${esc(status.cls)}">${esc(status.label)}</span>${item.coverageIncomplete?'<div class="muted credit-detail-partial">כיסוי LKG/חסר</div>':''}</td><td class="credit-detail-actions"></td></tr>`;
}
function manualMonthlyCreditRow(item){
  const record=item.record,status=chargeStatus(item.date);
  return `<tr data-bulk-collection="credits" data-bulk-id="${esc(record.id)}" data-credit-search-id="${esc(creditDetailItemIdentity(item))}" class="${esc(ui.bulkSelected.has(record.id)?'bulk-selected-row':'')}">${bulkCell('credits',record.id)}${creditCardDetailCell(item,'תוספת ידנית')}${creditDescriptionCell(item)}<td class="credit-detail-transaction-date">${transactionDateCell(item.transactionDate)}</td><td class="amount credit-detail-total">${money(record.totalAmount)}</td><td class="credit-detail-installment">${installmentCell(item)}</td><td class="credit-detail-charge"><b>${dateFmt(item.date)}</b><div class="amount credit-month-charge-amount">${money(item.amount)}</div></td><td class="credit-detail-status"><span class="badge ${esc(status.cls)}">${esc(status.label)}</span></td><td class="credit-detail-actions"><button class="iconbtn" data-action="open-credit-modal-2" data-click-arg0="${esc(record.id)}">עריכה</button></td></tr>`;
}
...
function creditMonthRow(m,max){
  ...
  for(const row of m.inst){
    ...
    const key=row.hidden?'hidden':(cardKey||`${card}\u0000${account}\u0000${owner}`),existing=groups.get(key)||{card,account,owner,cardKey,billingDate:row.date,total:0,pendingCount:0,incompleteCount:0,missingAmountCount:0,coverageGapCount:0,bankPendingSettlementDetected:false};
    if(row.date&&(!existing.billingDate||row.date<existing.billingDate))existing.billingDate=row.date;existing.total+=row.amount;if(row.status==='pending')existing.pendingCount++;if(row.amountStatus!=='known_ils')existing.missingAmountCount++;if(row.coverageIncomplete)existing.coverageGapCount++;if(!row.includedInIlsTotal||row.coverageIncomplete)existing.incompleteCount++;if(row.bankPendingSettlementDetected)existing.bankPendingSettlementDetected=true;groups.set(key,existing);
  }
  ...
  const detail=[...groups.values()].sort((a,b)=>String(a.card).localeCompare(String(b.card),'he')).map(item=>{
    const content=`<span><b>${esc(item.card)}</b>${item.account||item.pendingCount||item.incompleteCount?`<small>${item.account?esc(item.account):''}${item.owner?` · ${esc(item.owner)}`:''}${item.billingDate?` · תאריך החיוב ${esc(dateFmt(item.billingDate))}`:''}${item.pendingCount?` · ${esc(item.pendingCount)} ממתינות`:''}${item.missingAmountCount?` · ${esc(item.missingAmountCount)} ללא סכום ₪`:''}${item.coverageGapCount?` · ${esc(item.coverageGapCount)} בכיסוי LKG/חסר`:''}</small>`:''}${item.bankPendingSettlementDetected?'<small class="credit-bank-pending-note">החיוב זוהה בבנק · ממתין לרישום סופי</small>':''}</span><strong>${money(item.total)}${item.incompleteCount?' · חלקי':''}</strong>`;
...
return {renderCredit,setCreditSearch,toggleCreditSyncOptions,creditMonthCard};
}
