...
function renderCredit(){
  ...
  document.getElementById('content').innerHTML=`
    ...
    <section class="section credit-detail-section" style="margin-top:16px"><div class="section-head"><div><h3>עסקאות ותשלומים פעילים</h3><p>מוצגות כאן רק עסקאות שנותר בהן חיוב עתידי. עסקאות שהסתיימו אינן מעמיסות על הפירוט הרגיל ונשמרות בנפרד בהיסטוריה.</p></div></div><div style="overflow:auto"><table><thead><tr>${bulkHeader('credits')}<th>כרטיס</th><th>תיאור</th><th>סכום כולל</th><th>התקדמות</th><th>תשלום הבא</th><th>יתרה עתידית</th><th>מצב</th><th></th></tr></thead><tbody>${detailItems.length?detailItems.map(item=>item.source==='manual'?manualCreditRow(item.record):syncedCreditRow(item.series)).join(''):`<tr><td colspan="9"><div class="empty compact">אין עסקאות פעילות להצגה במסנן הנוכחי.</div></td></tr>`}</tbody></table></div></section>
...
function renderSyncedAccounts(summary,syncedFuture){
  ...
  const cards=cardModels.map(({p,a,included,account,name})=>{const pending=a.txns.filter(tx=>tx.status==='pending').length;return `<article class="credit-live-card ${included?'included':'excluded'}"><div><b>${esc(name)}</b><small>${esc(p.label)}${p.ownerLabel?` · ${esc(p.ownerLabel)}`:''} · ${esc(account)} · <span class="credit-card-state ${included?'on':'off'}">${included?'כלול בחישוב':'לא כלול בחישוב'}</span></small></div><div class="credit-live-balance"><span>${a.balance===null?'יתרה/חיוב קרוב לא זמין':'חיוב/יתרה שהחברה החזירה'}</span><b>${a.balance===null?'—':money(Math.abs(a.balance))}</b></div>${creditFrameMarkup(a,p.provider)}<small>${a.txns.length} תנועות${pending?` · ${pending} ממתינות`:''}${a.balanceDate?` · נכון ל־${dateFmt(String(a.balanceDate).slice(0,10))}`:''}</small></article>`}).join('');
