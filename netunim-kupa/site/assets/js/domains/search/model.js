...
export function searchKupaGlobalEntries(entries,query,{limitPerGroup=60}={}){const normalized=normalizeKupaSearchText(query),queryCompact=compact(query),tokens=normalized.split(' ').filter(Boolean);if(!normalized)return {query:'',total:0,groups:GROUPS.map(g=>({...g,total:0,items:[]}))};const matches=(Array.isArray(entries)?entries:[]).filter(x=>matchEntry(x,queryCompact,tokens)).map((x,index)=>({...x,_rank:rank(x,normalized),_index:index}));const groups=GROUPS.map(g=>{const items=matches.filter(x=>x.group===g.key).sort((a,b)=>a._rank-b._rank||String(a.context||'').localeCompare(String(b.context||''),'he')||a._index-b._index);return {...g,total:items.length,items:items.slice(0,Math.max(1,limitPerGroup)).map(({_rank,_index,...item})=>item)}});return {query:normalized,total:matches.length,groups}}
export function searchKupaGlobalData(state,query,options={}){return searchKupaGlobalEntries(buildKupaGlobalSearchEntries(state),query,options)}

export const KUPA_SEARCH_FRAGMENTS=Object.freeze({checks:['checks'],credit:['credits','creditSync','bank','bankFeed'],expenses:['expenses'],cash:['cash','rights'],bank:['bankFeed'],notes:['notes']});
export function buildKupaSearchFragment(state,name){const builders={checks:checkEntries,credit:creditEntries,expenses:expenseEntries,cash:cashEntries,bank:bankEntries,notes:noteEntries};return builders[name](state).filter(row=>row.id)}
