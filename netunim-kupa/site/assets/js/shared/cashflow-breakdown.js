...
export function cashflowBreakdownMarkup(cashflow){
  ...
  const expenses=(cashflow.expenseRows||[]).map(row=>({label:row.name||row.description||row.note||'הוצאה',date:row.dueDate,amount:-row.amount}));
