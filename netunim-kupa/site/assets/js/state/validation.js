...
function validNotesSheet(value){if(value===undefined)return true;if(!value||typeof value!=='object'||Array.isArray(value)||!Array.isArray(value.columns)||!value.columns.length||!Array.isArray(value.rows))return false;const workbook=Number(value.version)>=2;if(workbook){if(!Array.isArray(value.sheets)||!value.sheets.length||!value.sheets.every(sheet=>sheet&&typeof sheet==='object'&&String(sheet.id||'').trim()&&String(sheet.name||'').trim()))return false;const ids=new Set(value.sheets.map(sheet=>String(sheet.id)));if(ids.size!==value.sheets.length)return false;if(value.columns.some(column=>!column||typeof column!=='object'||!String(column.id||'').trim()||!ids.has(String(column.sheetId||''))))return false;if(value.rows.some(row=>!row||typeof row!=='object'||!String(row.id||'').trim()||!ids.has(String(row.sheetId||''))||!row.cells||typeof row.cells!=='object'||Array.isArray(row.cells)))return false;return true}return value.columns.every(column=>column&&typeof column==='object'&&String(column.id||'').trim())&&value.rows.every(row=>row&&typeof row==='object'&&String(row.id||'').trim()&&row.cells&&typeof row.cells==='object'&&!Array.isArray(row.cells))}
