...
let validatedBackupDirectory=null,validatedBackupAt=0,validatedBackupUntil=0,listedBackupDirectory=null;
...
async function maybeCreateAutomaticBackup(payload){if(!files.backupsDirHandle||!payload)return null;const now=Date.now();if(validatedBackupDirectory===files.backupsDirHandle&&now>=validatedBackupAt&&now<validatedBackupUntil){queueAutomaticBackup(payload,validatedBackupUntil-now);return null}const latest=await latestAutomaticBackup();let previous=null,valid=false;if(latest){try{previous=await readJsonHandle(latest.handle);valid=true;validatedBackupDirectory=files.backupsDirHandle;validatedBackupAt=now;validatedBackupUntil=latest.lastModified+AUTO_BACKUP_INTERVAL_MS}catch(e){console.error('backup validation',e)}}if(valid&&comparableBackupPayload(previous)===comparableBackupPayload(payload)){clearPendingAutomaticBackup();return null}if(latest&&valid){const remaining=AUTO_BACKUP_INTERVAL_MS-(Date.now()-latest.lastModified);if(remaining>0){queueAutomaticBackup(payload,remaining);return null}}const name=`${AUTO_BACKUP_PREFIX}${backupTimestamp()}.json`;await writeVerifiedBackup(name,payload);validatedBackupDirectory=files.backupsDirHandle;validatedBackupAt=Date.now();validatedBackupUntil=validatedBackupAt+AUTO_BACKUP_INTERVAL_MS;await pruneAutomaticBackups();clearPendingAutomaticBackup();return name}

async function backupSnapshotToComputer(snapshot=model.state,revision=session.dbRevision){if(!files.backupsDirHandle)return null;try{const name=await maybeCreateAutomaticBackup(payloadFromState(clone(snapshot),revision));if(name||listedBackupDirectory!==files.backupsDirHandle){session.serverInfo.backups=await listBackups();listedBackupDirectory=files.backupsDirHandle}return name}catch(e){console.error('automatic local backup failed',e);return null}}
