...
export function createStorageBrowser({storageV2=null,observeStorage=()=>{},legacyDrainActive=()=>false,legacyWriteAllowed=()=>true,legacyCloudPendingExists=()=>false,legacyCloudHeadVerifiedClean=()=>false,verifyLegacyCloudPending=async()=>null,model, session, files, normalizeState, prepareKupaCloudState=state=>state, idbPut, idbGet}){
...
function persistBrowserStateSync(record){if(storageV2?.cutoverActive)throw new Error('storage_v1_write_forbidden');try{const text=stringifyStorage('browser-snapshot',record);writeVerifiedStorage(localStorage,BROWSER_STATE_KEY,text);return true}catch(e){console.error('browser state localStorage',e);return false}}
...
async function loadBrowserState(){const local=loadBrowserStateSync();let idb=null;try{idb=await idbGet('sync',BROWSER_STATE_IDB_KEY)}catch(e){console.error('browser state idb load',e)}const lt=Number(local?.snapshotSeq||0),it=Number(idb?.snapshotSeq||0);session.localSnapshotSeq=Math.max(Number(session.localSnapshotSeq||0),lt,it);const chosen=!local||it>lt?idb:local,recovered=await storageV2?.recover?.(chosen?.state||null,{snapshotSeq:Math.max(lt,it),revision:Number(chosen?.revision||0)});if(storageV2?.cutoverActive&&!recovered)throw new Error('storage_v2_cutover_recovery_required');if(recovered){session.localSnapshotSeq=Math.max(session.localSnapshotSeq,Number(recovered.appMetadata?.snapshotSeq||0));return {schemaVersion:2,snapshotSeq:session.localSnapshotSeq,state:recovered.state,revision:Number(recovered.appMetadata?.revision||chosen?.revision||0),savedAt:new Date().toISOString()}}if(chosen){persistBrowserStateSync(chosen);queueBrowserStateIdb(chosen)}return chosen||null}
...
function persistStorageV2CompatibilitySnapshot(state,revision){const record=browserStateRecord(state,revision,{skipSequence:true});record.snapshotSeq=Number(session.localSnapshotSeq||0);const ok=persistBrowserStateSync(record);queueBrowserStateIdb(record);return ok}
