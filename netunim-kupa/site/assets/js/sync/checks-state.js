...
function markSharedChecksPending(snapshot=model.state.checks,message='',conflict=undefined,progress=null){
  ...
  checksSession.sharedChecksOutboxCommitPromise.catch(()=>{});
  return cacheOk;
}
...
async function clearSharedChecksPending(acknowledgedGeneration){
  ...
  const clearing=(checksSession.sharedChecksOutboxCommitPromise||Promise.resolve()).then(async()=>{
    try{await idbDelete('sync',SHARED_CHECKS_OUTBOX_KEY);return true}catch(e){console.error('checks outbox clear',e);checksSession.sharedChecksDurabilityDegraded=true;return false}
  });
  checksSession.sharedChecksOutboxCommitPromise=clearing;if(!await clearing)return false;
  if(checksSession.sharedChecksOutboxCommitPromise!==clearing||!acknowledgedGenerationMatches(checksSession.sharedChecksOutboxCached,acknowledgedGeneration))return false;
  try{localStorage.removeItem(SHARED_CHECKS_PENDING_KEY);}catch(e){console.error('checks cache clear',e);checksSession.sharedChecksDurabilityDegraded=true;return false}
  checksSession.sharedChecksOutboxCached=null;checksSession.sharedChecksDurabilityDegraded=false;return true;
}
