...
async function mirror(){const v2=await refreshStorageV2CloudState();if(v2)await replaceStorageV2CurrentState(model.state);else persistImmediateBrowserSnapshot(model.state,session.dbRevision,{storageBoundary:'shared-checks-remote-mirror'});if(files.backupsDirHandle)await backupSnapshotToComputer(model.state,session.dbRevision)}
