...
export function createSyncChecks({checksSession={}, model, session, files, tab, persistImmediateBrowserSnapshot, persistSharedChecksBase, markSharedChecksPending, getSharedChecksPending, clearSharedChecksPending, readSharedChecksDocument, toast, render, rpcSaveSharedChecks, setSaveStatus, setCloudHeaderStatus, sharedChecksPendingExists, backupSnapshotToComputer, sharedChecksHaveLocalWork, readSharedChecksMeta, refreshCloudHeaderTimestamp, touchChecksRevision=()=>{}}){
...
async function mirror(){persistImmediateBrowserSnapshot(model.state,session.dbRevision,{storageBoundary:'shared-checks-remote-mirror'});if(files.backupsDirHandle)await backupSnapshotToComputer(model.state,session.dbRevision)}
