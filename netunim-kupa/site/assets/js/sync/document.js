...
async function persistSupabaseState(snapshot,msg,generation=session.localGeneration){
  ...
  try{
    ...
    const row=res.row,newRev=Number(row?.revision||baseRevision+1),authoritative=prepareKupaCloudState(row?.state||candidate);session.dbRevision=newRev;session.lastSavedSnapshot=JSON.stringify(authoritative);session.serverInfo.lastSavedAt=row?.updated_at||new Date().toISOString();session.cloudConflictPending=false;
...
async function cloudPoll(){
  ...
  try{session.cloudSyncBusy=true;const row=await readSupabaseDocument();if(row&&Number(row.revision||0)>session.dbRevision){const base=lastSavedCloudState(),clean=!!base&&jsonEq(prepareKupaCloudState(model.state),base);if(clean){await applyCloudRow(row);toast('התקבל עדכון ממחשב אחר')}else{stageCloudPendingLocal(prepareKupaCloudState(model.state),'שינוי מקומי ממתין',session.dbRevision,base||prepareKupaCloudState(model.state),session.localGeneration,false);session.cloudSyncBusy=false;await reconcileCloudPending(row)}}}catch(e){console.error('cloud poll',e)}finally{session.cloudSyncBusy=false;await pollSharedChecks()}
}
