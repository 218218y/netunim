...
async function persistSupabaseState(snapshot,msg,generation=session.localGeneration){
  ...
  try{
    ...
    persistImmediateBrowserSnapshot(model.state,session.dbRevision,{storageBoundary:'cloud-write-ack'});if(businessChanged)render();await backupSnapshotToComputer(model.state,session.dbRevision);if(newer&&!session.cloudConflictPending)setTimeout(cloudPoll,0);return !session.cloudConflictPending
