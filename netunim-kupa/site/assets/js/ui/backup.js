...
export function createUiBackup({model,session,ui,files,checksSession,readJsonHandle,listBackups,createManualBackup,toast,renderSettings,stateFromPayload,persistImmediateBrowserSnapshot,persistSharedChecksBase,saveState,chooseFolder,prepareKupaCloudState,readSupabaseDocument,readSharedChecksDocument,getCloudPending,getSharedChecksPending,restoreGroupStore,stageRestoreGroup,applyRestoreGroup,listIncompleteRestoreGroups,listKupaCloudBackups,readKupaCloudBackupPoint,loadSupaSession,render,modal,closeModal,confirmDialog}){
  ...
  async function refreshCloudBackups(){
    if(session.connectionMode!=='supabase'||!session.backendReady){ui.cloudBackupCatalog=[];ui.cloudBackupError='גיבויי ענן זמינים כאשר הקופה פתוחה מ־Supabase.';return renderSettings()}
    ui.cloudBackupLoading=true;ui.cloudBackupError='';renderSettings();
    try{const raw=await listKupaCloudBackups();ui.cloudBackupCatalog=buildBackupCatalog(raw.rolling,raw.periodic)}catch(error){console.error('cloud backup list',error);ui.cloudBackupCatalog=[];ui.cloudBackupError=error.message||'קריאת גיבויי הענן נכשלה'}finally{ui.cloudBackupLoading=false;renderSettings()}
  }
  ...
  return {manualBackup,downloadJsonBackup,restoreBackup,resumeIncompleteRestore,refreshCloudBackups,previewCloudBackup,downloadCloudBackup,downloadSelectedCloudBackup,applySelectedCloudBackup,switchFolder,exportCSV,downloadText};
}
