...
function restoreDeleteIntents(before,after){const out={};for(const [key,keyField] of [['credits','id'],['cash','id'],['rights','id'],['notes','id'],['expenses','id']]){const kept=new Set((after?.[key]||[]).map(x=>String(x?.[keyField]||''))),ids=(before?.[key]||[]).map(x=>String(x?.[keyField]||'')).filter(id=>id&&!kept.has(id));if(ids.length)out[key]=ids}for(const part of ['rows','columns']){const beforeRows=before?.notesSheet?.[part]||[],afterRows=after?.notesSheet?.[part]||[],kept=new Set(afterRows.map(x=>String(x?.id||''))),ids=beforeRows.map(x=>String(x?.id||'')).filter(id=>id&&!kept.has(id));if(ids.length)out[`notesSheet.${part}`]=ids}return out}
