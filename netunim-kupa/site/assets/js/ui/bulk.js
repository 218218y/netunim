...
async function deleteBulkSelected(collection){
  if(!['checks','credits','cash','rights'].includes(collection))return;
  ...
  const set=new Set(ids),operations=ids.map(id=>({type:'delete',collection,id}));model.state[collection]=model.state[collection].filter(x=>!set.has(x.id));ui.bulkSelected.clear();if(collection==='checks')saveChecksState(`${ids.length} רשומות נמחקו`,{deletedIds:ids,mutationType:'bulk-delete',surface:`kupa.bulk.${collection}`,operations});else saveState(`${ids.length} רשומות נמחקו`,{deleteIntents:{[collection]:ids},mutationType:'bulk-delete',surface:`kupa.bulk.${collection}`,domains:[collection],operations});render()
}
