...
async function openCloudUsingSavedSession({interactive=true}={}){
  ...
  try{
    ...
    const pending=await getCloudPending();if(pending){session.connectionMode='supabase';session.backendReady=true;document.getElementById('connectScreen').style.display='none';session.dbRevision=Number(row.revision||0);session.lastSavedSnapshot=JSON.stringify(prepareKupaCloudState(row.state));await reconcileCloudPending(row);checksSession.sharedChecksBase=loadSharedChecksBase();checksSession.sharedChecksBankEvents=loadSharedChecksBankEvents();await syncSharedChecksFromCloud({quiet:true,required:true});render();startCloudPolling();return true}
...
async function connectSupabaseFromLogin(mode){
  ...
  try{
    ...
    const row=await readSupabaseDocument();closeModal();if(!row){await showCloudNoDocument();return}const pending=await getCloudPending();if(pending){session.connectionMode='supabase';session.backendReady=true;session.dbRevision=Number(row.revision||0);session.lastSavedSnapshot=JSON.stringify(prepareKupaCloudState(row.state));document.getElementById('connectScreen').style.display='none';await reconcileCloudPending(row);checksSession.sharedChecksBase=loadSharedChecksBase();checksSession.sharedChecksBankEvents=loadSharedChecksBankEvents();await syncSharedChecksFromCloud({quiet:true,required:true});render();startCloudPolling()}else await applyCloudRow(row)
  }catch(e){
    console.error(e);
