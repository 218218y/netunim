-- Reconcile a Hapoalim pending cheque deposit with its completed row when the bank
-- changes only the deposit presentation (for example direct deposit -> machine deposit).
-- This supersedes the merge RPC body only; historical migrations remain immutable.
-- Matching stays fail-closed: exact amount + balance + activity, semantic cheque-deposit
-- compatibility, the existing +/-3 day window, and exactly one pending candidate are required.
