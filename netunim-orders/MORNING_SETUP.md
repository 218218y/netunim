...
התקבול הראשון במסמך בנקאי נעול להעברה הבנקאית המקורית: סוג 4, סכום זהה לתנועה ואסמכתא תואמת כאשר קיימת. אפשר להוסיף עד 11 תקבולים נוספים; סכום מסמך 320/400 נגזר מסך כל התקבולים ונבדק שוב ב־Edge Function באגורות.
...
קישור תנועה↔מסמך נכתב ל־`bank_transaction_document_links` רק לאחר שהמסמך נקרא חזרה מ־Morning ואומת כ־`created`. רק אז התנועה מסומנת אוטומטית כמטופלת. אם הקישור המקומי נכשל לאחר שהמסמך כבר אומת, ההפקה נשארת נעולה ל־reconciliation ואינה שולחת POST נוסף.
