...
## הפקת מסמך מתנועת בנק

בחשבון העסקי בלבד, תנועת זכות סופית (`completed`) בשקלים שקיבלה מזהה ארכיון יציב יכולה לפתוח הפקת `חשבונית מס / קבלה` (320) או `קבלה` (400). תנועה `pending`, תנועת חובה, חשבון ביתי או מטבע שאינו ILS אינם ניתנים להפקה במסלול הזה. גם סימון ידני כ־"מטופל" מותר רק לאחר שהתנועה סופית; ה־RPC בצד השרת בודק בעלות, חשבון עסקי ו־`completed`, כך שההגנה אינה תלויה רק ב־UI.

במסמך שמקורו בבנק, תקבולי המקור מסומנים ונבדקים מול התנועה המקורית: בהעברה בנקאית נשמר תקבול סוג 4 בסכום התנועה ובאסמכתא התואמת כאשר קיימת; בהפקדת צ׳קים כל צ׳ק מובנה מהבנק נשמר כתקבול סוג 2 נפרד, ובהפקדה מרובה ניתן להסיר צ׳קים שאינם שייכים למסמך. ניתן להוסיף תקבולים ידניים עד 12 תקבולים בסך הכול; סכום מסמך 320/400 נגזר מסך כל התקבולים ונבדק שוב ב־Edge Function באגורות.

קישור לחוב הוא אופציונלי ומקומי בלבד. הממשק מציע רק חובות שעדיין לא הושלמו, לפי התאמת שם, ומחזק את הדירוג כאשר סכום הזיכוי תואם ליתרת התשלום שעדיין פתוחה; הוא אינו בוחר חוב בעצמו. רק בחירת המשתמש מפעילה את מנגנון `debtProgress` הקיים. זהות החוב אינה נשמרת ב־Morning ledger ואינה נכנסת ל־fingerprint של המסמך, כדי שמחיקת חוב או שינוי מקומי לא יחלישו idempotency של ההפקה.

קישור תנועה↔מסמך נכתב ל־`bank_transaction_document_links` רק לאחר שהמסמך נקרא חזרה מ־Morning ואומת כ־`created`. בהעברה בנקאית ובצ׳ק יחיד התנועה יכולה להיסמן אז אוטומטית כמטופלת. בהפקדה מרובת־צ׳קים הקישור נשמר אך `handled_at` אינו נקבע אוטומטית, משום שמסמך אחד יכול לייצג רק חלק מהצ׳קים; הסימון נשאר ידני עד שהמשתמש מחליט שכל ההפקדה טופלה. אם הקישור המקומי נכשל לאחר שהמסמך כבר אומת, ההפקה נשארת נעולה ל־reconciliation ואינה שולחת POST נוסף.

## PCN / ריכוז הכנסות - L

ה־payload של מסמכי ההכנסה באתר אינו שולח כרגע `accountingClassification` ואינו מקבע בקוד את האות `L`. ההגדרה החשבונאית בחשבון Morning נשארת מקור האמת. אין להוסיף לקוד `L`, קוד סיווג או UUID בלי חוזה API רשמי שמוכיח שהשדה נתמך במסמכי הכנסה ומהו המזהה הנכון לחשבון.

לפני Production יש לבדוק ב־Morning Sandbox מסמך 320 ומסמך 400 שנוצרו דרך האתר, ולוודא בתוך Morning שהמסמכים מקבלים את הגדרת ה־PCN הרצויה של העסק (`ריכוז הכנסות - L`). אם בדיקת read-back/API רשמית בעתיד תוכיח שצריך לשלוח סיווג מפורש במסמך הכנסה, יש למפות אותו לפי מזהה הסיווג שהחשבון מחזיר — לא hard-code של האות בלבד.


### שדות אמצעי תשלום והשלמה מתנועת בנק

בטופס תקבול מוצגים רק השדות הרלוונטיים לאמצעי התשלום: מזומן — תאריך וסכום; העברה בנקאית — אסמכתא, בנק, סניף וחשבון; צ׳ק — בנק, סניף, חשבון ומספר צ׳ק; כרטיס אשראי — סוג כרטיס וארבע ספרות אחרונות. בשדה ארבע הספרות אפשר להזין ארבע ספרות או `****`; אם הוא נשאר ריק הוא מנורמל אוטומטית ל־`****`. פתיחת מסמך מתוך חוב שיש בו אישור סליקה בוחרת כרטיס אשראי כברירת המחדל לתקבול. רשימת הבנקים המקומית מבוססת על קודי הזיהוי של בנק ישראל, אך שדה הבנק נשאר גם שדה טקסט כדי לא לחסום ערך ידני.

כאשר המקור הוא תנועת בנק, העברה ממלאת את `bankReference` כאסמכתא ומנסה להשלים בנק/סניף/חשבון רק מנתונים מפורשים בתנועה. בהפקדת צ׳קים עם `checkItems` כל צ׳ק נפתח כתקבול נפרד עם הבנק, הסניף, החשבון, מספר הצ׳ק והסכום שהבנק סיפק. בהפקדה מרובה ניתן להסיר צ׳ק שאינו שייך למסמך; השרת מאמת שהצ׳קים שנשארו קיימים בפירוט המקורי ושהפירוט המלא תואם לסכום תנועת הבנק.
...
שלב ב׳ של ה־runtime probe יבוצע רק לאחר השלמת קבלת שלב א׳. בשדרוג הנוכחי מסמך שמופק מתנועת בנק יכול להיקשר, רק לאחר בחירת המשתמש, לאותו מנגנון `debtProgress` שמשמש הפקה מתוך חוב; אין מנגנון חוב מקביל לבנק.
