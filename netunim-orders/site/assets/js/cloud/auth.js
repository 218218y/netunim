...
export function createCloudAuth({session={},assertSessionOwner=()=>true}){
...
async function authPassword(email,password){const r=await fetch(`${SUPA_CONFIG.url}/auth/v1/token?grant_type=password`,{method:'POST',headers:supaHeaders(),body:JSON.stringify({email,password})});const j=await r.json().catch(()=>({}));if(!r.ok)throw new Error(j.error_description||j.msg||j.message||'התחברות נכשלה');j.expires_at=Math.floor(Date.now()/1000)+Number(j.expires_in||3600);assertSessionOwner(j);saveSession(j);localStorage.setItem(CLOUD_EMAIL_KEY,email);return j}

async function authPasswordForLocalReset(email,password){if(!supaConfigured())throw new Error('הגדרת Supabase חסרה');const r=await fetch(`${SUPA_CONFIG.url}/auth/v1/token?grant_type=password`,{method:'POST',headers:supaHeaders(),body:JSON.stringify({email,password})});const j=await r.json().catch(()=>({}));if(!r.ok)throw new Error(j.error_description||j.msg||j.message||'התחברות נכשלה');if(!String(j?.access_token||'').trim())throw new Error('Supabase לא החזיר token תקין לאימות האיפוס');j.expires_at=Math.floor(Date.now()/1000)+Number(j.expires_in||3600);return j}
