...
export function createDomainsBankCache({checksSession, ui, computeKupaNetReadout, renderKupa=()=>{}, renderChecks, renderSummary, loadSession, readKupaReadOnlyCloud, readKupaReadOnlyMeta, refreshAlertCenter=()=>{}, refreshBankAlertArchive=async()=>false, touchFinanceRevision=()=>{}}){
...
function acceptKupaCloudRow(row,{renderIfChanged=false}={}){if(!row)return false;const previousRevision=Number(checksSession.kupaReadRevision||0),previousFinanceRevision=Number(checksSession.financeReadRevision||0),hadState=!!checksSession.kupaCloudReadState,nextRevision=Number(row.revision||0),nextFinanceRevision=Number(row.financeRevision||0),changed=!hadState||nextRevision!==previousRevision||nextFinanceRevision!==previousFinanceRevision;checksSession.kupaReadRevision=nextRevision;checksSession.financeReadRevision=nextFinanceRevision;checksSession.financeReadUpdatedAt=row.financeUpdatedAt||checksSession.financeReadUpdatedAt||null;if(changed)touchFinanceRevision();rememberKupaNetState(row.state||{});refreshAlertCenter();Promise.resolve(refreshBankAlertArchive()).then(archiveChanged=>{refreshAlertCenter();if(archiveChanged&&ui.currentView==='checks')renderChecks()}).catch(error=>console.error('orders bank alert archive refresh',error));if(renderIfChanged&&changed)renderKupaDependentView();return true}
