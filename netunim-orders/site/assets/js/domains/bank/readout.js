...
export function kupaSyncedInstallments(kupa){
  ...
  for(const profile of Array.isArray(sync.profiles)?sync.profiles:[])for(const account of Array.isArray(profile?.accounts)?profile.accounts:[]){
    ...
    const card=mapping.cardName||[CREDIT_PROVIDER_LABELS[profile.provider]||profile.label||'כרטיס אשראי',account.accountNumber?`••${String(account.accountNumber).slice(-4)}`:''].filter(Boolean).join(' ');
    for(const [index,tx] of (Array.isArray(account.txns)?account.txns:[]).entries()){
      ...
      rows.push({creditId:`SYNC:${identity}`,creditAccountKey:synchronizedCardKey(profile,account),date,amount,part,totalParts,card,source:'credit_sync'});
    }
  }
  return rows;
}
