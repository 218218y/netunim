...
import {checkMonthKey, checkMonthLabel, checkDateFmt} from '../../core/dates.js';
...
function toggleChecksBulkMode(){ui.checksBulkMode=!ui.checksBulkMode;ui.checksBulkSelected.clear();renderChecks()}
...
function checkRow(c){return `<tr data-check-id="${esc(c.id)}" class="${esc(ui.checksBulkSelected.has(c.id)?'checks-bulk-selected':'')}">${checksBulkCell(c.id)}<td><b>${esc(c.name)}</b></td><td class="checks-amount">${money(c.amount)}</td><td>${checkDateFmt(c.dueDate)}</td><td>${checkBadgeStatus(c.status)}</td><td>${checkDueBadge(c)}</td><td>${esc(c.checkNumber)||'—'}</td><td class="checks-muted">${esc(c.note)||'—'}</td><td><div class="checks-row-actions">${c.status==='בקופה'?`<button class="checks-iconbtn" data-action="mark-check-deposited" data-click-arg0="${esc(c.id)}">הופקד</button>`:''}${c.status==='הופקד - במעקב'?`<button class="checks-iconbtn" data-action="mark-check-cleared" data-click-arg0="${esc(c.id)}">נפרע</button>`:''}<button class="checks-iconbtn" data-action="open-check-modal-2" data-click-arg0="${esc(c.id)}">עריכה</button></div></td></tr>`}

return { visibleChecks, visibleChecksTotal, checkBadgeStatus, checkDueBadge, checksCloudLabel, toggleChecksBulkMode, toggleChecksBulkRow, checksVisibleIds, toggleChecksBulkVisible, syncChecksBulkUi, checksBulkControls, checksBulkHeader, checksBulkCell, renderChecks, renderChecksSearch, renderCheckGroups, checkRow };
}
