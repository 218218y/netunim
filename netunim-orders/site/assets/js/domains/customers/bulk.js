...
function customerBottomSummary(st){
  ...
  return `<section class="customer-bottom-summary"><div class="customer-bottom-head"><div><h2>סיכום לקוחות</h2><p>סיכום הנתונים מופיע בסוף אזור הטבלה ונגלל יחד איתו.</p></div></div><div class="customer-bottom-grid"><div><span>חוב פתוח</span><b class="badtext">${money(st.openTotal)}</b><small>${esc(st.open)} לקוחות</small></div><div><span>שולם · חסרה חשבונית</span><b class="warntext">${esc(st.missingInvoice)}</b><small>חשבונית עדיין לא נסגרה</small></div><div><span>נסגרו</span><b class="goodtext">${esc(st.closed)}</b><small>תשלום + חשבונית</small></div><div><span>מעקב הזמנות</span><b>${esc(st.trackedOrders)}</b><small>רשומות במעקב</small></div></div>${sourceNote}</section>`;
}
