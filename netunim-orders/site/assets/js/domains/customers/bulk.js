...
async function deleteSelectedCustomerRows(){
  const source=customerBulkSource(),valid=new Set(source.map(x=>x.id)),ids=[...customerUi.customerBulkSelected].filter(id=>valid.has(id));if(!ids.length)return toast('לא נבחרו שורות למחיקה');
  const collection=customerUi.customerTab==='debts'?'customerDebts':'customerOrders',label=collection==='customerDebts'?'חובות לקוחות':'רשומות מעקב הזמנות';
  ...
  if(blocked())return;
  const set=new Set(ids);model.state[collection]=model.state[collection].filter(x=>!set.has(x.id));customerUi.customerBulkSelected.clear();customerUi.customerBulkAnchorId=null;scheduleSave(`${ids.length} ${label} נמחקו`,{deleteIntents:{[collection]:ids},mutationType:'bulk-delete',surface:`orders.bulk.${collection}`,domains:[collection],operations:ids.map(id=>({type:'delete',collection,id}))});renderCustomers();
}
