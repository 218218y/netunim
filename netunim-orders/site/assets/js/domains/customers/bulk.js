...
export function createDomainsCustomersBulk({customerUi, model, renderCustomers, toast, scheduleSave, confirmDialog, onTabChange=()=>{}}){
...
async function deleteSelectedCustomerRows(){const source=customerBulkSource(),valid=new Set(source.map(x=>x.id)),ids=[...customerUi.customerBulkSelected].filter(id=>valid.has(id));if(!ids.length)return toast('לא נבחרו שורות למחיקה');const label=customerUi.customerTab==='debts'?'חובות לקוחות':'רשומות מעקב הזמנות';if(!await confirmDialog('מחיקת רשומות',`למחוק ${ids.length} ${label} שנבחרו?\n\nהמחיקה תישמר בגיבוי ובסנכרון כמו כל שינוי אחר.`,{confirmText:'מחק נבחרים'}))return;const collection=customerUi.customerTab==='debts'?'customerDebts':'customerOrders',set=new Set(ids);model.state[collection]=model.state[collection].filter(x=>!set.has(x.id));customerUi.customerBulkSelected.clear();scheduleSave(`${ids.length} ${label} נמחקו`,{deleteIntents:{[collection]:ids},mutationType:'bulk-delete',surface:`orders.bulk.${collection}`});renderCustomers()}
