...
function customerBottomSummary(st){
  ...
  return `<section class="customer-bottom-summary"><div class="customer-bottom-head"><div><h2>סיכום לקוחות</h2></div></div><div class="customer-bottom-grid"><div><span>חוב שסופק</span><b class="badtext">${money(st.openSuppliedTotal)}</b><small>${esc(st.openSupplied)} חובות פתוחים שסופקו</small></div><div><span>חוב שלא סופק</span><b class="warntext">${money(st.openUnsuppliedTotal)}</b><small>${esc(st.openUnsupplied)} חובות פתוחים שטרם סופקו</small></div><div><span>סה״כ חוב פתוח</span><b class="badtext">${money(st.openTotal)}</b><small>${esc(st.open)} חובות שטרם שולמו</small></div></div>${sourceNote}</section>`;
}
