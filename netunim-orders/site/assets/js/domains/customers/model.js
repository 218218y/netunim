...
export function customerStatsData(state){
  const rows=state.customerDebts||[],openRows=rows.filter(customerDebtIsOutstanding),openSuppliedRows=openRows.filter(d=>d.supplied===true),openUnsuppliedRows=openRows.filter(d=>d.supplied!==true),sum=items=>items.reduce((total,d)=>total+Number(d.amount||0),0),openSummary=ordersOpenCustomerDebtSummaryData(state);
  return{openTotal:openSummary.openTotal,openSuppliedTotal:sum(openSuppliedRows),openUnsuppliedTotal:sum(openUnsuppliedRows),allTotal:sum(rows),open:openSummary.open,openSupplied:openSuppliedRows.length,openUnsupplied:openUnsuppliedRows.length,missingInvoice:rows.filter(d=>d.paid&&!d.invoiceIssued).length,closed:rows.filter(d=>d.paid&&d.invoiceIssued).length,trackedOrders:(state.customerOrders||[]).length}
}
