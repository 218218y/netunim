...
function paymentRowMarkup(index,payment={},source=getSource()){
  ...
  return `<div class="morning-payment-row ${bankLocked?'bank-source':''}" data-payment-row="${index}" data-bank-source="${bankLocked?'true':'false'}">
    ...
    <div class="form-grid">
      ...
      <div class="field"><label>סכום תקבול</label><input class="number-input" data-payment-field="price" data-input="morning-payment-amount" type="number" min="0.01" step="1" value="${esc(priceValue)}" ${bankLocked?'readonly':''} placeholder="0.00"></div>
