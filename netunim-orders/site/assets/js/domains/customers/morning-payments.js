...
function paymentRowMarkup(index,payment={},source=getSource()){
  ...
  return `<div class="morning-payment-row ${bankSource?'bank-source':''}" data-payment-row="${index}" data-bank-source="${bankSource?'true':'false'}" data-bank-source-key="${esc(payment.bankSourceKey||'')}" data-bank-removable="${removable?'true':'false'}">
    ...
    <div class="form-grid morning-payment-fields">
      <div class="field morning-payment-type-field"><label>אמצעי תשלום</label><select data-payment-field="type" data-change="morning-payment-type" data-change-arg0="${index}" ${bankLocked?'disabled':''}>${Object.entries(PAYMENT_TYPES).map(([value,label])=>`<option value="${value}" ${Number(value)===type?'selected':''}>${esc(label)}</option>`).join('')}</select></div>
      <div class="field morning-payment-price-field"><label>סכום תקבול</label><input class="number-input" data-payment-field="price" data-input="morning-payment-amount" type="number" min="0" step="1" value="${esc(priceValue)}" ${bankLocked?'readonly':''} placeholder="0.00"></div>
      <div class="field morning-payment-date-field"><label>תאריך</label>${dateEditorMarkup(`morningPaymentDate${index}`,date,{label:'תאריך תשלום'})}</div>
      <div class="field morning-bank-fields morning-bank-name-field" data-payment-kinds="2,4"><label>בנק <small>(שם או קוד)</small></label><input data-payment-field="bankName" data-change="morning-payment-bank" data-change-arg0="${index}" list="morningBankDirectory" autocomplete="off" maxlength="120" value="${esc(bankValue)}" ${bankDetailsLocked?'readonly':''} placeholder="למשל 12 או הפועלים"></div>
      <div class="field morning-bank-fields morning-bank-branch-field" data-payment-kinds="2,4"><label>סניף</label><input data-payment-field="bankBranch" inputmode="numeric" maxlength="30" value="${esc(payment.bankBranch||'')}" ${bankDetailsLocked?'readonly':''}></div>
      <div class="field morning-bank-fields morning-bank-account-field" data-payment-kinds="2,4"><label>חשבון</label><input data-payment-field="bankAccount" inputmode="numeric" maxlength="40" value="${esc(payment.bankAccount||'')}" ${bankDetailsLocked?'readonly':''}></div>
      <div class="field morning-transfer-fields morning-transfer-reference-field" data-payment-kinds="4"><label>אסמכתא מספר העברה <small>(רשות)</small></label><input data-payment-field="transactionId" maxlength="80" value="${esc(payment.transactionId||'')}" ${bankLocked?'readonly':''} placeholder="מספר האסמכתא מהבנק"></div>
      <div class="field morning-check-fields morning-check-number-field" data-payment-kinds="2"><label>מספר צ׳ק</label><input data-payment-field="chequeNum" inputmode="numeric" maxlength="40" value="${esc(payment.chequeNum||'')}" ${bankLocked?'readonly':''}></div>
      <div class="field morning-card-fields morning-card-type-field" data-payment-kinds="3"><label>סוג כרטיס</label><select data-payment-field="cardType">${Object.entries(CARD_TYPES).map(([value,label])=>`<option value="${value}" ${Number(payment.cardType||1)===Number(value)?'selected':''}>${esc(label)}</option>`).join('')}</select></div>
      <div class="field morning-card-fields morning-card-last4-field" data-payment-kinds="3"><label>4 ספרות אחרונות</label><input data-payment-field="cardNum" inputmode="text" maxlength="4" value="${esc(payment.cardNum||'****')}" placeholder="****"></div>
    </div>
  </div>`;
}
...
function readPaymentRows({strict=true}={}){
  ...
  return rows.map((row,index)=>{const type=Number(row.querySelector('[data-payment-field="type"]')?.value||0),date=String(currentField(`morningPaymentDate${index}`)?.value||''),price=Number(row.querySelector('[data-payment-field="price"]')?.value||0),bankSourceKey=cleanText(row.dataset.bankSourceKey,180),payment={type,date,price,currency:'ILS'};
    ...
    if(type===3){const cardType=Number(row.querySelector('[data-payment-field="cardType"]')?.value||0),rawCardNum=String(row.querySelector('[data-payment-field="cardNum"]')?.value||'').trim(),cardNum=rawCardNum||'****';if(!CARD_TYPES[cardType])throw new Error(`יש לבחור סוג כרטיס בתקבול ${index+1}`);if(!/^(?:\d{4}|\*{4})$/.test(cardNum))throw new Error(`יש להזין 4 ספרות אחרונות או **** בתקבול ${index+1}`);Object.assign(payment,{dealType:1,cardType,cardNum})}
