...
function customerOrderRow(o){return `<tr data-customer-order-id="${esc(o.id)}" data-customer-bulk-id="${esc(o.id)}" class="${esc(customerUi.customerBulkSelected.has(o.id)?'bulk-selected-row':'')}">${customerBulkCell(o.id)}<td class="customer-order-field-cell customer-order-col-order">${customerOrderInput(o,'orderNumber','מספר הזמנה')}</td><td class="customer-order-field-cell customer-order-col-customer">${customerOrderInput(o,'customerName','שם לקוח')}</td><td class="customer-order-field-cell customer-order-col-mark">${customerOrderInput(o,'mark1','סימון 1')}</td><td class="customer-order-field-cell customer-order-col-mark">${customerOrderInput(o,'mark2','סימון 2')}</td><td class="customer-order-field-cell customer-order-col-mark">${customerOrderInput(o,'mark3','סימון 3')}</td><td class="customer-order-field-cell customer-order-col-mattresses">${customerOrderInput(o,'mattresses','מזרונים')}</td><td class="customer-order-field-cell customer-order-col-note">${customerOrderInput(o,'note','הערה')}</td><td class="module-actions customer-order-actions"><div class="row-actions"><button class="icon-btn customer-order-add" title="הוסף שורה חדשה" aria-label="הוסף שורה חדשה" data-action="add-customer-order">＋</button><button class="icon-btn customer-order-delete" title="מחק שורה" aria-label="מחק שורה" data-action="delete-customer-order" data-click-arg0="${esc(o.id)}"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M4 7h16M9 7V5h6v2m-8 0 1 13h8l1-13M10 11v5m4-5v5"/></svg></button></div></td></tr>`}
