...
function renderCustomers({resultsOnly=false,resetScroll=false}={}){
  ...
  if(customerUi.customerTab==='debts'){
    ...
    page=customerPage(rows);table=`<table class="customer-table ${esc(customerUi.customerBulkMode?'customer-bulk-table':'')}"><thead><tr>${customerBulkHeader()}<th class="customer-col-name">לקוח</th><th class="customer-col-amount table-head-center">סכום</th><th class="customer-col-paid table-head-center">שולם</th><th class="customer-col-supplied table-head-center">סופק</th><th class="customer-col-invoice table-head-center">חשבונית יצאה</th><th class="customer-col-state table-head-badge-text">מצב</th><th class="customer-col-note table-head-input-text">הערה</th><th class="customer-col-clearing table-head-center">סליקה</th><th class="customer-col-customer-id table-head-center">ת.ז</th><th class="customer-col-actions"></th></tr></thead><tbody>${page.rows.map(debtRow).join('')||`<tr><td colspan="${esc(customerUi.customerBulkMode?11:10)}" class="empty">אין חובות המתאימים לסינון.</td></tr>`}</tbody></table>`;
  }else{
