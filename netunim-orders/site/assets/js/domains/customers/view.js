...
function renderCustomers({resultsOnly=false}={}){
  ...
  if(customerUi.customerTab==='debts'){
    ...
    table=`<table class="customer-orders-table ${esc(customerUi.customerBulkMode?'customer-bulk-table':'')}"><thead><tr>${customerBulkHeader()}<th>הזמנה</th><th>לקוח</th><th>סימון 1</th><th>סימון 2</th><th>סימון 3</th><th>מזרונים</th><th>הערה</th><th class="customer-order-actions-head"><button class="icon-btn customer-order-add" title="הוסף שורה חדשה" aria-label="הוסף שורה חדשה" data-action="add-customer-order">＋</button></th></tr></thead><tbody>${rows.map(customerOrderRow).join('')||`<tr><td colspan="${esc(customerUi.customerBulkMode?9:8)}" class="empty">אין הזמנות המתאימות לסינון.</td></tr>`}</tbody></table>`;
  }
...
function debtRow(d){
  ...
  return `<tr data-customer-bulk-id="${esc(d.id)}" class="${esc(s.key==='closed'?'row-closed':'')} ${esc(customerUi.customerBulkSelected.has(d.id)?'bulk-selected-row':'')}">${customerBulkCell(d.id)}<td class="customer-col-name"><b>${esc(d.customerName)}</b>${d.phone?`<div class="customer-phone">${esc(d.phone)}</div>`:''}</td><td class="money badtext customer-col-amount customer-debt-amount ${esc(amountStateClass)}"><b>${money(d.amount)}</b></td><td class="customer-col-order">${esc(d.orderNumber||'—')}</td><td class="customer-col-paid">${debtToggle(d,'paid','שולם')}</td><td class="customer-col-supplied">${debtToggle(d,'supplied','סופק')}</td><td class="customer-col-invoice">${debtToggle(d,'invoiceIssued','חשבונית יצאה')}</td><td class="customer-col-state"><span class="badge ${esc(s.cls)}">${esc(s.text)}</span></td><td class="customer-col-note"><input class="inline-input" value="${esc(d.note||'')}" placeholder="הערה" data-keydown="blur-on-enter" data-blur="save-debt-note" data-blur-arg0="${esc(d.id)}"></td><td class="module-actions customer-col-actions"><button class="icon-btn" title="עריכה" data-action="open-debt-modal-2" data-click-arg0="${esc(d.id)}">✎</button></td></tr>`
}
