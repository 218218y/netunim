...
<div class="supplier-summary-grid">${suppliers.map(s=>`<button type="button" class="supplier-summary-card" data-action="open-supplier" data-click-arg0="${esc(s.id)}"><div class="supplier-summary-head"><div><h3>${esc(s.name)}</h3><span>${esc(yearLabel)} · ${esc(s.txCount)} תנועות</span></div><span class="supplier-summary-arrow">‹</span></div><div class="supplier-card-financial"><div><span>סה״כ חובה</span><b class="badtext">${money(s.financial.debit)}</b></div><div><span>סה״כ זכות</span><b class="goodtext">${money(s.financial.credit)}</b></div><div><span>סה״כ יתרה</span><b class="${esc(s.financial.net<0?'badtext':'goodtext')}">${money(s.financial.net)}</b></div></div><div class="supplier-summary-details"><div><span>טרם סופק</span><b class="${esc(s.pending?'warntext':'')}">${esc(s.pending)}</b></div><div><span>חשבונית חסרה</span><b class="${esc(s.missing?'badtext':'')}">${esc(s.missing)}</b></div><div><span>לא חתום</span><b class="${esc(s.unsigned?'warntext':'')}">${esc(s.unsigned)}</b></div><div><span>ח״מ</span><b>${esc(s.hm)}</b></div><div><span>יתרה כוללת</span><b class="${esc(s.b<0?'badtext':'goodtext')}">${money(s.b)}</b></div></div></button>`).join('')||'<div class="empty">אין ספקים להצגה.</div>'}</div></section>`;
}
