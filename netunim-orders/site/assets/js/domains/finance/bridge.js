...
async function fetchChequeImage(imageKey){const key=String(imageKey||'').trim().toLowerCase();if(!/^[a-f0-9]{64}$/.test(key))return null;const token=getBridgeToken();if(!token)throw bridgeError('חסר מפתח Bank Bridge במחשב זה.','BRIDGE_NOT_PAIRED');const controller=new AbortController(),timer=setTimeout(()=>controller.abort(),30000);try{const response=await fetch(`${BRIDGE_URL}/bank/cheque-image/${key}`,{headers:{Authorization:`Bearer ${token}`,Accept:'image/*'},signal:controller.signal,cache:'no-store'});if(response.status===404)return null;if(!response.ok){let data={};try{data=await response.json()}catch{}throw bridgeError(data.message||`טעינת תמונת שיק מה-Bridge נכשלה (${response.status})`,data.code||`HTTP_${response.status}`)}const blob=await response.blob();if(!blob.size||blob.size>5*1024*1024||!/^image\/(?:jpeg|png|webp|gif|bmp)$/i.test(blob.type))throw bridgeError('Bank Bridge החזיר קובץ תמונת שיק לא תקין','CHEQUE_IMAGE_INVALID');return blob}catch(error){if(error?.name==='AbortError')throw bridgeError('טעינת תמונת השיק מה-Bank Bridge לא הגיבה בזמן','BRIDGE_TIMEOUT');if(error?.code)throw error;throw bridgeError('לא ניתן לטעון את תמונת השיק מה-Bank Bridge המקומי','BRIDGE_UNAVAILABLE')}finally{clearTimeout(timer)}}
