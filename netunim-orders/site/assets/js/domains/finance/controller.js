...
const BANK_BRIDGE_VERSION=55;
const CREDIT_BRIDGE_VERSION=56;
...
export function createDomainsFinanceController({tab,checksSession,bridge,loadSession,refreshKupaReadout,readKupaReadOnlyCloud,rpcSaveKupaDocument,acceptKupaCloudRow,syncSharedChecksFromCloud,saveSharedChecksToCloud,checksHaveLocalWork,getSharedChecks=()=>[],toast,readFinanceSyncDocument=null,rpcSaveFinanceSync=null,claimFinanceSyncLease=async()=>({acquired:true}),releaseFinanceSyncLease=async()=>true,saveBankSyncSnapshot:publishBankSyncSnapshot=null,mergeBankTransactions=async()=>null,syncBankTransactionsSnapshot=async()=>null,readBankTransactions=async()=>[],readBankTransactionSnapshot=async()=>null,setBankTransactionHandled:saveBankTransactionHandled=async()=>null,acknowledgeBankTransactionMissing=async()=>null,acknowledgeBankTransactionAlert=async()=>null,syncBankChequeImages=async()=>({ok:true,warnings:[]}),touchBankDisplayRevision=()=>{},readRevision} ){
  ...
  const readProjection=createRevisionSelector({revision:readRevision,select:snapshot,name:'orders-finance'});
  ...
  async function mutateKupaCloud(mutator){
    ...
    for(let conflictAttempt=0;conflictAttempt<CLOUD_WRITE_POLICY.conflictAttempts;conflictAttempt++){
      ...
      const result=await runBusyCloudWriteWithPolicy(()=>rpcSaveKupaDocument(after,Number(row.revision||0),operationId,audit));
  ...
  async function mutateFinanceCloud(mutator,lease=null){
    if(!lease&&typeof rpcSaveFinanceSync==='function')return manualFinanceQueue(held=>mutateFinanceCloud(mutator,held));
    ...
    for(let conflictAttempt=0;conflictAttempt<CLOUD_WRITE_POLICY.conflictAttempts;conflictAttempt++){
      ...
      const result=await runBusyCloudWriteWithPolicy(()=>rpcSaveFinanceSync(candidate,Number(row?.revision||0),operationId,audit,lease));
  ...
  async function refreshBank({interactive=false,auto=false}={}){
    ...
    try{
      ...
      leaseToken=uid('FINLEASE');lease=await claimFinanceSyncLease('bank',leaseToken);leaseHeld=lease?.acquired===true;
      ...
      let imageSyncWarning='';try{const imageSync=await syncBankChequeImages([...(business.transactions||[]),...(home?.transactions||[])]);if(imageSync?.warnings?.length)imageSyncWarning=`תמונות שיקים: ${imageSync.warnings.join(' | ')}`}catch(error){imageSyncWarning=`תמונות שיקים לא סונכרנו לענן: ${error?.message||error}`}
      const [businessArchive,businessDirect,homeArchive,homeDirect]=await Promise.all([readBankTransactions(businessAccount,'business',{days:370}),readBankTransactionSnapshot(businessAccount,'business'),home&&homeAccount?readBankTransactions(homeAccount,'home',{days:370}):Promise.resolve([]),home&&homeAccount?readBankTransactionSnapshot(homeAccount,'home'):Promise.resolve(null)]);
      bankDisplayArchive.business={accountKey:String(businessAccount||''),syncKey:String(fetchedAt),rows:businessArchive,directSnapshot:businessDirect};if(homeAccount)bankDisplayArchive.home={accountKey:String(homeAccount||''),syncKey:String(fetchedAt),rows:homeArchive,directSnapshot:homeDirect};
  ...
  async function refreshCredit({interactive=false,auto=false,syncMode='full'}={}){
    ...
    try{
      ...
      leaseToken=uid('FINLEASE');lease=await claimFinanceSyncLease('credit',leaseToken);leaseHeld=lease?.acquired===true;
  ...
  return {snapshot,readSnapshot,ensureBankDisplayArchive,refreshFinanceData,refreshBankBridgeStatus,refreshCreditBridgeStatus,copySafeCreditDiagnostics,exportCreditDataDiagnostics,saveBridgeToken,configureBankBridge,selectBankBridgeAccount,deleteBankBridgeCredentials,exportBankChequeDiagnostics,refreshBank,toggleBankTransactionHandled,markBankMorningVerified,acknowledgeMissingBankTransaction,acknowledgePersistentBankAlert,refreshCredit,saveCreditProfile,deleteCreditProfile,resetCreditSync,saveCreditCardOrder,setCreditCardMapping,acknowledgeCreditSettlementWarning,maybeAutoRefreshBank,maybeAutoRefreshCredit,startAutoSync,setBankAutoEnabled,setCreditAutoEnabled,setCreditAutoMode,saveCashflowMinimum,saveCashflowCheckCutoff,mutateKupaCloud};
}
