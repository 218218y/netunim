...
function stockCard(i,location=''){
  ...
  return `<tr data-stock-bulk-id="${esc(i.id)}" data-warehouse-bulk-id="${esc(i.id)}" class="${short?'stock-short':''} ${selected?'bulk-selected-card':''}">
    ...
    <td><div class="stock-actions">${button('open-inventory-event-modal','הזמן')}${button('open-stock-receive','קליטה')}${button('open-inventory-event-modal-2','שמור')}<details class="warehouse-menu" data-dismiss-on-outside><summary aria-label="פעולות עבור ${esc(i.name)}" title="פעולות">⋯</summary><div class="warehouse-menu-content" data-menu-panel>${button('open-stock-transfer','העברה בין מחסנים')}${button('open-stock-adjustment-modal','ספירת מלאי')}${button('open-inventory-item-modal','עריכת פריט')}${button('open-inventory-details','פרטים ותנועות')}</div></details></div></td></tr>`;
}
