...
export function createDomainsNotesController({model, notesUi, scheduleSave, toast, mountViewLayout}){
...
function deleteStickyNote(id){const note=model.state.notes.find(x=>x.id===id);if(!note)return;if(!confirm('למחוק את הפתק הזה?'))return;model.state.notes=model.state.notes.filter(x=>x.id!==id);notesUi.notesBulkSelected.delete(id);scheduleSave('הפתק נמחק');renderNotes()}
