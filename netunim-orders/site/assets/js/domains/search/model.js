...
function checkEntries(state){return (Array.isArray(state.checks)?state.checks:[]).map(c=>{const account=c.account==='ביתי'?'ביתי':'עסקי';return{group:'checks',kind:'check',id:c.id,account,context:c.name||'לקוח',badge:`צ'ק ${account}`,title:c.name||"צ'ק",subtitle:[c.checkNumber?`מס׳ ${c.checkNumber}`:'',c.dueDate?`פירעון ${c.dueDate}`:'',c.note].filter(Boolean).join(' · '),amount:Number(c.amount||0),meta:[c.status,account].filter(Boolean),searchText:entrySearchText([c.name,c.amount,c.dueDate,c.status,account,c.depositDate,c.clearedDate,c.checkNumber,c.note,c.createdAt])}})}
