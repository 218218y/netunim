...
export function createDomainsServiceBulk({serviceUi, model, renderService, toast, scheduleSave}){
...
function deleteSelectedServiceCalls(){const valid=new Set(model.state.serviceCalls.map(x=>x.id)),ids=[...serviceUi.serviceBulkSelected].filter(id=>valid.has(id));if(!ids.length)return toast('לא נבחרו קריאות שירות למחיקה');if(!confirm(`למחוק ${ids.length} קריאות שירות שנבחרו?\n\nהפעולה מוחקת את הקריאות עצמן, כפי שעושה כפתור המחיקה בעריכת קריאה.`))return;const set=new Set(ids);model.state.serviceCalls=model.state.serviceCalls.filter(x=>!set.has(x.id));serviceUi.serviceBulkSelected.clear();scheduleSave(`${ids.length} קריאות שירות נמחקו`);renderService()}
