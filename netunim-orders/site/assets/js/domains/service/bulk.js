...
function toggleServiceBulkMode(){serviceUi.serviceBulkMode=!serviceUi.serviceBulkMode;serviceUi.serviceBulkSelected.clear();serviceUi.serviceBulkAnchorId=null;renderService()}

function toggleServiceBulkRow(id,checked,shiftKey=false){if(!serviceUi.serviceBulkMode||!model.state.serviceCalls.some(x=>x.id===id))return;serviceUi.serviceBulkAnchorId=applyBulkRangeSelection({selected:serviceUi.serviceBulkSelected,orderedIds:serviceVisibleBulkIds(),id,checked,shiftKey,anchorId:serviceUi.serviceBulkAnchorId});syncServiceBulkUi()}
