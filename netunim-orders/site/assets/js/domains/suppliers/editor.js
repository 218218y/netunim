...
import {ALL_SUPPLIERS_ID, validSupplierYear, supplierSortValue} from './model.js';
...
function openTransactionModal(id=null,supplierId=null,insertAfterId=null){
  ...
  const triValue=field=>t?(t[field]??null):false;
  modal(t?'עריכת תנועה':anchor?'הוספת תנועה באמצע הכרטיס':'תנועה חדשה',`<div class="form-grid"><div class="field supplier-transaction-supplier"><label>ספק</label><select id="fSupplier" ${anchor||validSupplierYear(t?.yearEnd)!==null?'disabled':''}>${opts}</select></div><div class="field supplier-transaction-action"><label>פעולה</label><input id="fAction" value="${esc(t?.action||'')}" placeholder="לדוגמה: הזמנה 1234 / תשלום"></div>${positionNotice}${yearNotice}<div class="field supplier-money-field is-debit"><label for="fDebit">סכום חובה</label><input id="fDebit" class="number-input" type="number" step="1" value="${esc(Number(t?.debit||0)||'')}"></div><div class="field supplier-money-field is-credit"><label for="fCredit">סכום זכות</label><input id="fCredit" class="number-input" type="number" step="1" value="${esc(Number(t?.credit||0)||'')}"></div>${triSelect('fInvoice','חשבונית התקבלה',triValue('invoiceReceived'),true)}${triSelect('fSigned','חתום',triValue('signed'),true)}${triSelect('fSupplied','סופק',triValue('supplied'),true)}<div class="field"><label>זמן / פרטי אספקה</label><input id="fSupplyInfo" value="${esc(t?.supplyInfo||'')}" placeholder="תאריך, שבוע, הערה…"></div><div class="field full"><label>הערה</label><textarea id="fNote">${esc(t?.note||'')}</textarea></div>${t?.source?`<div class="field full"><div class="notice">מקור: ${esc(t.source.sheet)} · שורה ${esc(t.source.row)}. עריכה כאן אינה משנה את קובץ האקסל המקורי.</div></div>`:''}</div>`,`<button class="btn primary" data-action="save-transaction" data-click-arg0="${esc(id||'')}" data-click-arg1="${esc(insertAfterId||'')}">שמור</button>${t?`<button class="btn danger" data-action="delete-transaction" data-click-arg0="${esc(t.id)}">מחק תנועה</button>`:''}<button class="btn" data-action="close-modal">ביטול</button>`)
}

async function saveTransaction(id,insertAfterId=''){
  const existing=id?model.state.transactions.find(x=>x.id===id):null;
  const keepAllSuppliers=supplierUi.currentSupplierId===ALL_SUPPLIERS_ID&&!!(existing||insertAfterId);
  ...
  const row={...(existing||{}),id:existing?.id||uid(),supplierId,sequence:existing?.sequence||0,kind:debit?'חיוב':credit?'זיכוי/תשלום':'הערה',action,debit,credit,invoiceReceived:parseTri('fInvoice'),signed:parseTri('fSigned'),supplied:parseTri('fSupplied'),supplyInfo:$('#fSupplyInfo').value.trim(),note:$('#fNote').value.trim(),updatedAt:new Date().toISOString()};delete row.hmIssued;
  if(existing){
