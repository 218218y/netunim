...
import {validSupplierYear, transactionWorkflowComplete} from './model.js';
import {$} from '../../state/constants.js';

// Dependencies are supplied by the composition root; this module has no startup side effects.
export function createDomainsSuppliersView({model, supplierUi, balanceRows, supplierYearContext, mountViewLayout, orderedSuppliers, captureSupplierViewport, restoreSupplierViewport, syncSupplierBulkUi, supplierMoveTargetRow, storeSupplierViewport, scheduleSave}){
function renderSupplier({scrollMode='auto'}={}){
  ...
  const rows=supplierUi.supplierMoveTargetId?all:all.filter(({t})=>{
...
function filterSupplierSearch(value){if(supplierUi.supplierMoveTargetId){document.querySelectorAll('tbody tr[data-search]').forEach(row=>row.hidden=false);syncSupplierBulkUi();return}const hadQuery=!!supplierUi.searchText.trim();supplierUi.searchText=value;const q=value.trim();document.querySelectorAll('tbody tr[data-search]').forEach(row=>row.hidden=!!q&&!row.dataset.search.includes(q));syncSupplierBulkUi();if(hadQuery&&!q){const wrap=$('#main')?.querySelector('.supplier-table-panel .table-wrap');if(wrap)requestAnimationFrame(()=>{wrap.scrollTop=wrap.scrollHeight;storeSupplierViewport(supplierUi.currentSupplierId,wrap)})}}
