...
export function bindOrdersRuntimeEvents({uiModal,uiNavigation,domainsSuppliersNavigation,cloudAuth,uiStatus,syncChecks,tab,domainsCustomers,domainsFinanceController,stateSnapshots,syncDocument,storageBrowser,storageChecks,uiFolders,uiAlertCenter,uiTabGuard}){
  ...
  window.addEventListener('pagehide',()=>{if(!tab.primaryTab)return;storageBrowser.localSnapshot(undefined,{storageBoundary:'pagehide-v1-checkpoint'});if(cloudAuth.cloudEnabled()&&stateSnapshots.cloudHasLocalWork()&&!storageBrowser.storageV2CloudOutboxActive())storageBrowser.markCloudPending();if(cloudAuth.loadSession()&&stateSnapshots.checksHaveLocalWork())storageChecks.markChecksPending()});
