...
export function bindOrdersRuntimeEvents({uiModal,uiNavigation,domainsSuppliersNavigation,cloudAuth,uiStatus,syncChecks,tab,session,domainsCustomers,domainsFinanceController,stateSnapshots,syncDocument,storageBrowser,storageChecks,uiFolders,uiAlertCenter,uiTabGuard,storageV2=null}){
  ...
  window.addEventListener('pagehide',()=>{if(!tab.primaryTab)return;if(!storageV2?.primaryReady)storageBrowser.localSnapshot(undefined,{storageBoundary:'pagehide-v1-checkpoint'});if(cloudAuth.cloudEnabled()&&!storageBrowser.storageV2CloudOutboxActive()&&stateSnapshots.cloudHasLocalWork())storageBrowser.markCloudPending();if(cloudAuth.loadSession()&&stateSnapshots.checksHaveLocalWork())storageChecks.markChecksPending()});
  window.addEventListener('beforeunload',event=>{if(!tab.primaryTab||!(storageV2?.durabilityAtRisk||session?.localUndurableGenerations?.size))return;event.preventDefault();event.returnValue=''});
