...
export function bindOrdersRuntimeEvents({uiModal,uiNavigation,domainsSuppliersNavigation,cloudAuth,uiStatus,syncChecks,tab,session,domainsCustomers,domainsFinanceController,stateSnapshots,syncDocument,storageBrowser,storageChecks,uiFolders,uiAlertCenter,uiTabGuard,storageV2=null,sharedChecksV2=null}){
  ...
  window.addEventListener('pagehide',()=>{if(!tab.primaryTab||session.storageProtocolBlocked)return;if(!storageV2?.primaryReady&&!storageV2?.cutoverActive)storageBrowser.localSnapshot(undefined,{storageBoundary:'pagehide-v1-checkpoint'});if(!storageV2?.cutoverActive&&cloudAuth.cloudEnabled()&&!storageBrowser.storageV2CloudOutboxActive()&&stateSnapshots.cloudHasLocalWork())storageBrowser.markCloudPending();if(!sharedChecksV2?.primaryReady&&!storageV2?.cutoverActive&&cloudAuth.loadSession()&&stateSnapshots.checksHaveLocalWork())storageChecks.markChecksPending()});
