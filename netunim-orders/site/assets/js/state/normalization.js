...
export function createStateNormalization({model={},externalWorkbooks=false}){
  function normalizeState(input){
    ...
    state.transactions=seed('transactions');state.transactions.forEach(transaction=>{delete transaction.hmIssued;if(transaction.yearEnd!==undefined&&transaction.yearEnd!==null&&transaction.yearEnd!==''){const year=Number(transaction.yearEnd);if(Number.isInteger(year)&&year>=2000&&year<=2100)transaction.yearEnd=year;else delete transaction.yearEnd}});
    state.customerDebts=seed('customerDebts');state.customerDebts.forEach(debt=>{const customerId=String(debt.customerId||debt.taxId||'').replace(/\D/g,'').slice(0,9),clearingApproval=String(debt.clearingApproval||'').replace(/\D/g,'').slice(0,20);debt.customerId=customerId;debt.clearingApproval=clearingApproval;delete debt.orderNumber;delete debt.taxId});state.customerOrders=seed('customerOrders');state.serviceCalls=seed('serviceCalls');state.inventoryItems=seed('inventoryItems');
