...
function scheduleSave(message='השינויים נשמרו',{deleteIntents={},mutationType='autosave',surface='orders',domains=null}={}){
...
async function manualSaveNow(){
  ...
  try{await session.ordersOutboxCommitPromise}catch(error){console.error('manual durable staging',error);setSave('השינוי לא נשמר — אין לסגור את החלון','error');setCloud('ענן: אחסון מקומי נכשל','error');return false}
