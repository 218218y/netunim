...
export function createStorageTabLock({tab,showSecondaryTabGuard}){
  ...
  function setPrimary(value){
    if(value&&tab.readOnlyTab)value=false;
    if(!value)tab.readOnlyTab=true;
    const changed=tab.primaryTab!==value||!tab.primaryTabReady;tab.primaryTab=value;tab.primaryTabReady=true;if(changed)showSecondaryTabGuard();return value
  }
  ...
  function scheduleElection(delay=100+Math.floor(Math.random()*250)){if(tab.readOnlyTab)return;clearTimeout(retryTimer);retryTimer=setTimeout(()=>{const lease=currentLease();if(lease?.invalid){scheduleElection(1000);return}if(!lease||Number(lease.expiresAt)<=Date.now())claimFallbackLease()},delay)}
  function claimFallbackLease(){if(tab.readOnlyTab){setPrimary(false);return false}const lease=currentLease();if(lease?.invalid){failClosed('fallback_lock_unproven');scheduleElection(1000);return false}if(lease&&lease.id!==tabId&&Number(lease.expiresAt)>Date.now()){setPrimary(false);scheduleElection(Math.max(250,Number(lease.expiresAt)-Date.now()+Math.floor(Math.random()*250)));return false}const won=writeLease();setPrimary(won);if(won)channel?.postMessage({type:'primary-heartbeat',id:tabId});return won}
  function heartbeat(){const lease=currentLease();if(lease?.invalid){failClosed('fallback_lock_unproven');scheduleElection(1000);return}if(tab.primaryTab){if(lease&&lease.id!==tabId&&Number(lease.expiresAt)>Date.now()){setPrimary(false);scheduleElection();return}if(!writeLease()){failClosed('fallback_lease_refresh_failed');scheduleElection()}}else if(!tab.readOnlyTab&&(!lease||Number(lease.expiresAt)<=Date.now()))scheduleElection()}
  ...
  async function acquireFallbackLock(){try{channel=typeof BroadcastChannel==='function'?new BroadcastChannel(`${FALLBACK_KEY}:channel`):null;channel?.addEventListener('message',event=>{if(event.data?.type==='primary-heartbeat'&&event.data.id!==tabId){const lease=currentLease();if(!lease?.invalid&&lease?.id!==tabId)setPrimary(false)}});window.addEventListener('storage',event=>{if(event.key!==FALLBACK_KEY)return;const lease=parseLease(event.newValue);if(event.newValue!==null&&!lease){failClosed('fallback_lease_corrupt');return}if(lease?.id!==tabId&&Number(lease?.expiresAt||0)>Date.now())setPrimary(false);else if(!tab.readOnlyTab)scheduleElection()});window.addEventListener('pagehide',releaseFallbackLease,{once:true});claimFallbackLease();heartbeatTimer=setInterval(heartbeat,HEARTBEAT_MS);await new Promise(resolve=>setTimeout(resolve,50+Math.floor(Math.random()*100)));heartbeat();return tab.primaryTab}catch(error){failClosed('fallback_backend_failed',error);scheduleElection(1000);return false}}
