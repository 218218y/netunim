...
function scheduleCheckSave(message,{deletedIds=[]}={}){if(rejectSecondaryMutation())return;refreshAlertCenter();const generation=++session.localGeneration,localOk=localSnapshot();checksSession.checksGeneration++;setSave(localOk?'מקומי: שומר…':'מקומי: שגיאה',localOk?'':'error',folderSaveTitle());clearTimeout(session.saveTimer);session.saveTimer=setTimeout(async()=>{session.saveTimer=null;try{if(folderBackupAvailable())await writeStateToFolder();else syncFolderAccessButton()}catch(e){console.error('folder save',e)}if(generation===session.localGeneration&&localOk)setSave('מקומי: שמור','',folderSaveTitle())},180);if(loadSession()){markChecksPending(model?.state?.checks,undefined,undefined,{deleteIds:deletedIds});queueSharedChecksSave(message)}else if(message)toast(message)}
