...
function refreshForMorningRecovery(){
  ...
  morningRefreshPromise=(async()=>{
    ...
    try{
      ...
      if(localSnapshot(undefined,{storageBoundary:'morning-cloud-refresh'})===false)return false;
...
async function saveCloudSnapshot(snapshot,startGeneration,pendingRecord=null){
  ...
  if(!localSnapshotWritten)localSnapshot(undefined,{storageBoundary:'cloud-ack-mirror'});try{if(files.dirHandle)await writeStateToFolder()}catch(localError){console.error('local backup/mirror',localError)}return true
}
