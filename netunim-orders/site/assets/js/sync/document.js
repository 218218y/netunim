...
async function saveCloudSnapshot(snapshot,startGeneration,pendingRecord=null){
  ...
  if(pendingNow){
    ...
    localSnapshotWritten=localSnapshot()!==false;markCloudPending(prepareCloudState(model.state),'',{baseRevision:session.cloudRevision,baseState:authoritative,conflict:null,deleteIntents:pendingNow?.deleteIntents||deleteIntents});await session.ordersOutboxCommitPromise;session.cloudSaveRequested=true
  }
  if(!localSnapshotWritten)localSnapshot();try{if(files.dirHandle)await writeStateToFolder()}catch(localError){console.error('local backup/mirror',localError)}return true
}
...
async function cloudPoll(){if(!tab.primaryTab)return;if(!cloudEnabled()||session.cloudBusy||!navigator.onLine)return;if(cloudHasLocalWork()){await requestCloudSave('סונכרנו שינויים מקומיים ועדכון מרחוק');await pollSharedChecks();return}let dataApiReadOk=false;try{session.cloudBusy=true;const meta=await readCloudMeta();dataApiReadOk=true;if(!meta)return;const metaRevision=Number(meta.revision||0);if(metaRevision<=session.cloudRevision){session.cloudUpdatedAt=meta.updated_at||session.cloudUpdatedAt;refreshCloudTimestamp();return}const row=await readCloud();if(!row)return;if(Number(row.revision||0)<=session.cloudRevision)return;const meaningful=!sameOrderCloudData(model.state,row.state);session.cloudRevision=Number(row.revision);session.cloudUpdatedAt=row.updated_at||meta.updated_at||session.cloudUpdatedAt;if(!meaningful){session.lastCloudState=prepareCloudState(row.state||model.state);try{localStorage.setItem(CLOUD_BASE_KEY,JSON.stringify(session.lastCloudState))}catch(error){console.error('cloud base mirror',error)};refreshCloudTimestamp();setCloud('ענן: מסונכרן','synced');return}applyOrderCloudState(row.state);session.lastCloudState=prepareCloudState(model.state);try{localStorage.setItem(CLOUD_BASE_KEY,JSON.stringify(session.lastCloudState))}catch(error){console.error('cloud base mirror',error)};localSnapshot();try{if(files.dirHandle)await writeStateToFolder()}catch(localError){console.error('local backup/mirror',localError)}setCloud('ענן: מסונכרן','synced');render();toast('התקבל עדכון מהענן')}catch(e){const normalized=normalizeCloudError(e);if(['network','timeout','service_unavailable','rate_limited'].includes(normalized.kind))console.warn('orders cloud poll deferred',e?.message||e);else console.error(e)}finally{session.cloudBusy=false;if(dataApiReadOk){await pollSharedChecks();await refreshKupaReadout({renderIfChanged:true})}}}
