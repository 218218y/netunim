...
async function saveCloudSnapshot(snapshot,startGeneration,pendingRecord=null){
  let base=clone(pendingRecord?.baseState||session.lastCloudState||snapshot),expected=Number(pendingRecord?.baseRevision??session.cloudRevision??0),res=null,serverSnapshot=clone(snapshot),mergedRemote=false;
  ...
  for(let conflictAttempt=0;conflictAttempt<CLOUD_WRITE_POLICY.conflictAttempts;conflictAttempt++){
    res=await runBusyCloudWriteWithPolicy(()=>rpcSave(serverSnapshot,expected,operationId));
    ...
    const merged=merge3(base,serverSnapshot,remote.state||{});
    if(merged.conflicts.length){session.cloudConflictBlocked=true;markCloudPending(snapshot,'',{conflict:{kind:'entity-conflict',items:merged.conflicts}});setCloud('ענן: התנגשות','error');toast('יש התנגשות בענן באותה רשומה. הנתונים המקומיים נשמרו ולא נדרסו.');return false}
  ...
  if(!res?.r?.ok)throw new Error(res?.j?.message||res?.txt||'שמירה לענן נכשלה');
  const replayed=res.row?.operation_replayed===true,authoritative=prepareCloudState(res.row?.state||serverSnapshot);
  session.cloudRevision=Number(res.row?.revision||expected+1);session.cloudUpdatedAt=res.row?.updated_at||session.cloudUpdatedAt;session.lastCloudState=clone(authoritative);localStorage.setItem(CLOUD_BASE_KEY,JSON.stringify(session.lastCloudState));session.cloudConflictBlocked=false;
  const pendingNow=await getCloudPending();
  if(Number(pendingNow?.generation||0)===Number(startGeneration)){
    if(mergedRemote||replayed||!sameOrderCloudData(model.state,authoritative))applyOrderCloudState(authoritative);
    const cleared=await clearCloudPending(startGeneration);
    if(!cleared){const latest=await getCloudPending();if(Number(latest?.generation||0)>Number(startGeneration))session.cloudSaveRequested=true;else{setCloud('ענן: השינוי אושר; ניקוי מקומי ממתין להתאוששות','error');return false}}
  }else{
    if(mergedRemote||replayed){const sharedChecks=clone(model.state.checks||[]),rebased=merge3(snapshot,prepareCloudState(model.state),authoritative,{preferLocalConflicts:true});model.state=normalizeState(rebased.state);model.state.checks=sharedChecks}
    localSnapshot();markCloudPending(prepareCloudState(model.state),'',{baseRevision:session.cloudRevision,baseState:authoritative,conflict:null});session.cloudSaveRequested=true
  }
