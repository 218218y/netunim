...
async function saveStorageV2CloudFlight(initialFlight){
  ...
  for(let conflictAttempt=0;conflictAttempt<CLOUD_WRITE_POLICY.conflictAttempts;conflictAttempt++){
    ...
    res=await runBusyCloudWriteWithPolicy(()=>rpcSave(serverSnapshot,expected,flight.operationId,exactIntents,audit));
    if(saveBusy(res))throw new Error('save_busy');
    if(!revisionConflict(res))break;
    await contentionBackoff(conflictAttempt);
...
async function requestStorageV2CloudSave(message='השינויים סונכרנו'){
  if(!tab.primaryTab)return false;if(message)session.cloudSaveMessage=message;session.cloudSaveRequested=true;if(!cloudEnabled())return false;
  ...
  session.cloudSavePromise=(async()=>{let allOk=true;while(session.cloudSaveRequested&&cloudEnabled()&&navigator.onLine&&!session.cloudConflictBlocked){
    ...
    if(state.control?.conflict){session.cloudConflictBlocked=true;session.cloudSaveRequested=false;setCloud('ענן: התנגשות','error');return false}
    const currentFlight=state.flight||null,retryRecord=v2RetryRecord(state,currentFlight),retryDelay=outboxRetryScheduler.schedule(retryRecord,()=>requestCloudSave(msg));
