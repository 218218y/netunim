...
function restoreDeleteIntents(before,after){const out={};for(const key of ['suppliers','transactions','customerDebts','customerOrders','serviceCalls','notes','inventoryItems','inventoryEvents','warehouseOrders']){const kept=new Set((after?.[key]||[]).map(x=>String(x?.id||''))),ids=(before?.[key]||[]).map(x=>String(x?.id||'')).filter(id=>id&&!kept.has(id));if(ids.length)out[key]=ids}return out}
...
async function applyJsonRestore(){
  ...
  try{
    ...
    let remoteRow=null;if(cloudEnabled()&&navigator.onLine){remoteRow=await readCloud();if(!remoteRow)throw new Error('לא נמצא מסמך ניהול ההזמנות בענן');session.cloudRevision=Number(remoteRow.revision||0);session.cloudUpdatedAt=remoteRow.updated_at||session.cloudUpdatedAt;session.lastCloudState=prepareCloudState(remoteRow.state||{});localStorage.setItem(CLOUD_BASE_KEY,JSON.stringify(session.lastCloudState))}
    if(restoreChecks&&loadSession()&&navigator.onLine){const kupaRow=await readSharedChecksCloud();if(!kupaRow)throw new Error('לא נמצא מסמך הקופה בענן');checksSession.checksCloudBase=normalizeSharedChecks(kupaRow.state?.checks||[]);checksSession.checksBankEvents=normalizeSharedBankEvents(kupaRow.state?.bankEvents);checksSession.checksCloudRevision=Number(kupaRow.revision||0);checksSession.checksCloudUpdatedAt=kupaRow.updated_at||checksSession.checksCloudUpdatedAt;persistChecksBase(checksSession.checksCloudBase,checksSession.checksBankEvents)}
    model.state=imported;if(!restoreChecks)model.state.checks=currentChecks;const restoredDeleteIds=restoreChecks?currentChecks.map(x=>x.id).filter(id=>!model.state.checks.some(x=>x.id===id)):[];ui.pendingJsonRestore=null;closeModal();session.localGeneration++;const localOk=localSnapshot();if(!localOk){model.state=normalizeState(clone(current));localSnapshot();throw new Error('השמירה המקומית לאחר השחזור נכשלה; המצב הקודם הוחזר')};
    if(cloudEnabled()){markCloudPending(undefined,'',{deleteIntents:restoreDeleteIntents(current,model.state)});session.cloudSaveRequested=true}if(folderBackupAvailable())await writeStateToFolder(true);
    let cloudOk=true;if(cloudEnabled())cloudOk=await requestCloudSave('גיבוי JSON שוחזר וסונכרן');
    let checksOk=true;if(restoreChecks&&loadSession()){checksSession.checksGeneration++;markChecksPending(model.state.checks,undefined,undefined,{deleteIds:restoredDeleteIds});checksSession.checksSaveRequested=true;checksOk=await saveSharedChecksToCloud('הצ׳קים מהגיבוי שוחזרו וסונכרנו')}
