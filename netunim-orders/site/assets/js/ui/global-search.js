...
export function createUiGlobalSearch({documentBridge=null,searchRevision,model,ui,notesUi={},supplierUi,customerUi,serviceUi,warehouseUi,prepareView,render,openInventoryItemModal}){
  let resultByKey=new Map(),highlightTimer=null,backdropPointerId=null,mode='site',documentSearchMode='content',documentSequence=0,documentAbort=null;
  const byId=id=>document.getElementById(id);
  const refs=()=>({trigger:byId('globalSearchButton'),backdrop:byId('globalSearchBackdrop'),input:byId('globalSearchInput'),results:byId('globalSearchResults'),meta:byId('globalSearchMeta'),close:byId('globalSearchClose'),siteMode:byId('globalSearchSiteMode'),documentsMode:byId('globalSearchDocumentsMode'),documentModebar:byId('globalSearchDocumentModebar'),documentContentMode:byId('globalSearchDocumentContentMode'),documentNameMode:byId('globalSearchDocumentNameMode')});
  ...
  function documentIntro(){const content=documentSearchMode==='content';return `<div class="global-search-empty document-search-intro"><div class="global-search-empty-icon">PDF</div><b>${content?'חיפוש בתוכן ה־PDF במחשב זה':'חיפוש לפי שם קובץ PDF'}</b><p>${content?'החיפוש משתמש רק בתוכן שכבר מאונדקס ב־Everything המקומי.':'החיפוש בודק את שמות קבצי ה־PDF באינדקס Everything המקומי.'} הקלד שתי אותיות לפחות.</p><div class="global-search-scopes"><span>${content?'OCR / תוכן מאונדקס':'שם קובץ'}</span><span>PDF בלבד</span><span>אינדקס מקומי</span></div></div>`}
  ...
  function documentErrorHtml(error){
    ...
    const hint=code==='DOCUMENT_BRIDGE_UNAVAILABLE'?'ודא ש־Document Bridge פועל במחשב זה.':code==='EVERYTHING_NOT_RUNNING'?'פתח את Everything במחשב והשאר אותו פועל ברקע.':code==='ROOTS_NOT_CONFIGURED'?'הרץ configure_document_bridge.bat במחשב והגדר את תיקיות ה־PDF.':code==='ROOT_DIAGNOSTICS_FAILED'||code==='CONTENT_NOT_INDEXED'?'הרץ שוב את מתקין Document Bridge וקרא את בדיקת ה־PDF והתוכן המאונדקס לכל תיקייה.':'בדוק את הגדרת Everything ואת אינדוקס תוכן ה־PDF במחשב זה.';
  ...
  async function renderDocumentResults(value=''){
    ...
    if(!raw){meta.textContent=documentSearchMode==='content'?'חיפוש תוכן באינדקס Everything המקומי':'חיפוש שמות PDF באינדקס Everything המקומי';results.innerHTML=documentIntro();return}
    if(raw.length<2){meta.textContent='הקלד לפחות שני תווים';results.innerHTML=documentIntro();return}
    const controller=new AbortController();documentAbort=controller;const content=documentSearchMode==='content';meta.textContent=content?'מחפש בתוכן ה־PDF במחשב זה…':'מחפש בשמות קבצי PDF…';results.innerHTML='<div class="global-search-empty"><div class="global-search-empty-icon document-search-spinner">⌕</div><b>מחפש באינדקס Everything המקומי…</b><p>המסמכים עצמם נשארים במחשב.</p></div>';
    try{
      const data=await documentBridge.search(raw,{mode:documentSearchMode,limit:40,signal:controller.signal});if(mode!=='documents'||sequence!==documentSequence)return;
      ...
      if(!rows.length){results.innerHTML=`<div class="global-search-empty"><div class="global-search-empty-icon">∅</div><b>לא נמצא PDF מתאים</b><p>${documentSearchMode==='content'?'החיפוש בוצע בתוכן המאונדקס של תיקיות המסמכים שהוגדרו במחשב זה.':'החיפוש בוצע בשמות קבצי ה־PDF שבתיקיות שהוגדרו במחשב זה.'}</p></div>`;return}
      results.innerHTML=`<section class="global-search-group document-search-group"><header><b>מסמכים במחשב</b><span>${esc(rows.length)}</span></header>${data.partial?'<div class="document-search-warning">חלק מהתיקיות לא החזירו תשובה. התוצאות הזמינות מוצגות.</div>':''}<div class="global-search-group-results">${rows.map(item=>{const metaParts=[item.rootLabel,item.modified?`עודכן ${localDate(item.modified)}`:'',bytes(item.size)].filter(Boolean);return `<button class="global-search-result document-search-result" type="button" data-document-result-id="${esc(item.id)}"><span class="global-search-result-main"><span class="global-search-result-kicker">${documentSearchMode==='content'?'תוכן PDF':'שם קובץ'}${item.rootLabel?` · ${esc(item.rootLabel)}`:''}</span><b>${esc(item.name||'מסמך PDF')}</b>${item.relativePath?`<span class="global-search-result-subtitle">${esc(item.relativePath)}</span>`:''}</span>${metaParts.length?`<span class="global-search-result-meta">${metaParts.map(x=>`<em>${esc(x)}</em>`).join('')}</span>`:''}<span class="global-search-result-arrow" aria-hidden="true">↗</span></button>`}).join('')}</div></section>`;
  ...
  function updateModeUi(){
    const {siteMode,documentsMode,documentModebar,documentContentMode,documentNameMode,input}=refs();const documents=mode==='documents',content=documentSearchMode==='content';
    ...
    if(documentModebar)documentModebar.hidden=!documents;
    ...
    if(input)input.placeholder=documents?(content?'חפש מילים בתוך תוכן ה־PDF במחשב…':'חפש בשם קובץ PDF…'):'שם, הזמנה, טלפון, צ׳ק, סכום, הערה, מיקום…';
  }
  function setDocumentSearchMode(next){
    const normalized=next==='name'?'name':'content';if(documentSearchMode===normalized)return;documentSearchMode=normalized;scheduledDocumentRender.cancel();documentSequence+=1;documentAbort?.abort();documentAbort=null;updateModeUi();const {input}=refs();renderDocumentResults(input?.value||'');requestAnimationFrame(()=>input?.focus())
  }
  ...
  async function pairDocumentBridge(){
    ...
    try{const status=await documentBridge.status();if(mode!=='documents')return;const roots=Array.isArray(status.roots)?status.roots:[],pdfs=roots.reduce((sum,row)=>sum+(Number(row.pdfCount)||0),0),indexed=roots.reduce((sum,row)=>sum+(Number(row.indexedContentCount)||0),0);if(meta)meta.textContent=`מחובר ל-Everything ${status.everythingVersion||''} · ${roots.length} תיקיות · ${pdfs} PDF · ${indexed} עם תוכן מאונדקס`;renderDocumentResults(input?.value||'')}
