...
async function openDocument(ownerId:string,body:any){const id=clean(body?.document_id,80);if(!isUuid(id))return json({ok:false,code:'invalid_document_id',message:'מזהה המסמך אינו תקין'},400);const {data:owned,error:ownedError}=await admin.from('morning_document_operations').select('document_id').eq('owner_id',ownerId).eq('state','created').eq('document_id',id).limit(1).maybeSingle();if(ownedError)return dbError(ownedError,'morning_document_access_check_failed');if(!owned)return json({ok:false,code:'morning_document_not_owned',message:'המסמך אינו משויך לחשבון המחובר'},403);try{const data=await morningRequest(`/documents/${encodeURIComponent(id)}`,{method:'GET'}),url=documentUrl(data);if(url)return json({ok:true,url});try{const links=await morningRequest(`/documents/${encodeURIComponent(id)}/download/links`,{method:'GET'}),fallback=documentUrl({url:links})||documentUrl(links);if(fallback)return json({ok:true,url:fallback})}catch{/* older accounts can still return URL on the document itself */}return json({ok:false,code:'morning_document_url_missing',message:'Morning לא החזירה קישור צפייה למסמך'},502)}catch(error:any){return json({ok:false,code:'morning_document_open_failed',message:String(error?.message||'פתיחת המסמך נכשלה')},error?.status===404?404:502)}}
...
Deno.serve(async(req:Request)=>{
  ...
  if(action==='status')return status(user.id,body);if(action==='preview')return preview(user.id,body);if(action==='create')return create(user.id,body);if(action==='open_document')return openDocument(user.id,body);return json({ok:false,code:'morning_unknown_action',message:'פעולת Morning אינה מוכרת'},400);
});
