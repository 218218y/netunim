...
export function cashflowExplorerMarkup(cashflow,action,role,dateEditorMarkup){
  ...
  return `<div data-cashflow-explorer><div class="cashflow-date-controls" data-no-draft-guard dir="rtl"><div><span>תאריך התחזית</span>${editor}</div><button type="button" class="btn" data-action="${esc(action)}" data-click-arg0="${esc(role)}">חזרה לחישוב האוטומטי</button><p class="muted">אשראי ידוע, צ׳קים, הכנסות והוצאות קבועות בכל חודש עד התאריך, כולל. רכישות אשראי עתידיות שטרם התקבלו אינן כלולות בתחזית.</p><p data-cashflow-date-error class="cashflow-breach" role="status" hidden></p></div><div data-cashflow-result>${cashflowExplorerResultMarkup(cashflow)}</div></div>`;
}
...
export function cashflowBreakdownMarkup(cashflow){
  ...
  const income=(cashflow.incomeRows||[]).map(row=>({label:row.name,date:row.dueDate,amount:row.amount,estimated:true,note:`לפי זיכוי בנק מ־${date(row.sourceDate)}${row.bankSettlementState==='awaiting'?' · ממתין לזיכוי בבנק':''}`}));
...
function bankRecurringIncomeMarkup(cashflow){
  const sources=(cashflow.recurringIncomeObligations||[]).filter(row=>row.lastCredit),warnings=cashflow.recurringIncomeWarnings||[];
  if(!sources.length&&!warnings.length)return '';
  return `<section class="cashflow-breakdown-section"><h3>הכנסות קבועות במעקב בנקאי</h3><p class="muted">קצבת הילדים מחושבת כאומדן ל־20 בכל חודש לפי הזיכוי האחרון. זיכוי סופי באותו חודש, מוקדם או מאוחר, מחליף את האומדן. הקדמה בחודש מסוים אינה משנה את מועד האומדן הבא.</p><ul>${sources.map(row=>`<li><b>${esc(row.name)}</b>: זיכוי אחרון ${esc(date(row.lastCredit.date))} · ${esc(money(row.lastCredit.amount))}; אומדן ממתין / הבא ${esc(date(row.nextDueDate))} · ${esc(money(row.nextAmount))}</li>`).join('')}</ul>${warnings.length?'<aside class="soft-note">התחזית חלקית: נתוני קצבת הילדים חסרים או שיש כמה זיכויים באותו חודש. לא נבחר סכום חדש מתוך זיכויים מסופקים; יש לבדוק את התנועות בבנק.</aside>':''}</section>`;
}
