-- Preserve verified credit-card settlement identity returned by Hapoalim.
-- Only explicit card detail fields are stored; full PAN values never leave the local bridge.
...
comment on column public.bank_transactions.credit_settlement_details is
  'Normalized credit settlement metadata. cardLast4s is populated only from explicit bank-labelled card fields; issuer/permission references stay separate.';
