...
ok('waitForHapoalimSessionReady' in server and 'window.bnhpApp?.restContext' in server and '/ServerServices/general/accounts' in server and 'HAPOALIM_POST_LOGIN_TIMEOUT_MS' in server, 'bank post-login readiness: visible homepage is not treated as API-ready until SPA context and accounts endpoint both respond')
...
ok('/current-account/transactions?' in server and "method:'POST'" in server and 'normalizeRecentTransactions' in server and 'HAPOALIM_TRANSACTION_LOOKBACK_DAYS' in server and 'HAPOALIM_TRANSACTION_LIMIT' in server, 'bank transactions: recent Hapoalim transactions are fetched separately with bounded lookback and row count')
ok("catch(e){transactionWarning=" in server and 'return {balance,accountNumber:selected.accountNumber,branchNumber:selected.branchNumber,accountId,transactions,transactionWarning}' in server, 'bank partial success: transaction failure is non-fatal after a valid balance has been read')
...
ok('const BRIDGE_VERSION=6' in server and 'version:BRIDGE_VERSION' in server and 'Number(j.version)>=6' in installer and '/health' in installer, 'bank bridge install: installer verifies session-aware bridge v6 after hidden startup')
...
ok("refreshBankBalance({interactive:false,auto:true})" in controller and "'refresh-bank-from-hapoalim':(element,event)=>{refreshBankBalance({interactive:false,auto:false})}" in actions and "'open-bank-auth':(element,event)=>{refreshBankBalance({interactive:true,auto:false})}" in actions, 'bank refresh modes: automatic/manual refresh are silent; visible authentication is explicit')
...
ok('bridgeVersion:BRIDGE_VERSION' in server and 'upgradeRequired=bridgeVersion<6' in controller, 'bank bridge upgrade handshake: site rejects pre-v6 local runtime before a bank data refresh')

ok('BANK_FEED_VERSION=1' in feed and 'BANK_FEED_TRANSACTION_LIMIT=20' in feed and 'transactionWarning' in feed, 'bank shared contract: canonical versioned feed has a bounded transaction list and partial-success warning')
...
ok('עדכון הבנק האחרון הצליח' in view and 'syncTimeLabel(lastSync)' in view and 'bank-transactions-list' in view and 'תנועות אחרונות מהבנק' in view, 'bank UI: successful last-update time and recent transactions have dedicated visible presentation')
