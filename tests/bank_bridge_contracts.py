...
ok('const BRIDGE_VERSION=17' in server and 'version:BRIDGE_VERSION' in server and 'Number(j.version)>=17' in installer and '/health' in installer, 'financial bridge install: installer verifies the Camoufox fingerprint-hardening bridge v17 after hidden startup')
...
ok('createSaneCamoufoxFingerprint' in camoufox and 'sanitizeCamoufoxFingerprint' in camoufox and 'camoufoxFingerprintLooksSane' in camoufox and 'openQualifiedLoginSession' in camoufox and "fingerprint,i_know_what_im_doing:true" in camoufox and 'detector-dom.min.js' in camoufox, 'Amex WAF root fix: the adapter filters known BrowserForge consistency leaks, qualifies the anonymous login session before credentials, and preserves the upstream detector-script block')
