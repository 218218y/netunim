...
ok('const BRIDGE_VERSION=20' in server and 'version:BRIDGE_VERSION' in server and 'Number(j.version)>=20' in installer and '/health' in installer, 'financial bridge install: installer verifies Bridge v20 after hidden startup')
...
ok('bridgeVersion:BRIDGE_VERSION' in server and 'const BANK_BRIDGE_VERSION=19' in controller and 'upgradeRequired=bridgeVersion<BANK_BRIDGE_VERSION' in controller, 'bank bridge upgrade handshake: site rejects pre-v19 local runtime before dual-account refresh')
...
ok('<details class="bank-sync-settings">' in view and '<summary class="bank-sync-toolbar">' in view and 'id="bankSyncHeadline"' in view and 'bank-sync-head-actions' in view and 'bank-sync-chevron' in view and view.index('<div class="bank-transactions-region">')>view.index('</details>') and 'רענן שני חשבונות' in view, 'bank UI: the sync result stays in the collapsed disclosure toolbar, one refresh updates both accounts, and role-switchable transactions begin immediately below it')
