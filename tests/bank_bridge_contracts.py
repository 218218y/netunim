...
ok('const BRIDGE_VERSION=18' in server and 'version:BRIDGE_VERSION' in server and 'Number(j.version)>=18' in installer and '/health' in installer, 'financial bridge install: installer verifies Bridge v18 after hidden startup')
...
ok('creditDetailPartitionsData' in credit_model and 'CREDIT_DETAIL_HISTORY_DAYS=60' in credit_model and 'היסטוריית עסקאות שהסתיימו' in credit_view and 'עסקאות ותשלומים פעילים' in credit_view and "creditDetailGroupedRows(detailItems,{focusMonthKey:detailFocus?.monthKey||''})" in credit_view and 'creditDetailGroupedRows(items,{historical:true})' in credit_view and 'credit-detail-month-divider' in credit_view and '.credit-detail-month-divider th' in app_css, 'credit detail lifecycle: active and completed rows stay chronologically ordered but render with explicit month boundaries, while completed zero-remaining series remain in a separate bounded 60-day history')
...
ok('credit-forecast-month' in credit_view and 'credit-forecast-row' in credit_view and 'credit-forecast-breakdown' in credit_view and 'creditForecastColumns' in credit_view and 'Math.ceil(months.length/2)' in credit_view and credit_view.index('עסקאות ותשלומים פעילים') < credit_view.index('${creditMonthlySummaryDetails(future)}') < credit_view.index("${summary.hasData?renderSyncedAccounts(summary):''}"), 'credit UI hierarchy: balanced two-column expandable monthly forecast leads into active transactions, then collapsed monthly summary, with live issuer data last')
...
ok('showCompleted=Number(series.totalParts)>1' in credit_view and 'showCompleted=Number(cr.installments)>1' in credit_view and '<span class=\"muted\">מהחברה</span>' not in credit_view, 'credit detail polish: completed-count helper is reserved for installment series and synchronized rows do not carry a redundant issuer-source tail label')
ok("'credit-detail-focus'" in actions and "'clear-credit-detail-focus'" in actions and "creditDetailFocus:null" in contexts and 'detailItemMatchesFocus' in credit_view and 'scrollIntoView' in actions and 'data-click-arg1' in credit_view and 'credit-active-transactions' in credit_view, 'credit forecast drilldown: a month/card click uses separate transient detail focus, filters active transactions by that card and month, and scrolls to the detail section without replacing the primary card filter')
