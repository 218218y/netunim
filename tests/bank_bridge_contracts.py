...
ok('const BRIDGE_VERSION=8' in server and 'version:BRIDGE_VERSION' in server and 'Number(j.version)>=8' in installer and '/health' in installer, 'bank bridge install: installer verifies full-feed bridge v8 after hidden startup')
...
ok('bridgeVersion:BRIDGE_VERSION' in server and 'upgradeRequired=bridgeVersion<8' in controller, 'bank bridge upgrade handshake: site rejects pre-v8 local runtime before a bank data refresh')

ok('BANK_FEED_VERSION=2' in feed and 'BANK_FEED_TRANSACTION_LIMIT=1000' in feed and 'balanceAfter' in feed and 'transactionWarning' in feed, 'bank shared contract: v2 feed keeps the full thirty-day page plus bank-provided per-transaction balances')
