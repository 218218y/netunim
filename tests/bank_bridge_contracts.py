...
ok("'credit-account-filter'" in actions and "'credit-owner-filter'" in actions and "'credit-provider-filter'" in actions and "'credit-card-filter'" in actions and "creditAccountFilter:'all'" in contexts and "creditOwnerFilter:'all'" in contexts and "creditProviderFilter:'all'" in contexts and "creditCardFilter:'all'" in contexts and 'filterMatch(ui,row)' in credit_view, 'credit filters: account, owner, issuer and individual-card dimensions are independent and composable across the page')
...
ok('primaryCardFilterMatch' in credit_view and 'includedCards.filter(card=>primaryCardFilterMatch(ui,card))' in credit_view and "const provider=ui.creditProviderFilter||'all'" in credit_view and "if(provider==='all')return ''" in credit_view and 'card.provider===provider' in credit_view and "ui.creditProviderFilter==='all'||(ui.creditCardFilter!=='all'&&!availableCardKeys.has(ui.creditCardFilter))" in credit_view, 'credit filter controls: account and owner constrain issuer availability, while individual card controls stay hidden until one issuer is selected and stale card selections reset safely')
...
ok('grid-template-columns:210px minmax(0,1fr)' in app_css and '.credit-forecast-column{flex:1 1 0' in app_css and '.credit-forecast-list{flex-direction:column}' in app_css, 'credit layout: desktop navigation is narrower and forecast columns stay balanced on desktop while collapsing safely on compact screens')
