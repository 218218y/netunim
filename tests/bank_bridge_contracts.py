...
ok('const BRIDGE_VERSION=13' in server and 'version:BRIDGE_VERSION' in server and 'Number(j.version)>=13' in installer and '/health' in installer, 'financial bridge install: installer verifies the credit-reset/dedup bridge v13 after hidden startup')
...
ok('CREDIT_LOGIN_HTML_RESPONSE' in lib and 'creditThrownScrapeFailure' in server and 'safeCreditErrorMessage' in credit_feed and 'Sisma' in credit_feed, 'credit diagnostics security: issuer HTML/technical failures are classified without persisting credential-bearing scraper payloads')
...
ok('סיכום חודשי לפי כרטיס' in credit_view and 'סה״כ לכרטיס' in credit_view and 'creditMonthlySummaryMarkup' in credit_view, 'credit summary UI: synchronized selected cards have a monthly card-by-card matrix plus monthly and grand totals')
ok('creditCardMappingKey(profile.profileId,account.accountNumber)' in credit_view and 'set-credit-card-included' in credit_view and "option ${accountClass==='עסקי'" in credit_view, 'credit classification: every discovered card has explicit include/exclude plus independent business/home mapping')
...
ok("creditSync:{version:2,mode:'manual'" in contexts and 'n.creditSync=normalizeCreditSync(n.creditSync)' in normalization, 'credit state: first deployment defaults to non-destructive manual mode and normalizes the synchronized feed')
