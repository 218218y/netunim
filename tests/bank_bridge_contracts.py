...
ok('const BRIDGE_VERSION=7' in server and 'version:BRIDGE_VERSION' in server and 'Number(j.version)>=7' in installer and '/health' in installer, 'bank bridge install: installer verifies navigation-stable bridge v7 after hidden startup')
...
ok('bridgeVersion:BRIDGE_VERSION' in server and 'upgradeRequired=bridgeVersion<7' in controller, 'bank bridge upgrade handshake: site rejects pre-v7 local runtime before a bank data refresh')
...
ok('<details class="bank-sync-settings">' in view and '<summary>' in view and 'הגדרות חיבור וסנכרון' in view and 'bank-sync-chevron' in view, 'bank UI: advanced Hapoalim pairing/account options are collapsed by default behind a semantic disclosure while refresh/auth and last-sync diagnostics stay visible')
