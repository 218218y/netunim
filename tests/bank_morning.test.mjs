...
const debtPrefill=debtMorningPrefill({...pickerDebts[0],orderNumber:'LEGACY-ORDER'});assert.equal(debtPrefill.taxId,'123456782');assert.equal(debtPrefill.orderNumber,'');assert.equal(debtPrefill.remarks,'אשראי נסלק באתר ת.ב.י רהיטים. אישור סליקה באתר - 1234');assert.equal(debtMorningPrefill({customerId:'123456782'}).remarks,'','no clearing approval means Morning remarks stay completely empty');
