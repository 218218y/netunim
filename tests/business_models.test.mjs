...
test('alert center presents readable cashflow/check cards and updates the persistent header indicator',()=>{
 ...
 const previousDocument=globalThis.document;let captured=null,closed=0,checkTarget='',cashflowTarget='',directDeposits=0;
 globalThis.document={getElementById:id=>id==='alertCenterButton'?button:id==='alertCenterSlot'?slot:id==='alertCenterCount'?count:null};
 try{
  const center=createUiAlertCenter({model,financeSnapshot:()=>({kupa:{bank:{currentBalance:-250,adjustments:[]},credits:[],expenses:[],cashflowSettings:{}}}),modal:(title,body,foot)=>{captured={title,body,foot}},closeModal:()=>{closed++},navigateToChecks:id=>{checkTarget=id},navigateToCashflow:account=>{cashflowTarget=account},markCheckDeposited:id=>{const check=model.state.checks.find(row=>row.id===id);if(!check||check.status!=='בקופה')return false;check.status='הופקד - במעקב';directDeposits++;return true}});
  assert.equal(center.refreshIndicator().length,2);assert.equal(count.textContent,'2');assert.equal(count.hidden,false);assert.equal(button.hidden,false);assert.equal(slot.hidden,false);assert.equal(classState.has('active'),true);
  assert.equal(center.showStartupAlerts(),true);assert.equal(captured.title,'התראות בפתיחת המערכת');assert.match(captured.body,/alert-center-card cashflow-warning alert-center-card-action/);assert.match(captured.body,/data-action="open-alert-target"/);assert.match(captured.body,/יתרה צפויה/);assert.match(captured.body,/alert-center-card check-warning alert-center-check-card/);assert.match(captured.body,/data-action="mark-alert-check-deposited"/);assert.match(captured.body,/>הופקד<\/button>/);assert.match(captured.body,/לקוח לבדיקה/);assert.match(captured.body,/מס׳ צ׳ק 12345/);assert.match(captured.body,/להפקיד בסניף/);assert.match(captured.body,/סימן האזהרה בראש המסך/);
  ...
  assert.equal(center.markAlertCheckDeposited('DUE'),true,'the direct warning action deposits the same check without navigation');assert.equal(directDeposits,1);assert.equal(model.state.checks[0].status,'הופקד - במעקב');assert.equal(count.textContent,'1','depositing a due check removes only its alert immediately');assert.doesNotMatch(captured.body,/לקוח לבדיקה/,'the open warning modal is refreshed after direct deposit');assert.match(captured.body,/יתרה צפויה/,'other warnings remain visible after direct deposit');
  assert.equal(center.markAlertCheckDeposited('DUE'),false,'an already deposited check cannot be reopened by the warning action');
...
test('customer order tracking uses the clean manual row model for inline add, edit and delete',async()=>{
 const state={customerDebts:[],customerOrders:[{id:'O1',orderNumber:'10',customerName:'קיים',mark1:'V',mark2:'VV',mark3:'VVV',mattresses:'זוגי',note:'הערה',urgent:true}]};
 const customerUi={customerTab:'orders',customerFilter:'all',customerSearch:'קיים',customerBulkSelected:new Set(['O1']),customerBulkMode:false};
 ...
 assert.match(existing,/data-blur-arg1="orderNumber"/);assert.match(existing,/data-blur-arg1="mark3"/);assert.match(existing,/value="זוגי"/);assert.match(existing,/data-label="חובות"/);assert.match(existing,/customer-order-urgent/);assert.match(existing,/data-action="toggle-customer-order-urgent"/);assert.doesNotMatch(existing,/data-action="add-customer-order"/);assert.match(existing,/data-action="delete-customer-order"/);
 const id=editor.addCustomerOrder();assert.ok(state.customerOrders.some(x=>x.id===id));assert.equal(customerUi.customerSearch,'');
 const added=state.customerOrders.find(x=>x.id===id);assert.deepEqual({orderNumber:added.orderNumber,customerName:added.customerName,mark1:added.mark1,mark2:added.mark2,mark3:added.mark3,mattresses:added.mattresses,note:added.note,urgent:added.urgent},{orderNumber:'',customerName:'',mark1:'',mark2:'',mark3:'',mattresses:'',note:'',urgent:false});
 ...
 editor.saveCustomerOrderField(id,'orderNumber',{value:'20'});editor.saveCustomerOrderField(id,'customerName',{value:'חדש'});editor.saveCustomerOrderField(id,'mark1',{value:'א'});editor.saveCustomerOrderField(id,'mark2',{value:'ב'});editor.saveCustomerOrderField(id,'mark3',{value:'ג'});editor.saveCustomerOrderField(id,'mattresses',{value:'זוגי'});editor.saveCustomerOrderField(id,'note',{value:'בדיקה'});editor.saveCustomerOrderField(id,'urgent',{value:'true'});
 assert.deepEqual({orderNumber:added.orderNumber,customerName:added.customerName,mark1:added.mark1,mark2:added.mark2,mark3:added.mark3,mattresses:added.mattresses,note:added.note,urgent:added.urgent},{orderNumber:'20',customerName:'חדש',mark1:'א',mark2:'ב',mark3:'ג',mattresses:'זוגי',note:'בדיקה',urgent:true});
 await editor.deleteCustomerOrder('O1');assert.equal(state.customerOrders.some(x=>x.id==='O1'),false);assert.equal(customerUi.customerBulkSelected.has('O1'),false);assert.equal(saved,10);assert.equal(rendered,2);
});

test('customer order table keeps dedicated header/mobile add actions, has no mattress filters, and exposes urgent treatment',()=>{
 const state={customerDebts:[],customerOrders:[{id:'O1',orderNumber:'1',customerName:'לקוח',mark3:'VVV'}]};
 const customerUi={customerTab:'orders',customerFilter:'all',customerSearch:'',customerBulkSelected:new Set(),customerBulkMode:false};
 ...
 try{
  ...
  view.renderCustomers();assert.match(main.innerHTML,/>חובות<\/th>/);assert.match(main.innerHTML,/customer-order-header-add/);assert.match(main.innerHTML,/customer-order-mobile-add/);assert.match(main.innerHTML,/data-action="toggle-customer-order-urgent"/);assert.doesNotMatch(view.customerOrderRow(state.customerOrders[0]),/data-action="add-customer-order"/);assert.doesNotMatch(main.innerHTML,/data-action="customer-order-filter/);assert.doesNotMatch(main.innerHTML,/>הכל<\/button><button[^>]+>מזרונים<\/button>/);assert.doesNotMatch(main.innerHTML,/דורש תשומת לב/);assert.doesNotMatch(main.innerHTML,/סימוני V \/ VV \/ VVV/);
 }finally{if(previousDocument===undefined)delete globalThis.document;else globalThis.document=previousDocument}
});
