...
const isracardCompleteDiagnostic=isracardDiagnosticEvents.find(event=>event.stage==='Complete');assert(isracardCompleteDiagnostic,'Isracard adapter emits a completion diagnostic');assert.equal(isracardCompleteDiagnostic.syncMode,'daily');assert.equal(isracardCompleteDiagnostic.browserMode,'headless');assert.equal(isracardCompleteDiagnostic.browserProduct,'chrome');assert.equal(isracardCompleteDiagnostic.profileId,isracardProfile.profileId,'credit diagnostics identify the exact profile without exposing credentials');
