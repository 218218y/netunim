...
import {filterExcludedGroupCards,isApprovedTransactionSettled,preserveInstalledChromiumIdentity,unsettledApprovedTransactions} from '../netunim-kupa/bank-bridge/isracard-group-utils.mjs';

assert.equal(CREDIT_CONNECTOR_CONTRACT_VERSION,2);

const settledApproval={purchaseDate:'07/09/2026',businessName:' בית עסק ',originalAmount:75,currencyIso:'ILS'},settledVoucher={purchaseDate:'07/09/2026',businessName:'בית עסק',originalAmount:75,originalCurrencyIso:'ILS'};
assert.equal(isApprovedTransactionSettled(settledApproval,[settledVoucher]),true,'DigitalV3 settlement matching uses the upstream PR #1159 purchase-date/amount/currency/merchant contract');
assert.deepEqual(unsettledApprovedTransactions([settledApproval,{...settledApproval,businessName:'עסק אחר'}],[],[{immediateVouchersCurrencyDate:[settledVoucher]}]).map(row=>row.businessName),['עסק אחר'],'immediate-debit vouchers suppress the same pending approval just like statement vouchers');
const excludedSeen=[];assert.deepEqual(filterExcludedGroupCards([{cardSuffix:'1111'},{cardSuffix:'2222'}],['1111'],suffix=>excludedSeen.push(suffix)).map(row=>row.cardSuffix),['2222']);assert.deepEqual(excludedSeen,['1111'],'excluded Isracard-family cards are removed immediately after discovery, before per-card data calls');
...
const quickScope=creditSyncScope({syncMode:'quick',now:fixedNow}),forecastScope=creditSyncScope({syncMode:'forecast',now:fixedNow}),recoveryScope=creditSyncScope({syncMode:'recovery',now:fixedNow});
assert.equal(quickScope.startDate.toISOString().slice(0,10),'2026-08-04','quick scope reaches 30 days back');assert.equal(quickScope.futureMonths,1,'quick scope reaches one month forward');
assert.equal(forecastScope.startDate.toISOString().slice(0,10),'2026-08-04','forecast scope also rechecks the recent 30 days');assert.equal(forecastScope.futureMonths,12,'forecast scope reaches twelve months forward');
assert.equal(recoveryScope.startDate.toISOString().slice(0,10),'2026-04-26','recovery scope preserves the 130-day history window');assert.equal(recoveryScope.futureMonths,12,'recovery also rebuilds the twelve-month forecast');
assert.equal(creditSyncScope({syncMode:'daily',now:fixedNow}).syncMode,'quick','legacy daily requests remain backwards compatible');assert.equal(creditSyncScope({syncMode:'full',now:fixedNow}).syncMode,'recovery','legacy full requests keep their old heavy recovery meaning');
...
assert.equal(genericResult.accounts[0].months.find(row=>row.month==='2026-09').transactions[0].transactionTime,'14:27','MAX maps the live provider field dealData.purchaseTime even when purchaseDate itself is date-only midnight');
...
assert.equal(genericResult.accounts[0].pendingTransactions[0].transactionTime,'05:10','MAX pending rows use the same proven dealData.purchaseTime provider field');
...
assert.deepEqual(maxRawTransactionTime({purchaseDate:'2026-09-14T00:00:00',dealData:{purchaseTime:'14:27'}}),{time:'14:27',sourcePath:'dealData.purchaseTime',ambiguous:false,candidates:[{path:'dealData.purchaseTime',time:'14:27',value:'14:27'}]});
assert.equal(maxRawTransactionTime({dealData:{purchaseTime:'09:42:00'}}).time,'09:42','MAX accepts the exact live purchase-time field including seconds');
assert.equal(maxRawTransactionTime({transactionTime:'14:27',purchaseTime:'14:28',dealData:{purchaseHour:'0510'}}).time,'','unproven look-alike time fields never influence production MAX mapping');
...
const maxExcludedEvents=[];const maxExcludedAdapter=new MaxAdapter({profile:{profileId:'max-excluded',provider:'max',label:'MAX excluded',credentials:{username:'u',password:'p'}},companyId:'max',excludedAccountNumbers:['9999'],createScraper:()=>({scrape:async()=>({success:true,accounts:[{accountNumber:'4444',txns:[{identifier:'keep-max',status:'completed',date:'2026-09-01T00:00:00Z',processedDate:'2026-09-10T00:00:00Z',chargedAmount:-10,chargedCurrency:'ILS'}]},{accountNumber:'9999',txns:[{identifier:'drop-max',status:'completed',date:'2026-09-01T00:00:00Z',processedDate:'2026-09-10T00:00:00Z',chargedAmount:-20,chargedCurrency:'ILS',rawTransaction:{shortCardNumber:'9999'}}]}]})}),now:()=>new Date(fixedNow),syncMode:'quick',onDiagnostic:event=>maxExcludedEvents.push(event)});
const maxExcludedResult=await maxExcludedAdapter.scrape();assert.deepEqual(maxExcludedResult.accounts.map(row=>row.accountNumber),['4444'],'excluded MAX accounts never reach normalized/cloud connector output even though upstream v6.10 fetches an all-card month');assert.equal(maxExcludedResult._dataDiagnostics.some(row=>row.accountNumber==='9999'),false,'excluded MAX accounts never enter local raw data diagnostics');assert.equal(maxExcludedEvents.some(event=>event.stage==='CardExcluded'&&event.accountSuffix==='9999'),true,'MAX exclusion is observable without exposing card details');
...
assert.deepEqual(isracardDigitalOptions.excludedAccountNumbers,['0000'],'Isracard native DigitalV3 receives the authoritative excluded-card selection before its per-card data loop');
const isracardCompleteDiagnostic=isracardDiagnosticEvents.find(event=>event.stage==='Complete');assert(isracardCompleteDiagnostic,'Isracard adapter emits a completion diagnostic');assert.equal(isracardCompleteDiagnostic.syncMode,'quick');assert.equal(isracardCompleteDiagnostic.browserMode,'headless');assert.equal(isracardCompleteDiagnostic.browserProduct,'chrome');assert.equal(isracardCompleteDiagnostic.profileId,isracardProfile.profileId,'credit diagnostics identify the exact profile without exposing credentials');
...
assert.equal(amexDigitalOptions.futureMonthsToScrape,1,'quick Amex DigitalV3 keeps the bounded recent+next billing horizon');
...
function chromiumIdentityFixture({product='chrome',withBrandedHints=true}={}){
  const isEdge=product==='edge',major='152',full='152.0.8112.50',productBrand=isEdge?'Microsoft Edge':'Google Chrome';
  const state={userAgent:'',userAgentMetadata:null,userAgentPlatform:'',preDocument:null,interception:false,requestHandler:null};
  ...
  const brands=[{brand:'Not(A:Brand',version:'99'},{brand:'Chromium',version:major},...(withBrandedHints?[{brand:productBrand,version:major}]:[])];
  const fullVersionList=[{brand:'Not(A:Brand',version:'99.0.0.0'},{brand:'Chromium',version:full},...(withBrandedHints?[{brand:productBrand,version:full}]:[])];
  return {state,page:{
    evaluate:async()=>({userAgent,navigatorPlatform:'Win32',userAgentData:{brands,mobile:false,platform:'Windows',architecture:'x86',bitness:'64',model:'',platformVersion:'19.0.0',uaFullVersion:full,fullVersionList,wow64:false}}),
    setUserAgent:async value=>{state.userAgent=typeof value==='string'?value:value.userAgent;state.userAgentMetadata=typeof value==='string'?null:value.userAgentMetadata;state.userAgentPlatform=typeof value==='string'?'':value.platform||''},
...
assert(amexIdentityFixture.state.userAgentMetadata.brands.some(row=>row.brand==='Google Chrome'&&row.version==='152'),'the installed Chrome low-entropy brand is preserved in native Client Hints metadata');
assert(amexIdentityFixture.state.userAgentMetadata.fullVersionList.some(row=>row.brand==='Google Chrome'&&row.version==='152.0.8112.50'),'the installed Chrome full-version brand is preserved exactly, not synthesized');
...
let normalContinue=null;amexIdentityFixture.state.requestHandler({url:()=> 'https://he.americanexpress.co.il/personalarea/Login',abort:()=>{throw new Error('normal request must continue')},continue:(overrides,priority)=>{normalContinue={overrides,priority}}});await new Promise(resolve=>setImmediate(resolve));assert.equal(normalContinue.priority,10,'DigitalV3 login keeps upstream cooperative request flow; Client Hints are preserved via CDP identity metadata, not per-request header rewriting');
const edgeIdentityFixture=chromiumIdentityFixture({product:'edge'}),edgeIdentity=await preserveInstalledChromiumIdentity(edgeIdentityFixture.page);
assert.equal(edgeIdentity.product,'edge');assert.equal(edgeIdentity.clientHintsState,'preserved');assert(edgeIdentityFixture.state.userAgentMetadata.brands.some(row=>row.brand==='Microsoft Edge'),'installed Edge keeps its native branded Client Hints instead of being rewritten as Chrome');
const unbrandedIdentityFixture=chromiumIdentityFixture({withBrandedHints:false});await assert.rejects(()=>prepareAmexDigitalV3Page(unbrandedIdentityFixture.page),error=>error.code==='CREDIT_BROWSER_IDENTITY_UNAVAILABLE'&&error.stage==='BrowserIdentity'&&error.clientHintsState==='unbranded','Amex fails closed before issuer navigation when Chrome/Edge cannot expose branded native Client Hints');assert.equal(unbrandedIdentityFixture.state.userAgent,'','an incomplete browser identity is never applied to the page');
...
const irrelevantFramesFixture=fakeScraper(),irrelevantFramesResult=await adapterFor(irrelevantFramesFixture.scraper,fetchFixture({framesBody:{statusCode:87,statusTitle:'לא נמצאו כרטיסים רלוונטיים להצגה',result:null}}),'quick').scrape();
...
await assert.rejects(()=>adapterFor(loginTimeoutFixture.scraper,fetchFixture(),'quick',{onDiagnostic:event=>loginTimeoutEvents.push(event)}).scrape(),error=>error.code==='CREDIT_LOGIN_TIMEOUT'&&error.stage==='LoginFlow');
...
const diagnostic=sanitizeCreditDiagnosticEvent({provider:'amex',profileId:'p',syncMode:'forecast',browserMode:'headless',browserProduct:'edge',clientHintsState:'preserved',browserMajorVersion:152,userAgent:'Mozilla secret fingerprint',userAgentMetadata:{brands:[{brand:'secret',version:'152'}]},stage:'LoginPage',httpStatus:403,responseShape:shape,startupFailureReason:'timeout',identityState:'legacy_unverified',profileRecovery:'fresh_profile',launchAttempt:2,providerStatus:'2',providerReturnCode:'E',username:'secret-user',password:'secret-password',rawHtml:'<html>secret</html>',authorization:'Bearer secret'}),serializedDiagnostic=JSON.stringify(diagnostic);
assert.equal(serializedDiagnostic.includes('secret'),false,'diagnostics use an allowlist and cannot retain credentials, tokens, raw HTML, User-Agent or raw Client Hints');
assert.equal(diagnostic.httpStatus,403);assert.equal(diagnostic.syncMode,'forecast');assert.equal(diagnostic.browserMode,'headless');assert.equal(diagnostic.browserProduct,'edge');assert.equal(diagnostic.clientHintsState,'preserved');assert.equal(diagnostic.browserMajorVersion,152);assert.equal(diagnostic.fingerprint.length,16);assert.equal(diagnostic.startupFailureReason,'timeout');assert.equal(diagnostic.identityState,'legacy_unverified');assert.equal(diagnostic.profileRecovery,'fresh_profile');assert.equal(diagnostic.launchAttempt,2);assert.equal(diagnostic.providerStatus,'2');assert.equal(diagnostic.providerReturnCode,'E');
