...
const currentMonthCarryState={credits:[],creditSync:currentMonthCarrySync},currentMonthForecast=[...creditForecastInstallmentsData(currentMonthCarryState,'2026-09-10'),...syncedPendingForecastData(currentMonthCarryState,'2026-09-10')],currentMonthForecastTotal=currentMonthForecast.filter(row=>String(row.date).startsWith('2026-09')).reduce((sum,row)=>sum+row.amount,0),currentMonthDetailTotal=creditMonthlyDetailData(currentMonthCarryState,'2026-09-10').months.find(month=>month.key==='2026-09').total;
assert.equal(currentMonthForecastTotal,1125,'Kupa credit-page current-month summary keeps completed rows dated earlier in the month together with fresh pending approvals');
assert.equal(currentMonthForecastTotal,currentMonthDetailTotal,'Kupa current-month strip and transactions/payments table use the same complete billing-month total');
assert.equal(creditForecastInstallmentsData(currentMonthCarryState,'2026-09-10').some(row=>row.date==='2026-09-05'&&row.amount===1000),true,'Kupa credit forecast month scope starts at the first day of the current month, not at the current day');
const zeroBilledPendingSync=normalizeCreditSync({version:4,profiles:[{profileId:'pending-zero-billed',provider:'max',accounts:[{accountNumber:'4545',balanceDate:'2026-09-10',pendingStatus:'success',pendingTransactions:[{id:'auth-zero-billed',status:'pending',transactionDate:'2026-09-03',processedDate:'2026-09-03',chargedAmount:0,chargedCurrency:'ILS',originalAmount:-237.4,originalCurrency:'ILS',description:'אישור עם חיוב זמני אפס'}]}]}],cardMappings:{'pending-zero-billed:4545':{included:true,hidden:false}}});
...
assert.equal(zeroBilledPendingDetail.amount,0,'issuer chargedAmount=0 stays zero in the provisional monthly charge until settlement');
...
assert.equal(businessCheckCashflow.checkCutoffDay,14);assert.equal(businessCheckCashflow.checkCutoffDate,'2999-09-14');assert.deepEqual(businessCheckCashflow.checkRows.map(row=>row.id),['biz-in'],'business cash-flow includes only still-held business checks through day 14 inclusive');assert.equal(businessCheckCashflow.checks,100);assert.equal(businessCheckCashflow.expectedChange,-40,'business expected bank change nets the 100 check deposit against 100 credit and 40 expenses');assert.equal(bankProjectedThisMonthData(checkCashflowState,'2999-09-01'),960,'business projected checking adds qualifying business checks after subtracting credit and expenses');
assert.equal(homeCheckCashflow.checkCutoffDay,9);assert.equal(homeCheckCashflow.checkCutoffDate,'2999-09-09');assert.deepEqual(homeCheckCashflow.checkRows.map(row=>row.id),['home-in'],'home cash-flow includes only still-held home checks through day 9 inclusive');assert.equal(homeCheckCashflow.checks,300);assert.equal(homeCheckCashflow.expectedChange,-10,'home expected bank change nets the 300 check deposit against 250 credit and 60 expenses');assert.equal(bankHomeProjectedThisMonthData(checkCashflowState,'2999-09-01'),1990,'home projected checking adds qualifying home checks without leaking business checks');
