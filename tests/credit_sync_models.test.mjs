...
const excludedBaseline=normalizeCreditSync({version:4,syncedAt:'2026-09-01T00:00:00Z',profiles:[{profileId:'excluded-merge',provider:'max',accounts:[{accountNumber:'9999',months:[{month:'2026-09',tier:'core',fetchStatus:'success',fetchedAt:'2026-09-01T00:00:00Z',transactions:[{id:'archived',processedDate:'2026-09-10',chargedAmount:-10,status:'completed'}]}]}]}],cardMappings:{'excluded-merge:9999':{included:false,hidden:true,account:'עסקי'}}});
...
const monthlyReplaced=mergeCreditSyncResult(monthlyBase,{contractVersion:2,syncedAt:'2026-09-02T00:00:00Z',profiles:[{profileId:'monthly',provider:'visaCal',coreComplete:true,accounts:[{accountNumber:'1111',months:[{month:'2026-11',tier:'forecast',fetchStatus:'success',fetchedAt:'2026-09-02T00:00:00Z',transactions:[{id:'replacement',processedDate:'2026-11-10',chargedAmount:-91,status:'completed'}]}]}]}],errors:[]});
assert.deepEqual(monthlyReplaced.profiles[0].accounts[0].months.find(row=>row.month==='2026-11').transactions.map(tx=>tx.id),['replacement'],'a successful refresh replaces one issuer billing-month slice instead of appending rows from the previous refresh');
...
assert.equal(businessCycle.nextCreditTotal,100);assert.equal(businessCycle.targetExpenseTotal,40,'business expenses through mid-month are included');
assert.equal(homeCycle.nextCreditTotal,250);assert.equal(homeCycle.targetExpenseTotal,60,'home expenses through mid-month are included');
assert.equal(bankProjectedThisMonthData(splitAccountState),860,'business projected checking includes the monthly expense window');
assert.equal(bankHomeProjectedThisMonthData(splitAccountState),1690,'home projected checking includes the monthly expense window');
...
assert.equal(businessCheckCashflow.checkCutoffDay,14);assert.equal(businessCheckCashflow.checkCutoffDate,'2999-09-15');assert.deepEqual(businessCheckCashflow.checkRows.map(row=>row.id),['biz-in','biz-out'],'all held business cheques through the common horizon are included');assert.equal(businessCheckCashflow.checks,300);assert.equal(businessCheckCashflow.expectedChange,160,'business expected bank change contains only obligations through the horizon');assert.equal(bankProjectedThisMonthData(checkCashflowState,'2999-09-01'),1160,'business projected checking excludes deposits after the horizon');
assert.equal(homeCheckCashflow.checkCutoffDay,9);assert.equal(homeCheckCashflow.checkCutoffDate,'2999-09-15');assert.deepEqual(homeCheckCashflow.checkRows.map(row=>row.id),['home-in','home-out'],'all held home cheques through the common horizon are included');assert.equal(homeCheckCashflow.checks,700);assert.equal(homeCheckCashflow.expectedChange,390,'home expected bank change nets the qualifying deposit against the next credit cycle');assert.equal(bankHomeProjectedThisMonthData(checkCashflowState,'2999-09-01'),2390,'home projected checking nets the account cheques and expenses');
...
assert.equal(bankNextCycleCommitmentsData(customCutoffState,'2999-09-01').checks,300,'legacy business cutoff no longer truncates the projection');assert.equal(bankHomeNextCycleCommitmentsData(customCutoffState,'2999-09-01').checks,700,'legacy home cutoff no longer truncates the projection');
...
assert.equal(bankNextCycleCommitmentsData(noBusinessBalance,'2999-09-01').targetExpenseTotal,40,'expense obligations remain visible when the bank balance is unavailable');
...
assert.equal(businessAwaitingBank.nextCreditTotal,0,'next month is outside the automatic horizon while the current debit awaits posting');
assert.equal(businessAwaitingBank.credit,1000);assert.equal(businessAwaitingBank.projected,9000,'cash-flow subtracts the current unsettled debit before rolling to next month');
...
assert.equal(homeAwaitingBank.nextCreditTotal,0);assert.equal(homeAwaitingBank.projected,4500,'a pending bank row is not treated as proof that the home balance already contains the debit');
...
assert.equal(businessPosted.settlingCredit,0);assert.equal(businessPosted.credit,1200);assert.equal(businessPosted.projected,7800,'once MAX posts, the current debit is removed and the automatic horizon advances to next month');
assert.equal(homePosted.settlingCredit,0);assert.equal(homePosted.credit,600);assert.equal(homePosted.projected,3900,'home cash-flow removes the posted debit and includes the next monthly cycle');
...
assert.equal(businessHardCap.settlingCredit,1000,'an unmatched cycle remains held until bank settlement');
...
assert.equal(businessHardCap.credit,1000);assert.equal(businessHardCap.projected,9000,'the current debit is retained and next month is not included yet');
...
assert.equal(homeHardCapCycle.settlingCredit,500);assert.equal(homeHardCapCycle.expiredSettlementCredit,500);assert.equal(homeHardCapCycle.expiredSettlementWarnings[0].releaseDate,'2026-09-17','a due-on-15 home cycle stays reserved and gains an unmatched warning on the 17th');
...
assert.equal(lateMatchedCycle.expiredSettlementWarnings.length,0,'a late debit within the monthly cycle proves settlement');
...
assert.equal(splitPostedCycle.projected,4007,'ambiguous old estimates stay reserved within the reconciliation window');
...
const historyState={credits:[],creditSync:normalizeCreditSync({version:3,profiles:[{profileId:'history',provider:'max',accounts:[{accountNumber:'1000',txns:[
  ...
  {id:'edge',processedDate:'2025-09-15',chargedAmount:-20,chargedCurrency:'ILS',description:'ספטמבר קודם'},
  {id:'old',processedDate:'2025-08-01',chargedAmount:-50,chargedCurrency:'ILS',description:'ישן מדי'},
...
assert.equal(CREDIT_DETAIL_HISTORY_MONTHS,12);
assert.deepEqual(monthlyHistory.months.map(x=>x.key),['2025-09','2026-08','2026-10'],'monthly detail keeps twelve prior calendar months plus every actually known future billing month');
...
const identifiedDuplicate=normalizeCreditSync({version:4,profiles:[{profileId:'dedupe',provider:'max',accounts:[{accountNumber:'3333',months:[{month:'2026-09',tier:'core',fetchStatus:'success',fetchedAt:'2026-09-01T00:00:00Z',transactions:[{id:'same-id',status:'completed',date:'2026-08-30T00:00:00Z',processedDate:'2026-09-10T00:00:00Z',chargedAmount:-25,chargedCurrency:'ILS',description:'עסקה'},{id:'same-id',status:'completed',date:'2026-08-30T00:00:00Z',processedDate:'2026-09-10T00:00:00Z',chargedAmount:-25,chargedCurrency:'ILS',description:'עסקה'}]}]}]}]});
assert.equal(identifiedDuplicate.profiles[0].accounts[0].months[0].transactions.length,1,'an exact duplicate with the same non-empty issuer identity inside one billing month is stored once');
...
const creditController=createDomainsCreditController({
  ...
  bridge:{creditStatus:async()=>({bridgeVersion:60,contractVersion:2,profiles:[]})},
  modal:()=>{},armModalDraftGuard:()=>{},closeModal:()=>{},confirmDialog:async()=>true,
});
for(const method of ['creditSyncUiState','refreshCreditBridgeStatus','copySafeCreditDiagnostics','exportCreditDataDiagnostics','openCreditConnectionModal','deleteCreditConnection','resetCreditSync','refreshCreditSync','setCreditCardMapping','setCreditAutoRefresh','setCreditAutoMode','maybeAutoRefreshCreditSync']){
...
assert.equal(creditController.creditSyncUiState().autoMode,'smart','Kupa defaults automatic credit refresh to the smart policy when no preference exists');creditController.setCreditAutoMode('forecast');assert.equal(creditController.creditSyncUiState().autoMode,'forecast','Kupa stores an explicit automatic forecast policy independently of the on/off toggle');creditController.setCreditAutoMode('full');assert.equal(creditController.creditSyncUiState().autoMode,'recovery','legacy full preferences migrate to the explicit recovery policy');creditController.setCreditAutoRefresh(false);

let automaticSyncOptions=null,automaticStatusRenders=0;const automaticModel={state:{creditSync:normalizeCreditSync({})}},automaticController=createDomainsCreditController({model:automaticModel,saveState:async()=>{},saveFinancePatch:async()=>({saved:true}),toast:()=>{},render:()=>{},renderStatus:()=>{automaticStatusRenders++},bridge:{creditStatus:async()=>({bridgeVersion:60,contractVersion:2,profiles:[{profileId:'auto-profile'}]}),syncCreditCards:async options=>{automaticSyncOptions=structuredClone(options);return {syncedAt:new Date().toISOString(),attemptedCount:1,deferredCount:0,profiles:[{profileId:'auto-profile',provider:'max',coreComplete:true,accounts:[]}],errors:[]}}},modal:()=>{},armModalDraftGuard:()=>{},closeModal:()=>{},confirmDialog:async()=>true,refreshFinanceCloudSnapshot:async()=>({verified:true,state:{creditSync:normalizeCreditSync({})}}),claimFinanceSyncLease:async()=>({acquired:true}),releaseFinanceSyncLease:async()=>true});automaticController.setCreditAutoMode('forecast');assert.equal(await automaticController.refreshCreditSync({auto:true}),undefined);assert.equal(automaticSyncOptions.syncMode,'forecast','Kupa automatic refresh honors an explicit forecast policy');automaticController.setCreditAutoMode('smart');automaticSyncOptions=null;assert.equal(await automaticController.refreshCreditSync({auto:true}),undefined);assert.equal(automaticSyncOptions.syncMode,'recovery','Kupa smart automatic refresh bootstraps with recovery when the verified cloud feed has no historical baseline');assert.equal(automaticStatusRenders,2,'Kupa publishes the busy state immediately when each automatic credit refresh starts');automaticController.setCreditAutoRefresh(false);
...
const resetController=createDomainsCreditController({
  ...
  bridge:{creditStatus:async()=>({bridgeVersion:60,contractVersion:2,profiles:[{profileId:'old'}]}),resetCreditProfiles:async()=>{resetBridgeCalls++;return {ok:true,profiles:[]}}},
  modal:()=>{},armModalDraftGuard:()=>{},closeModal:()=>{},confirmDialog:async()=>true,
});
