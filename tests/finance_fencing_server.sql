...
begin
  perform public.save_kupa_document_v6('main',0,'{"version":5,"businessName":"fixture","credits":[],"cash":[],"rights":[],"notes":[],"expenses":[],"cards":[],"notesSheet":{"rows":[],"columns":[]},"bank":{"adjustments":[]}}','fixture-kupa','{}','{}');
  foreach kind in array array['bank','credit'] loop
    ...
    begin
      if kind='bank' then perform public.save_bank_sync_snapshot_v6('main','{}','expired',0,kind,'A',100);
      else perform public.save_finance_sync_document_v6('main',0,'{}','expired','{}',kind,'A',100);end if;
    ...
    execute 'set local role authenticated';
    if kind='bank' then
      perform public.save_bank_sync_snapshot_v6('main','{"currentBalance":200,"source":"hapoalim"}', 'B-snapshot',0,kind,'B',101);
      perform public.sync_bank_transactions_snapshot_v6('test','business','[]',clock_timestamp(),null,null,false,kind,'B',101);
    else
      select revision into r from public.finance_sync_documents where owner_id=auth.uid() and document_name='main';
      perform public.save_finance_sync_document_v6('main',r.revision,'{"creditSync":{"syncedAt":"B"}}','B-credit','{}',kind,'B',101);
    end if;
    ...
    begin
      if kind='bank' then perform public.save_bank_sync_snapshot_v6('main','{"currentBalance":100}','A-snapshot',0,kind,'A',100);
      else perform public.save_finance_sync_document_v6('main',(before_finance->>'revision')::bigint,'{"creditSync":{"syncedAt":"A"}}','A-credit','{}',kind,'A',100);end if;
      raise exception 'stale_holder_wrote';
    exception when sqlstate 'PT409' then if sqlerrm<>'stale_finance_sync_fence' then raise;end if;end;
    -- A valid token with the wrong epoch must also fail (not only the old token).
    begin
      if kind='bank' then perform public.save_bank_sync_snapshot_v6('main','{}','wrong-epoch',0,kind,'B',100);
      else perform public.save_finance_sync_document_v6('main',0,'{}','wrong-epoch','{}',kind,'B',100);end if;
      raise exception 'wrong_epoch_wrote';
    exception when sqlstate 'PT409' then if sqlerrm<>'stale_finance_sync_fence' then raise;end if;end;
    if kind='bank' then
      begin perform public.merge_bank_transactions_v6('test','business','[]',kind,'A',100);raise exception 'stale_merge_wrote';
      exception when sqlstate 'PT409' then if sqlerrm<>'stale_finance_sync_fence' then raise;end if;end;
      begin perform public.sync_bank_transactions_snapshot_v6('test','business','[]',clock_timestamp(),null,null,false,kind,'A',100);raise exception 'stale_archive_wrote';
      exception when sqlstate 'PT409' then if sqlerrm<>'stale_finance_sync_fence' then raise;end if;end;
    end if;
