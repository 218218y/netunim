...
const apiPayload320=api.normalizeInput({operation_id:crypto.randomUUID(),document:{type:320,amount:100,date:'2026-09-08',description:'API payload contract',client:{name:'Test'},payment:[{type:1,date:'2026-09-08',price:100}]}}).payload;assert.equal(Object.prototype.hasOwnProperty.call(apiPayload320,'amount'),false,'the local form amount is not forwarded as an unsupported Morning document field');assert.equal(apiPayload320.income[0].price,100);assert.equal(apiPayload320.payment[0].price,100);
const apiPayload400=api.normalizeInput({operation_id:crypto.randomUUID(),document:{type:400,amount:100,date:'2026-09-08',description:'Receipt API payload contract',client:{name:'Test'},payment:[{type:1,date:'2026-09-08',price:100}]}}).payload;assert.equal(Object.prototype.hasOwnProperty.call(apiPayload400,'amount'),false,'receipt amount is likewise represented only by Morning income/payment fields');assert.equal(apiPayload400.income[0].price,100);assert.equal(apiPayload400.payment[0].price,100);
