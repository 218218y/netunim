...
const ordersAckId='credit_settlement_unmatched:עסקי:2026-09-10:max:sync:p:1111';
const ordersAck=normalizeCreditSync({version:4,settlementWarningAcks:{[ordersAckId]:'2026-09-12T08:00:00Z'}});
assert.equal(ordersAck.settlementWarningAcks[ordersAckId],'2026-09-12T08:00:00.000Z','Orders keeps the same durable unmatched-settlement acknowledgement schema as Kupa');
...
const ordersPendingLegacy=normalizeCreditSync({version:3,profiles:[{profileId:'pending-legacy',provider:'max',accounts:[{accountNumber:'5656',pendingFetchedAt:'2026-09-09T08:00:00Z',txns:[{id:'approval-with-date',date:'2026-09-09T10:00:00Z',processedDate:'2026-09-09T10:00:00Z',chargedAmount:-200,chargedCurrency:'ILS',description:'אישור חדש',status:'pending'}]}]}],cardMappings:{'pending-legacy:5656':{included:true,manualFrame:1000}}});
...
assert.equal(creditFrameStatus(ordersPendingLegacyAccount,ordersPendingLegacy.cardMappings['pending-legacy:5656'],'2026-09-09').available,800,'calculated frame fallback subtracts live pending authorizations');
const ordersPendingDirect=normalizeCreditSync({version:4,profiles:[{profileId:'pending-direct',provider:'max',accounts:[{accountNumber:'5757',pendingFetchedAt:'2026-09-09T08:00:00Z',cardFrame:1000,availableCredit:650,pendingStatus:'success',pendingTransactions:[{id:'approval-direct',date:'2026-09-09T10:00:00Z',processedDate:'2026-09-09T10:00:00Z',chargedAmount:-200,chargedCurrency:'ILS',status:'pending'}]}]}],cardMappings:{'pending-direct:5757':{included:true}}});
...
const probeBridge={
  getBridgeToken:()=> 'paired',bankAutoEnabled:()=>false,creditAutoEnabled:()=>false,creditAutoMode:()=> 'smart',setBankAutoEnabled(){},setCreditAutoEnabled(){},setCreditAutoMode(){},setBridgeToken:v=>v,
...
Object.defineProperty(globalThis,'document',{value:{getElementById:id=>id==='main'?drilldownMain:id==='ordersCreditDetailRegion'?{scrollIntoView:()=>{drilldownScrolled++},set outerHTML(value){drilldownMain.innerHTML=drilldownMain.innerHTML.replace(/<section id="ordersCreditDetailRegion"[\s\S]*?<\/section>/,value)}}:null,querySelector:()=>null},configurable:true});
...
drilldownView.setCreditDetailUpcoming('15');
assert.equal(drilldownUi.creditDetailChargeDay,'all','clicking the active nearest-charge day again restores all due days');
drilldownView.setCreditDetailUpcoming();
assert.equal(drilldownUi.creditDetailMode,'upcoming');assert.equal(drilldownUi.creditDetailChargeDay,'all');assert.equal(drilldownUi.creditDetailFocus,null);assert.match(drilldownMain.innerHTML,/credit-upcoming-toggle active/,'nearest-charge main button restores the unfiltered active state');
...
assert.equal(drilldownUi.creditDetailChargeDay,'all','forecast card drilldown never inherits a due-day filter');
...
drilldownView.setCreditDetailMonth('2026-09','15');
assert.equal(drilldownUi.creditDetailChargeDay,'15');assert.deepEqual(drilldownUi.creditDetailFocus,{monthKey:'2026-09',cardKey:''});
assert.match(drilldownMain.innerHTML,/credit-charge-day active[^>]*data-action="orders-credit-detail-month-day"[^>]*data-click-arg0="2026-09"[^>]*data-click-arg1="15"/,'month 15 filter is selected inside the month frame');
drilldownView.setCreditDetailMonth('2026-09','15');
assert.equal(drilldownUi.creditDetailChargeDay,'all','clicking the active month due-day again restores both 10 and 15');
drilldownView.setCreditDetailMonth('2026-09','10');
...
assert.match(rangeUiMain.innerHTML,/<details class="credit-cycle-menu credit-history-menu[^>]*data-dismiss-on-outside[\s\S]*<b>חיובים קודמים<\/b>[\s\S]*data-click-arg0="2026-08"/,'past months use a dismissible compact menu instead of a horizontal heading rail');
assert.match(rangeUiMain.innerHTML,/<details class="credit-cycle-menu credit-future-menu[^>]*data-dismiss-on-outside[\s\S]*<b>חיובים הבאים<\/b>[\s\S]*data-click-arg0="2026-09"/,'current and future months use the same dismissible next-charges menu behavior');
...
const disclosureView=createDomainsFinanceView({
  ...
  controller:{snapshot:()=>({kupa:{bank:{}},bank:{},creditSync:normalizeCreditSync({}),cards:[],credits:[],bankLastSyncAt:null,creditLastSyncAt:null,bankAutoEnabled:false,creditAutoEnabled:false,creditAutoMode:'smart',bridgeTokenConfigured:true,bankBusy:false,creditBusy:false,bankError:'',creditError:'',bankErrorAt:null,creditErrorAt:null,bankStatus:{bridgeVersion:52,configured:true},creditStatus:null,bankStatusChecked:true,creditStatusChecked:true,bankBridgeError:'',creditBridgeError:''})},
  checksView:{syncChecksBulkUi(){},checksCloudLabel:()=>'',checksMarkup:()=>''},dashboardView:{summaryMarkup:()=>''},mountViewLayout(){},modal(){},closeModal(){},confirmDialog:async()=>false,dateEditorMarkup:testDateEditorMarkup,
});
...
const bridge={
  ...
  status:async()=>({bridgeVersion:55,configured:true}),creditStatus:async()=>({bridgeVersion:61,contractVersion:2,profiles:[{profileId:'p1'}]}),
...
const controller=createDomainsFinanceController({
  ...
  saveBankSyncSnapshot,mergeBankTransactions,syncBankTransactionsSnapshot:mergeBankTransactions,readBankTransactions,readBankTransactionSnapshot:async()=>null,
...
assert.deepEqual(lastCreditSyncOptions,{interactive:false,syncMode:'forecast',selection:[]},'ordinary Orders manual credit refresh rechecks the recent 30 days and refreshes the 12-month forecast with the explicit excluded-card selection');
