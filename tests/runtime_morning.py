...
const setReceiptAmount=value=>{const payment=document.querySelector('[data-payment-row] [data-payment-field="price"]');assert(payment,'Missing editable receipt amount');payment.value=String(value);payment.dispatchEvent(new Event('input',{bubbles:true}));assert(Number(document.getElementById('morningAmount').value)===Number(value),'Document total follows payment rows');};
...
state=normalizeState({version:4,customerDebts:[{id:'prefill-only',customerName:'Verified debt',amount:100,paid:false,invoiceIssued:false,closedAt:null,phone:'123',orderNumber:'A1'}]});stageFixtureState('fixture initial debt');
...
setPaymentType(3);assert(transferFields.hidden&&bankFields.hidden&&checkFields.hidden&&!cardFields.hidden,'Credit card shows card fields only');
