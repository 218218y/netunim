            ...
            if label == "orders":
                layout = browser.evaluate(
                    r"""(async()=>{
                      ...
                      customerFilter='invoice';renderCustomers();await frame();const {customerDebtProgressData}=await import('./assets/js/shared/customer-debt-progress.js');const customerInvoiceExpected=money(state.customerDebts.reduce((sum,d)=>sum+customerDebtProgressData(d).remainingInvoice,0)),customerInvoiceTotal=customerVisibleTotal();
