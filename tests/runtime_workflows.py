...
'orders':r"""
 ...
 openStockTransfer(item.id);fill({transferFrom:'לא ידוע',transferTo:'מחסן גדול',transferQty:'5'});saveModal();await saved();
 let balances=inventoryLocationStatsData(state,item.id);assert(balances['לא ידוע'].onHand===7&&balances['מחסן גדול'].onHand===5&&inventoryStats(item.id).onHand===12,'transfer preserves global stock');
 ...
 openStockTransfer(item.id,'לא ידוע');fill({transferTo:'מקלט',transferQty:'5'});saveModal();await saved();assert(state.inventoryEvents.filter(e=>e.type==='transfer').length===1,'transfer cannot consume reserved stock');closeModal();
 ...
 balances=inventoryLocationStatsData(state,item.id);assert(balances['מחסן גדול'].onHand===4&&balances['לא ידוע'].onHand===7,'count affects only selected warehouse');
 ...
 const reservation=state.inventoryEvents.find(e=>e.type==='reserve');pickupReservation(reservation.id);await acceptStyledConfirm();await saved();assert(inventoryLocationStatsData(state,item.id)['לא ידוע'].onHand===4&&inventoryStats(item.id).reserved===0,'pickup debits reservation warehouse');
