...
ok('data-action="refresh-orders-credit-quick"' in orders_finance_view and 'data-action="refresh-orders-credit-recovery"' in orders_finance_view and "refreshOrdersCredit(false,'forecast')" in orders_actions and "refreshOrdersCredit(false,'quick')" in orders_actions and "refreshOrdersCredit(false,'recovery')" in orders_actions and 'resolveCreditAutoSyncMode' in orders_finance_controller and "normalizeCreditFetchMode(syncMode,'forecast')" in orders_finance_controller and "selection:creditSyncScrapeSelection(checksSession.kupaCloudReadState?.creditSync)" in orders_finance_controller and "body:{interactive:!!interactive,syncMode:mode,selection:Array.isArray(selection)?selection:[]}" in (O/'site/assets/js/domains/finance/bridge.js').read_text(encoding='utf-8') and 'value="smart"' in orders_finance_view and 'value="forecast"' in orders_finance_view and 'value="recovery"' in orders_finance_view, 'Orders credit workload controls: main forecast, advanced quick/recovery and smart automatic policy are explicit from UI action through the local bridge request')
