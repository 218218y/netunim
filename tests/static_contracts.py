...
ok("const CREDIT_BRIDGE_VERSION=31" in orders_finance_controller and "CREDIT_CONNECTOR_CONTRACT_VERSION" in orders_finance_controller and "return version>=CREDIT_BRIDGE_VERSION&&contract>=CREDIT_CONNECTOR_CONTRACT_VERSION" in orders_finance_controller,
   "orders Kupa UI: credit controls require Bridge v31 / Credit Connector contract v2 so older bridges cannot silently ignore the corrected Frames semantics")
...
ok('data-action="refresh-orders-credit-daily"' in orders_finance_view and "refreshOrdersCredit(false,'full')" in orders_actions and "refreshOrdersCredit(false,'daily')" in orders_actions and "syncMode:auto?'daily':syncMode==='full'?'full':'daily'" in orders_finance_controller and "selection:creditSyncScrapeSelection(checksSession.kupaCloudReadState?.creditSync)" in orders_finance_controller and "body:{interactive:!!interactive,syncMode:mode,selection:Array.isArray(selection)?selection:[]}" in (O/'site/assets/js/domains/finance/bridge.js').read_text(encoding='utf-8'), 'Orders credit workload controls: main manual full and advanced daily modes are explicit from UI action through the local bridge request')
