...
ok("const CREDIT_BRIDGE_VERSION=30" in orders_finance_controller and "CREDIT_CONNECTOR_CONTRACT_VERSION" in orders_finance_controller and "return version>=CREDIT_BRIDGE_VERSION&&contract>=CREDIT_CONNECTOR_CONTRACT_VERSION" in orders_finance_controller,
   "orders Kupa UI: credit controls require Bridge v30 / Credit Connector contract v2 so older bridges cannot silently ignore syncMode")
...
ok('data-action="refresh-orders-credit-full"' in orders_finance_view and "refreshOrdersCredit(false,'full')" in orders_actions and "body:{interactive:!!interactive,syncMode:mode}" in (O/'site/assets/js/domains/finance/bridge.js').read_text(encoding='utf-8'), 'Orders credit workload controls: daily/full mode is explicit from UI action through the local bridge request')
