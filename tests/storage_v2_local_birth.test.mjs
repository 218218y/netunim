...
function fixture(){
  let plan=null,marker=null,mainState=null,sharedState=null,failAfterMain=false,sourceReads=0,legacyClean=true;
  ...
  const db={
    ...
    async advanceLocalBirth(_scope,id,from,to){if(failAfterMain&&from==='prepared'){failAfterMain=false;throw Error('crash-after-main')};if(plan.id!==id||plan.phase!==from)throw Error('phase-mismatch');plan={...plan,phase:to};return clone(plan)},
  ...
  return {make,cache,source,get plan(){return clone(plan)},get marker(){return clone(marker)},get sourceReads(){return sourceReads},set failAfterMain(value){failAfterMain=value},set legacyClean(value){legacyClean=value}};
}
