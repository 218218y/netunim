...
test('Orders degraded outbox head is not trusted until the failed IndexedDB mirror is repaired',()=>withExpectedConsoleErrors([
 ['browser state mirror','idb unavailable'],['orders outbox IndexedDB','idb unavailable'],
],async()=>{const store=stores(),f=fixture();store.failIDB=true;f.model.state.notes[0].content='repair-me';f.persistence.scheduleSave();clearTimeout(f.session.saveTimer);await f.session.ordersOutboxCommitPromise;await f.files.browserStateWritePromise;assert.equal([...store.db.values()].some(row=>row?.domain==='orders'),false);store.failIDB=false;assert.equal((await f.browser.getCloudPending()).snapshot.notes[0].content,'repair-me');assert.equal([...store.db.values()].some(row=>row?.domain==='orders'&&row.snapshot?.notes?.[0]?.content==='repair-me'),true)}));
