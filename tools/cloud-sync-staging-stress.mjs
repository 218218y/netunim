...
async function stressDocument({label,table,documentName,rpcName,makeState}){
  const before=await readOne(table,documentName),candidate=makeState(structuredClone(before.state)),operationId=`stress-${label}-${crypto.randomUUID()}`;
  const calls=await Promise.all(Array.from({length:concurrency},()=>rpc(rpcName,{p_document_name:documentName,p_expected_revision:Number(before.revision),p_state:candidate,p_operation_id:operationId})));
  ...
  const restore=await rpc(rpcName,{p_document_name:documentName,p_expected_revision:Number(after.revision),p_state:before.state,p_operation_id:`restore-${label}-${crypto.randomUUID()}`});assert.equal(restore.ok,true,`${label} restore failed`);
...
const [orders,kupa]=await Promise.all([
  stressDocument({label:'orders',table:'order_management_documents',documentName:'suppliers',rpcName:'save_order_management_document_v3',makeState:state=>({...state,_syncV3Stress:marker})}),
  stressDocument({label:'kupa',table:'kupa_documents',documentName:'main',rpcName:'save_kupa_document_v3',makeState:state=>({...state,_syncV3Stress:marker})}),
]);

const checks=await stressDocument({
  label:'shared-checks',table:'shared_checks_documents',documentName:'main',rpcName:'save_shared_checks_document_v3',
  makeState:state=>{
...
const finance=await stressDocument({label:'finance',table:'finance_sync_documents',documentName:'main',rpcName:'save_finance_sync_document_v3',makeState:state=>({...state,_syncV3Stress:marker})});
