...
def sync(check=False):
    ...
    for label in ('kupa', 'orders'):
        ...
        shell += ['./'+p.relative_to(site).as_posix() for p in sorted((site/'assets').rglob('*.js'))]
        ...
        for item in shell[1:]+['./_headers']:
            ...
            digest.update((site/item[2:]).read_bytes())
        updated=re.sub(r"const CACHE='[^']+';",f"const CACHE='{label}-app-shell-esm-{digest.hexdigest()[:12]}';",updated)
        if updated != source:
            clean = False
            if check:
